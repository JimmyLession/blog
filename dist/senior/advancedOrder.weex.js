...
var _advancedOrder = __webpack_require__(3);

var _advancedOrder2 = _interopRequireDefault(_advancedOrder);
...
_advancedOrder2.default.el = '#root';
new Vue(_advancedOrder2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\senior\\advancedOrder.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-70bcf37d"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var promptModule = weex.requireModule('promptModule');
// import pop from "../../components/buyGoodsPop.vue";
//新的pop
...
exports.default = {
    components: { title: _titleBar2.default, refresher: _refresh2.default, item: _commodityList2.default, line: _line2.default, pop: _newBuyGoodsPop2.default, empty: _empty2.default },
    data: function data() {
        var _ref;
        ...
        return _ref = {
            ...
            actionType: '',
            depot_id: '',
            isOffsetGoods: false }, _defineProperty(_ref, 'isSelectRegions', false), _defineProperty(_ref, 'buyGoods', null), _defineProperty(_ref, 'buySpec', ''), _defineProperty(_ref, 'enableStyles', {
            lineSpacing: '15px',
            color: '#999999',
            width: 'auto',
            height: '50px',
    ...
    methods: {
        ...
        fetch: function fetch() {
            var self = this;
            self.Get(self.api().tob_list, {}, function (res) {
                if (res.data.code == "0") {
        ...
        getGoodsSku: function getGoodsSku(type) {
            ...
            self.Get(self.api().new_goods_sku, {
                id: self.goods_id,
                is_store: 0,
                address_id: self.address_id
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    ...
                    if (result == 'dismiss') {
                        if (res.data.code == 0) {
                            self.depot_id = res.data.data.depot_id;
                            var goods = res.data.data;
                            if (!self.isSelectRegions || self.isOffsetGoods || !self.goodsSku) {
                                self.goodsSku = goods;
                                self.goodsSku.buyNum = self.goodsSku.default_no;
                                self.goodsSku.other_depot_goods = self.resetDepots(res.data.data.other_depot_goods);
                                self.goodsSku.spec = self.resetSpecs(res.data.data.spec);
                                if (self.goodsSku.spec.length > 0 && self.buySpec) {
                                    self.goodsSku.default_no = self.buySpec.min_count;
                                    self.goodsSku.buyNum = self.goodsSku.default_no;
                                }
                                self.goodsSku.buySpec = self.buySpec;
                            } else {
                                goods.other_depot_goods = self.goodsSku.other_depot_goods;
                                goods.spec = self.goodsSku.spec;
                                goods.default_no = self.goodsSku.default_no;
                                goods.buyNum = self.goodsSku.buyNum;
                                goods.buySpec = self.goodsSku.buySpec;
                                self.goodsSku = goods;
                            }
                            self.isOffsetGoods = false;
                            self.isSelectRegions = false;
                            self.actionType = type;
                            self.buyGoodsRegions = res.data.data.all_region;
                            if (!self.buyGoodsRegions.address_id) {
                                self.buyGoodsRegions.address_id = self.address_id;
                            }
                            self.getShopFee(self.goods_id, self.goodsSku.buyNum);
                            if (!self.showAddCart) self.showAddCart = true;
                        } else {
                            promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                        }
                    }
            ...
            if (datas && datas.length > 0) {
                for (var i = 0; i < datas.length; i++) {
                    ...
                    if (data.goods_id != 0) {
                        ...
                        if (self.depot_id == data.depot_id) {
                            data.checked = true;
        ...
        resetSpecs: function resetSpecs(datas) {
            ...
            if (datas != '' && datas.length > 0) {
                ...
                var spec = {};
                spec.type = 0;
                spec.stock = self.goodsSku.stock;
                self.buySpec = spec;
            }
            return specs;
        },
        ok: function ok(data) {
            ...
            self.buyGoods = data.goods;
            self.buySpec = data.spec;
        ...
        buy: function buy() {
            ...
            self.Post(self.api().buy_now, {
                goods_id: self.buyGoods.id,
                goods_num: self.buyGoods.buyNum,
                spec_type: self.buySpec.type,
                spec_stock: self.buySpec.stock,
                type: 'quick',
                address_id: self.address_id
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    ...
                    if (result == 'dismiss') {
                        if (res.data.code == 0) {
                            self.closeMask();
                            self.jumpTo2('order/confirmOrder', '', { type: 'quick', address_id: self.address_id });
                        } else {
                            promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                        }
                    }
        ...
        shopBuy: function shopBuy() {
            var self = this;
            self.Post(self.api().shop_buy, {
                goods_id: self.buyGoods.id,
                goods_num: self.buyGoods.buyNum,
                spec_type: self.buySpec.type,
                spec_stock: self.buySpec.stock,
                address_id: self.address_id,
                tax_region_id: self.buyGoods.tax_region_id
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    var result = ret.result;
                    if (result == 'dismiss') {
                        if (res.data.code == 0) {
                            ...
                            self.changeCartNum(count);
                            self.closeMask();
                            event.postMessage(self.eventBusTag().RESET_CART);
                        } else {
                            promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                        }
                    }
    ...
    created: function created() {
        var self = this;
        self.id = self.GetUrlPara(weex.config.bundleUrl, 'id');
        self.from = self.GetUrlPara(weex.config.bundleUrl, 'from');
        self.getTopHeight(self);
        if (self.from && self.from == 1) {
            self.isIpx(self);
        } else {
        ...
        goodsAddressEvent.onmessage = function (event) {
            ...
            self.all_region = region;
            self.address_id = region.id;
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  ...
  }) : _vm._e()], 2) : _vm._e()]), _vm._l((_vm.list), function(item, index) {
    return _c('cell', {
      key: index,
      staticClass: ["item"],
      appendAsTree: true,
