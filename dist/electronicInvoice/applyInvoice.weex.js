...
var _applyInvoice = __webpack_require__(2);

var _applyInvoice2 = _interopRequireDefault(_applyInvoice);
...
_applyInvoice2.default.el = '#root';
new Vue(_applyInvoice2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\electronicInvoice\\applyInvoice.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-f0506532"
...
var bottom = 0;
exports.default = {
    components: { title: _titleBar2.default, dialog: _dialog2.default },
    filters: {
    ...
    data: function data() {
        return {
            ...
            bottom: bottom,
            // 个人的信息填写
            valueNameDefault: '请输入姓名',
            valueName: '', //输入的姓名
            // 公司的信息填写
            valueCompanyDefault: '请输入公司全称',
            valueCompany: '', //输入的公司名
            valueABNNumberDefault: '请输入ABN号',
            valueABNNumber: '', //输入ABN号
            valueAdressDefault: '请输入地址信息（可选填项）',
            valueAdress: '', //输入的地址值

            isPersonal: true, //判断当前申请发票信息是个人还是公司
            isChoose: true, //是否被选中
            type: 1, //用于防止用频繁点击
            isShowDialog: false, //默认不显示dialog弹框

            invoice_money: "0.00", //开票金额
            order_id: '', //需要开发票的订单id
            isInvoiceOrderPage: ''
        };
    },
    ...
    methods: {
        ...
        //点击查看开发票的规则
        clickToRules: function clickToRules() {
            this.jumpTo2('electronicInvoice/instructionsInvoices', '', {});
        },
        ...
        //获取输入的地址值
        onInputAdress: function onInputAdress(e) {
            this.valueAdress = e.value;
        },

        //获取输入的公司名
        onInputCompany: function onInputCompany(e) {
            this.valueCompany = e.value;
        },

        //获取输入的AMN号
        onInputABNNumber: function onInputABNNumber(e) {
            this.valueABNNumber = e.value;
        },

        //点击选择是个人还是公司类型发票
        clickToChoose: function clickToChoose(type) {
            if (this.type != type) {
            ...
            if (type == 1) {
                ...
                this.valueABNNumber = '';
                this.valueAdress = '';
                this.valueCompanyDefault = '请输入公司全称';
                this.valueABNNumberDefault = '请输入ABN号';
                this.valueAdressDefault = '请输入地址信息（可选填项）';
            }
            if (type == 2) {
                this.valueName = '';
                this.valueAdress = '';
                this.valueNameDefault = '请输入姓名';
                this.valueAdressDefault = '请输入地址信息（可选填项）';
            }
        },

        //点击提交申请---要弹出框判断
        clickToApply: function clickToApply() {
            // console.log("姓名++++++++："+this.valueName);
            // console.log("地址值++++++++："+this.valueAdress);
            // console.log("公司名++++++++："+this.valueCompany);
            // console.log("AMN号++++++++："+this.valueABNNumber);
            if (this.isPersonal) {
                //个人提交申请走这里
                if (this.valueName.length == 0) {
                    promptModule.showPrompt({ msg: '请输入姓名', type: 'err' });
                    return;
                }
                ...
                if (this.valueCompany.length == 0) {
                    promptModule.showPrompt({ msg: '请输入公司名', type: 'err' });
                    return;
                }
                if (this.valueABNNumber.length != 11) {
                    promptModule.showPrompt({ msg: '请输入正确ABN号', type: 'err' });
                    return;
                }
                this.isShowDialog = true;
            }
        ...
        confirmApply: function confirmApply() {
            ...
            self.Post(self.api().generate_invoice_aus, {
                order_ids: self.order_id,
                type: 1,
                invoice_header: self.isPersonal ? self.valueName : self.valueCompany,
                invoice_money: self.invoice_money,
                header_type: self.isPersonal ? 0 : 1,
                company_mailbox: '',
                receive_name: self.valueName,
                receive_address: self.valueAdress,
                remarks: '',
                abn_number: self.isPersonal ? '' : self.valueABNNumber
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    if (ret.result == 'dismiss') {
                        if (res.data.code == "0") {
                            promptModule.showListener({ msg: res.data.msg, type: 'success' }, function () {
                                // self.jumpTo2('electronicInvoice/applyInvoiceResult','', {id:res.data.data,isInvoiceOrderPage:self.isInvoiceOrderPage});
                                self.interceptPush('electronicInvoice/applyInvoiceResult', { id: res.data.data, isInvoiceOrderPage: self.isInvoiceOrderPage }, 'intercept');
                            });
                        } else {
        ...
        //通过订单id order_id获取该订单的金额
        getInvoiceMoney: function getInvoiceMoney() {
            var self = this;
            self.Get(self.api().invoice_orders_list, {
                order_id: self.order_id
            }, function (res) {
                if (res.data.code == 0) {
                    self.invoice_money = res.data.data[0].aud_amount;
                } else {
                    promptModule.showPrompt({ msg: res.data.msg, type: "err" });
                }
            });
    ...
    created: function created() {
        ...
        self.isIpx(self);
        self.order_id = self.GetUrlPara(weex.config.bundleUrl, 'order_id');
        //isInvoiceOrderPage  是用于判断是否是由多笔订单页面过来,是:kill(3) 否:kill(2)
        self.isInvoiceOrderPage = self.GetUrlPara(weex.config.bundleUrl, 'isInvoiceOrderPage');
        //下面的开票金额需要一个判断,两种情况:
        // 1.多笔订单开发票金额是由weex.config.bundleUrl 带过去的;
        // 2.是由单笔订单过来的就通过getInvoiceMoney()用订单号order_id去请求金额;
        if (self.GetUrlPara(weex.config.bundleUrl, 'invoice_money') != 'undefined') {
            self.invoice_money = self.GetUrlPara(weex.config.bundleUrl, 'invoice_money');
        } else {
            self.getInvoiceMoney();
        }
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
    ...
    staticClass: ["head-type-personal"],
    on: {
      "click": function($event) {
        _vm.clickToChoose(1)
      }
    ...
    staticClass: ["image"],
    attrs: {
      "src": _vm.isPersonal ? 'http://img.abmau.com/abmau/56ec58709a05fdf2d0917339bf8221c5.png' : 'http://img.abmau.com/abmau/1a65729be20667583b160ebf98e0f396.png'
    }
  }), _c('text', {
    staticClass: ["head-type-text"]
  }, [_vm._v("个人")])]), _c('div', {
    staticClass: ["head-type-company"],
    on: {
      "click": function($event) {
        _vm.clickToChoose(2)
      }
    ...
    staticClass: ["image"],
    attrs: {
      "src": _vm.isPersonal ? 'http://img.abmau.com/abmau/1a65729be20667583b160ebf98e0f396.png' : 'http://img.abmau.com/abmau/56ec58709a05fdf2d0917339bf8221c5.png'
    }
  }), _c('text', {
    staticClass: ["head-type-text"]
  }, [_vm._v("公司")])])]), _c('div', {
  ...
  }, [_vm._v("$" + _vm._s(_vm._f("number")(_vm.invoice_money)))])]), (_vm.isPersonal) ? _c('div', {
    ...
    staticClass: ["name-text", "commen-text", "edit"],
    attrs: {
      "placeholder": _vm.valueNameDefault,
      "type": "text",
      "returnKeyType": "done",
      "value": _vm.valueName
    },
    on: {
      ...
      "onInput": function($event) {
        _vm.onInputName($event)
      }
    }
  })], 1) : _vm._e(), (!_vm.isPersonal) ? _c('div', {
    staticClass: ["name", "commen", "border-bottom"]
  }, [_c('text', {
    staticClass: ["red"]
  }, [_vm._v("*")]), _c('text', {
    ...
    staticClass: ["name-text", "commen-text", "edit"],
    attrs: {
      "placeholder": _vm.valueCompanyDefault,
      "type": "text",
      "returnKeyType": "done",
      "value": _vm.valueCompany
    },
    on: {
      "closeKeyboard": _vm.closeKeyboard,
      "onInput": function($event) {
        _vm.onInputCompany($event)
      }
    }
  })], 1) : _vm._e(), (!_vm.isPersonal) ? _c('div', {
    staticClass: ["name", "commen", "border-bottom"]
    ...
    staticClass: ["name-text", "commen-text", "edit"],
    attrs: {
      "placeholder": _vm.valueABNNumberDefault,
      "type": "number",
      "returnKeyType": "done",
      "value": _vm.valueABNNumber
    },
    on: {
      "closeKeyboard": _vm.closeKeyboard,
      "onInput": function($event) {
        _vm.onInputABNNumber($event)
      }
    }
  })], 1) : _vm._e(), _c('div', {
    ...
    staticClass: ["name-text", "commen-text", "edit"],
    attrs: {
      "placeholder": _vm.valueAdressDefault,
      "type": "text",
      "returnKeyType": "done",
      "value": _vm.valueAdress
    },
    on: {
      "closeKeyboard": _vm.closeKeyboard,
      "onInput": function($event) {
        _vm.onInputAdress($event)
      }
    }
  })], 1), _c('div', {
    class: ['apply-btn', (_vm.valueName.length > 0) || (_vm.valueCompany.length > 0 && _vm.valueABNNumber.length > 0) ? 'active' : ''],
    on: {
      "click": function($event) {
        _vm.clickToApply()
      }
    ...
    staticClass: ["apply-btn-text"]
  }, [_vm._v("提交")])]), _c('div', {
    staticClass: ["apply-rules"]
  }, [_c('text', {
    staticClass: ["apply-rules-text1"]
  }, [_vm._v("请确保您已经仔细阅读过")]), _c('text', {
    staticClass: ["apply-rules-text2"],
    on: {
      "click": function($event) {
        _vm.clickToRules()
      }
    }
  }, [_vm._v("发票须知")])])]), _c('dialog', {
    attrs: {
      "show": _vm.isShowDialog,
      "confirmBackgroundColor": "#fff",
      "confirmTextColor": "#333333",
      "content": "提交开票信息前请仔细核对信息是否正确，一旦错误将无法重新开票。",
      "title": "提示",
      "confirmText": "确定提交",
      "cancelText": "核对信息"
    },
