...
var _invoiceOrder = __webpack_require__(2);

var _invoiceOrder2 = _interopRequireDefault(_invoiceOrder);
...
_invoiceOrder2.default.el = '#root';
new Vue(_invoiceOrder2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\electronicInvoice\\invoiceOrder.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-bc6236ae"
...
module.exports = {
  ...
  "item-content-middle-right-title": {
    "width": "425",
    "color": "#333333",
    "fontSize": "28",
    "lineHeight": "42"
  },
  "item-content-middle-right-price": {
    "color": "#CF1126",
    "fontSize": "20",
    "lineHeight": "38"
  },
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            total_price: '0.00', //需要开发票的总金额
            order_ids: [] //开票订单的id (多个,以字符串的形式传递给接口)
        };
    },
    ...
    methods: {
        ...
        //点击全选
        clickToChooseAll: function clickToChooseAll() {
            this.total_price = 0;
            this.total_price = Number(this.total_price);
            for (var i = 0; i < this.data.length; i++) {
                this.data[i].isSelectCurrent = true;
                this.total_price += Number(this.data[i].aud_amount);
            }
        },

        //点击选择当前
        clickToChooseCurrent: function clickToChooseCurrent(index) {
            this.total_price = 0;
            this.data[index].isSelectCurrent = !this.data[index].isSelectCurrent;
            for (var i = 0; i < this.data.length; i++) {
                if (this.data[i].isSelectCurrent) {
                    this.total_price += Number(this.data[i].aud_amount);
                }
            }
        },
        ...
        onloading: function onloading() {
            ...
            this.getALLInvoiceOrder();
        },

        //获取所有需要开发票的订单
        getALLInvoiceOrder: function getALLInvoiceOrder() {
            var self = this;
            self.Get(self.api().invoice_orders_list, {
                offset: self.offset,
                limit: self.limit
            }, function (res) {
                if (res.data.code == 0) {
                    if (res.data._count > self.offset) {
                        self.offset += 10;
                        self.allLoadingFinish = false;
                    } else {
                        self.allLoadingFinish = true;
                    }
        ...
        //跳转到申请发票的页面 并且把订单的ids和金额带过去
        jumpToApplyInvoice: function jumpToApplyInvoice() {
            var self = this;
            for (var i = 0; i < self.data.length; i++) {
                if (self.data[i].isSelectCurrent) {
                    self.order_ids.push(self.data[i].id);
                }
            }
            var order_id = self.order_ids.join(',');
            if (order_id == '') {
                promptModule.showPrompt({ msg: '请选择订单', type: "err" });
                return;
            }
            self.jumpTo2('electronicInvoice/applyInvoice', '', { order_id: order_id, invoice_money: self.total_price, isInvoiceOrderPage: true });
            self.order_ids = [];
        }
    },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
    ...
    return _c('cell', {
      ...
      class: ['item-content', index == 0 ? 'first' : '']
    }, [_c('div', {
      staticClass: ["item-content-head"]
    }, [_c('text', {
      staticClass: ["item-content-head-text"]
    }, [_vm._v("订单号：" + _vm._s(item.sn))])]), _c('div', {
      staticClass: ["item-content-middle"],
      on: {
        "click": function($event) {
          _vm.clickToChooseCurrent(index)
        }
      }
    }, [_c('div', {
      staticClass: ["item-content-middle-left"]
    }, [(item.isSelectCurrent) ? _c('image', {
      staticClass: ["image"],
      attrs: {
        "src": "http://img.abmau.com/abmau/56ec58709a05fdf2d0917339bf8221c5.png"
      }
    }) : _vm._e(), (!item.isSelectCurrent) ? _c('image', {
      staticClass: ["image"],
      attrs: {
        "src": "http://img.abmau.com/abmau/1a65729be20667583b160ebf98e0f396.png"
      }
    }) : _vm._e()]), _c('div', {
      staticClass: ["item-content-middle-center"]
    }, [_c('image', {
      staticClass: ["goods-img"],
      attrs: {
        "src": item.order_goods[0].image_url
      }
    })]), _c('div', {
      staticClass: ["item-content-middle-right"]
    }, [_c('text', {
      staticClass: ["item-content-middle-right-title"]
    }, [_vm._v(_vm._s(item.order_goods[0].name))]), _c('text', {
      staticClass: ["item-content-middle-right-price"]
    }, [_vm._v("$" + _vm._s(item.aud_amount))])])])])])
  }), (_vm.data && _vm.data.length >= 10) ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
    ...
    staticClass: ["loading-over-txt"]
  }, [_vm._v("已经全部加载完毕")]) : _vm._e()])]) : _vm._e()], 2), _c('div', {
    staticClass: ["bottom"]
  }, [_c('div', {
    staticClass: ["bottom-left"]
  }, [_c('text', {
    staticClass: ["bottom-left-money"]
  }, [_vm._v("$" + _vm._s(_vm._f("number")(_vm.total_price)))]), _c('text', {
    staticClass: ["bottom-left-title"]
  }, [_vm._v("开票金额")])]), _c('text', {
    staticClass: ["bottom-right"],
    on: {
      "click": function($event) {
        _vm.jumpToApplyInvoice()
      }
    }
  }, [_vm._v("提交")])])])], 1)
},staticRenderFns: []}
