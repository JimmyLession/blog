...
var _payOrder = __webpack_require__(2);
...
_payOrder2.default.el = '#root';
new Vue(_payOrder2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\order\\payOrder.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-31de5c1e"
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            b_order_ids: '',
            payText: '',
            aoBiImage: 'http://img.abmau.com/abmau/e2c207fbe97c20518682f8f20581bde7.png',
            paySymbol: '¥',
            aud_total_price: '',
            ...
            new_pay_amount: '',
            type: 1,
            //1.==pay 代表普通商品确认下单 2.==tryout 代表试用商品确认下单 3.==continue普通商品继续付款 4.==tryout_continue 代表试用商品继续付款
            pay_source: '',
            payInfo: '',
            isOpenPay: false,
            pay_balance_info: '',
    ...
    methods: {
        finish: function finish() {
            var _this = this;
            if (_this.pay_source === "tryout_continue" || _this.pay_source == "continue") {
                _this.popBar();
            } else {
                promptModule.show({ title: '是否要放弃付款？',
                    message: '订单会保留30分钟，请尽快支付',
                    cancel: '确认离开',
                    confirm: '继续支付' }, function (ret) {
                    var result = ret.result;
                    if (result == 'cancel') {
                        if (_this.pay_source == 'tryout') refresh.postMessage(_this.eventBusTag().REFRESH_TRYOUT);
                        if (weex.config.env.platform.toLowerCase() === 'ios') {
                            if (_this.pay_source == 'pay' || _this.pay_source == 'pay_Gift') {
                                _this.jumpTo2('order/orderLv', '', { index: 0, pay_source: _this.pay_source });
                            }
                            if (_this.pay_source == 'tryout') _this.interceptPush('tryout/my_tryout', { entryType: 'pay_tryout' }, 'intercept');
                        } else if (weex.config.env.platform.toLowerCase() === 'android') {
                            if (_this.pay_source == 'pay' || _this.pay_source == 'pay_Gift') {
                                _this.jumpTo2('order/orderLv', '', { index: 0, pay_source: _this.pay_source });
                                setTimeout(function () {
                            ...
                            if (_this.pay_source == 'tryout') {
                                _this.jumpTo2('tryout/my_tryout', "", {});
                                _this.popBar();
                            }
                        }
                    }
        ...
        clickRecharge: function clickRecharge() {
            this.jumpTo2('web/webView', '', { title: "充值说明", url: decodeURIComponent('https://wap.abmau.com/rechargeExplain') });
        },
        ...
        pay: function pay(event) {
            var _this = this;
            if (!_this.selectAliPay && !_this.selectWx && !_this.selectBalance && !_this.selectAd) {
                promptModule.showPrompt({ msg: '请选择支付方式', type: 'prompt' });
                return;
            }
            if (event.timestamp - _this.lastTimestamp < 3000) return;
            _this.lastTimestamp = event.timestamp;
            var pay_type = 0;
            if (this.selectAliPay) {
                pay_type = 2;
            } else if (_this.selectWx) {
                pay_type = 1;
            } else if (_this.selectBalance) {
                pay_type = 0;
            } else if (_this.selectAd) {
                pay_type = 4;
            }
            loadingModule.show({ msg: '正在支付...' });
            _this.Post(_this.api().order_pay, {
                b_order_ids: _this.b_order_ids,
                pay_type: pay_type
            }, function (res) {
                if (res.data.code == 0) {
                    if (pay_type == 4) pay_type = 0; //如果是澳币支付方式当做账户余额支付逻辑处理
                    comModal.pay({ pay_type: pay_type, json: JSON.stringify(res.data.data), pay_source: _this.pay_source }, function (ret) {
                        var result = ret.result;
                        switch (result) {
                            case 'success':
                                _this.sendPayStatus(2);
                                break;
                            case 'failed':
                                _this.sendPayStatus(1);
                                break;
                            case 'unknow':
                                _this.sendPayStatus(-1);
                                break;
        ...
        sendPayStatus: function sendPayStatus(status) {
            var _this = this;
            _this.Post(_this.api().pay_status, {
                status: status,
                b_order_ids: _this.b_order_ids
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    var result = ret.result;
                    if (status == 2 && _this.pay_source == "pay_Gift") event.postMessage(_this.eventBusTag().REFRESH_COUPON);
                    if (result == 'dismiss') {
                        if (weex.config.env.platform.toLowerCase() === 'ios') {
                            if (!_this.isOpenPay) {
                                _this.isOpenPay = true;
                                if (_this.pay_source == 'tryout_continue' || _this.pay_source == 'continue') {
                                    refresh.postMessage(_this.eventBusTag().REFRESH_TRYOUT);
                                    _this.popBar();
                                } else {
                                    if (_this.pay_source == 'tryout') {
                                        refresh.postMessage(_this.eventBusTag().REFRESH_TRYOUT);
                                        _this.interceptPush('tryout/my_tryout', { entryType: 'pay_tryout' }, 'intercept');
                                    }
                                    if (_this.pay_source == 'pay' || _this.pay_source == 'pay_Gift') {
                                        _this.jumpTo2('order/orderLv', '', { index: 0, pay_source: _this.pay_source });
                                    }
                            ...
                            if (status == 2 || status == -1) {
                                if (_this.pay_source == 'tryout_continue' || _this.pay_source == 'continue') {
                                    ...
                                    if (_this.pay_source == 'pay' || _this.pay_source == 'pay_Gift') {
                                        ...
                                        setTimeout(function () {
                                            _this.popBar();
                                            if (_this.pay_source == "pay_Gift") event.postMessage(_this.eventBusTag().REFRESH_PRIMARYVIP);
                                        }, 2500);
                                    }
                                    if (_this.pay_source == 'tryout') {
                                        _this.jumpTo2('tryout/my_tryout', "", {});
        ...
        fetch: function fetch() {
            var _this = this;
            _this.Get(_this.api().order_confirm_pay, {
                b_order_ids: _this.b_order_ids
            }, function (res) {
                if (res.data.code == 0) {
                    ...
                    _this.goods_payment = res.data.data.goods_payment;
                    _this.old_goods_payment = res.data.data.old_goods_payment;
                    _this.old_pay_amount = res.data.data.old_pay_amount;
                    _this.new_pay_amount = res.data.data.new_pay_amount;
                    _this.aud_goods_payment = res.data.data.aud_goods_payment;
                    _this.aud_total_price = parseFloat(res.data.data.aud_total_price).toFixed(2);
                    _this.type = res.data.data.type;
                    _this.rate = res.data.data.rate;
                    _this.is_out_goods = res.data.data.is_out_goods;
                    _this.payText = "需支付";
                    _this.payInfo = _this.total_price;
                    _this.isZero = parseFloat(_this.total_price).toFixed(2) == 0.00;
                    if (_this.type == 1 || _this.type == 2) _this.isHintAlipay = false;else _this.isHintAlipay = true;
                    if (_this.isZero) _this.isHintAlipay = true;
                    if (_this.type == 1 || _this.type == 3) _this.isHintWx = false;else _this.isHintWx = true;
                    if (_this.isZero) _this.isHintWx = true;
                    if (_this.type == 4) {
                        _this.isHintWx = true;
                        _this.isHintAlipay = true;
                    }
                    if (parseFloat(res.data.data.aud_goods_payment) < parseFloat(_this.aud_total_price) && _this.type == 5) {
                        _this.isShowRecharge = true;
                    } else {
                        _this.isShowRecharge = false;
                    }
                    if (_this.type == 5) {
                        _this.is_balance = false;
                        _this.isHintWx = true;
                        _this.isHintAlipay = true;
                        _this.selectBalance = false;
                        _this.selectAd = true;
                        _this.paySymbol = '$';
                        if (parseFloat(res.data.data.aud_goods_payment) < _this.aud_total_price) {
                            _this.aud_goods_payment = '账户余额不足（请联系管家充值）';
                        } else {
                            _this.aud_goods_payment = '账户余额：$' + parseFloat(res.data.data.aud_goods_payment).toFixed(2);
                            _this.payText = "澳币支付";
                        }
                    }
                    if (parseFloat(_this.goods_payment) + parseFloat(_this.old_goods_payment) == 0.00 && !_this.isZero) {
                        _this.is_balance = false;
                        return;
                    }
                    if (!_this.isZero && _this.is_out_goods == 1) return;
                    if (_this.type == 5) {
                        _this.is_balance = false;_this.selectBalance = false;return;
                    }
                    _this.is_balance = true;
                    if (parseFloat(_this.goods_payment) + parseFloat(_this.old_goods_payment) < parseFloat(_this.total_price)) {
                        _this.pay_balance_info = '账户余额不足（请联系管家充值）';
                    } else {
                        if (parseFloat(_this.goods_payment) < parseFloat(_this.tariff_fee) + parseFloat(_this.shipping_fee)) {
                            _this.pay_balance_info = '账户余额不足（2017.09.26前的货款不能用于支付税费或运费，请联系管家充值）';
                        } else {
                            _this.selectBalance = true;
                            _this.pay_balance_info = '账户余额：¥' + (parseFloat(_this.goods_payment) + parseFloat(_this.old_goods_payment)).toFixed(2) + '元';
                            _this.payText = "余额支付";
                        }
                    }
                } else if (res.data.code == 8003) {
                    promptModule.showListener({ msg: res.data.msg, type: 'success' }, function (res) {
                        _this.popBar();
                    });
                } else {
                    promptModule.showPrompt({ msg: res.data.msg, type: "err" });
    ...
    created: function created() {
        ...
        this.isIpx(this);
        this.b_order_ids = this.GetUrlPara(weex.config.bundleUrl, 'b_order_ids');
        this.pay_source = this.GetUrlPara(weex.config.bundleUrl, 'type');
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["icon-text"]
  }, [_vm._v("返回")])])], 2), _vm._t("middle", [_c('text', {
    staticClass: ["nav_tittle"]
  }, [_vm._v("选择支付方式")])]), _c('div', {
    staticClass: ["right"]
  }, [_vm._t("right", [_c('text', {
    staticClass: ["icon-text"]
  }, [_vm._v("需支付")]), (!_vm.selectAd) ? _c('text', {
    staticClass: ["total_price"]
  }, [_vm._v(" " + _vm._s(_vm.paySymbol) + _vm._s(_vm.total_price))]) : _vm._e(), (_vm.selectAd) ? _c('text', {
    staticClass: ["total_price"]
  }, [_vm._v(" " + _vm._s(_vm.paySymbol) + _vm._s(_vm.aud_total_price))]) : _vm._e()])], 2)], 2), _c('line'), _c('div', {
    staticClass: ["parent"],
    style: {
      top: _vm.height + 'px',
      bottom: (_vm.bottom + 100) + 'px'
    }
  }, [_c('div', {
    staticClass: ["line"]
  }), (!_vm.isHintWx) ? _c('div', {
  ...
  }, [(!_vm.selectWx) ? _c('image', {
    staticClass: ["status"],
    attrs: {
      "src": "assets://icon_pay_unselect"
    }
  }) : _vm._e(), (_vm.selectWx) ? _c('image', {
    staticClass: ["status"],
    attrs: {
      "src": "assets://icon_pay_selected"
    }
  }) : _vm._e(), _c('image', {
    staticClass: ["pay_image"],
    attrs: {
      "src": "assets://weixin"
    }
  }), _c('text', {
  ...
  }, [_vm._v("微信支付")]), (!_vm.isHintAlipay && _vm.is_out_goods == 1) ? _c('div', {
    staticClass: ["pay_item_alipay"],
    on: {
      "click": function($event) {
        _vm.showIdentity = !_vm.showIdentity
      }
    }
  }, [(!_vm.showIdentity) ? _c('image', {
    staticClass: ["morex"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
  }) : _vm._e(), (_vm.showIdentity) ? _c('image', {
    staticClass: ["mores"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
  }) : _vm._e()]) : _vm._e()]) : _vm._e(), (!_vm.isHintWx) ? _c('line') : _vm._e(), (!_vm.isHintAlipay && _vm.showIdentity) ? _c('div', {
    staticClass: ["pay_item"],
    on: {
      "click": function($event) {
        _vm.selectPay(0)
  ...
  }, [(!_vm.selectAliPay) ? _c('image', {
    staticClass: ["status"],
    attrs: {
      "src": "assets://icon_pay_unselect"
    }
  }) : _vm._e(), (_vm.selectAliPay) ? _c('image', {
    staticClass: ["status"],
    attrs: {
      "src": "assets://icon_pay_selected"
    }
    ...
    staticClass: ["pay_text"]
  }, [_vm._v("支付宝支付")])]) : _vm._e(), (!_vm.isHintAlipay && _vm.showIdentity) ? _c('line') : _vm._e(), (!_vm.isHintAlipay && (_vm.isHintWx || _vm.is_out_goods == 0)) ? _c('div', {
    staticClass: ["pay_item"],
    on: {
      "click": function($event) {
        _vm.selectPay(0)
    ...
    staticClass: ["pay_text"]
  }, [_vm._v("支付宝支付")])]) : _vm._e(), (!_vm.isHintAlipay && (_vm.isHintWx || _vm.is_out_goods == 0)) ? _c('line') : _vm._e(), (_vm.is_balance) ? _c('div', {
    staticClass: ["pay_item_balance"],
    on: {
      "click": function($event) {
        _vm.selectPay(2)
      }
    }
  }, [(!_vm.selectBalance) ? _c('image', {
    staticClass: ["status"],
    attrs: {
      "src": "assets://icon_pay_unselect"
    }
  }) : _vm._e(), (_vm.selectBalance) ? _c('image', {
    staticClass: ["status"],
    attrs: {
      "src": "assets://icon_pay_selected"
    }
  }) : _vm._e(), _c('div', [_vm._m(0), _c('div', {
    staticClass: ["content"]
  }, [_c('text', {
    staticClass: ["pay_balance_info"]
  }, [_vm._v(_vm._s(_vm.pay_balance_info))])])])]) : _vm._e(), (_vm.is_balance) ? _c('line') : _vm._e(), (_vm.type == 5) ? _c('div', {
    staticClass: ["pay_item_balance"],
    on: {
      "click": function($event) {
        _vm.selectPay(3)
  ...
  }, [(!_vm.selectAd) ? _c('image', {
    staticClass: ["status"],
    attrs: {
      "src": "assets://icon_pay_unselect"
    }
  }) : _vm._e(), (_vm.selectAd) ? _c('image', {
    staticClass: ["status"],
    attrs: {
      "src": "assets://icon_pay_selected"
    }
  }) : _vm._e(), _c('div', [_c('div', {
    staticClass: ["pay_balance_ad"]
  }, [_c('div', {
    staticClass: ["pay_balance_recharge"]
  }, [_c('image', {
    staticClass: ["pay_image"],
    attrs: {
      "src": _vm.aoBiImage
    ...
    staticClass: ["pay_text"]
  }, [_vm._v("澳币支付")])]), (_vm.isShowRecharge) ? _c('div', {
    staticClass: ["pay_balance_recharge"],
    on: {
      "click": function($event) {
        _vm.clickRecharge()
  ...
  }, [_vm._v("澳币账户如何")]), _c('text', {
    staticClass: ["agm"]
  }, [_vm._v("充值?")])]) : _vm._e()]), _c('div', {
    staticClass: ["content"]
  }, [_c('text', {
    staticClass: ["pay_balance_info"]
  }, [_vm._v(_vm._s(_vm.aud_goods_payment))])])])]) : _vm._e(), (_vm.type == 5) ? _c('line') : _vm._e(), (_vm.type == 5) ? _c('text', {
    staticClass: ["exchange"]
  }, [_vm._v("*当前汇率1澳元≈" + _vm._s(_vm.rate) + "人民币元")]) : _vm._e(), (_vm.is_out_goods == 1) ? _c('div', {
    staticStyle: {
      width: "750px"
    }
  }, [_c('text', {
    staticClass: ["hint"]
  }, [_vm._v("跨境订单（保税仓发货、澳洲直邮国内）10月22日起不支持账户余额支付，只能使用支付宝、微信进行支付。")]), _c('text', {
    staticClass: ["hint"]
  }, [_vm._v("对于人民币账户有余额的客户，使用支付宝、微信、快捷银行卡支付之后，系统将会在订单结算之后（推送到仓库24小时之后）将人民币账户余额自动转到提现账户中，直接提现即可。")])]) : _vm._e()], 1), (!_vm.selectAd) ? _c('text', {
    staticClass: ["pay"],
    on: {
      "click": _vm.pay
    }
  }, [_vm._v(_vm._s(_vm.payText) + " " + _vm._s(_vm.paySymbol) + _vm._s(_vm.payInfo))]) : _vm._e(), (_vm.selectAd) ? _c('text', {
    staticClass: ["pay"],
    on: {
      "click": _vm.pay
    }
  }, [_vm._v(_vm._s(_vm.payText) + " " + _vm._s(_vm.paySymbol) + _vm._s(_vm.aud_total_price))]) : _vm._e()], 1)
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["pay_balance"]
  }, [_c('image', {
    staticClass: ["pay_image"],
    attrs: {
      "src": "assets://icon_balance"
    }
  }), _c('text', {
    staticClass: ["pay_text"]
  }, [_vm._v("余额支付")])])
