...
var _orderLv = __webpack_require__(4);

var _orderLv2 = _interopRequireDefault(_orderLv);
...
_orderLv2.default.el = '#root';
new Vue(_orderLv2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\order\\orderLv.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-ccc27a46"
...
var _orderManager = __webpack_require__(20);
...
var titleHeight = 0;
var height = 0;
var orderEvent = new BroadcastChannel('order_manager');
var refreshEvent = new BroadcastChannel('refresh_try');
exports.default = {
    components: { WxcTabPage: _wxcTabPage2.default, title: _titleBar2.default, orderManager: _orderManager2.default },
    data: function data() {
        return {
            height: height,
            tabList: [],
    ...
    methods: {
        ...
        wxcTabPageCurrentTabSelected: function wxcTabPageCurrentTabSelected(e) {
            ...
            switch (self.index) {
                case 0:
                    break;
                case 1:
                    self.showWaitPay = true;
                    break;
                case 2:
                    self.showWaitSend = true;
                    break;
                case 3:
                    self.showDelivers = true;
                    break;
                case 4:
        ...
        resetPositionData: function resetPositionData() {
            this.$refs['all-order'].resetSelectPosition();
            if (this.showWaitPay) {
                this.$refs['pay-order'].resetSelectPosition();
            }
            if (this.showWaitSend) {
                this.$refs['send-order'].resetSelectPosition();
            }
            if (this.showDelivers) {
                this.$refs['take-order'].resetSelectPosition();
            }
            if (this.showFinish) {
                this.$refs['finish-order'].resetSelectPosition();
            }
        },
        searchOrder: function searchOrder(event) {
            var value = event.value;
            if (value) {
                this.jumpTo2('order/orderSearch', '', { keyword: value });
            }
        },
        changeSearch: function changeSearch(isShow) {
            this.showSearch = isShow;
        },
        cancelOrder: function cancelOrder(show) {
            switch (show) {
                case 0:
                    if (this.showWaitSend) {
                        this.$refs['send-order'].refreshData();
                    }
                    if (this.showWaitPay) {
                        this.$refs['pay-order'].refreshData();
                    }
                    break;
                case 1:
                    this.$refs['all-order'].refreshData();
                    if (this.showWaitSend) {
                        this.$refs['send-order'].refreshData();
                    }
                    break;
        ...
        finishOrder: function finishOrder(show) {
            switch (show) {
                case 0:
                    if (this.showFinish) {
                        this.$refs['finish-order'].refreshData();
                    }
                    break;
                case 3:
                    this.$refs['all-order'].refreshData();
    ...
    created: function created() {
        this.getWeexHeight(this);
        this.getTitleHeight(this);
        this.getStatusBarHeight(this);
        this.showSelect = this.GetUrlPara(weex.config.bundleUrl, 'index');
        this.showSelect = this.showSelect && this.showSelect != 'undefind' ? this.showSelect : 0;
        this.pay_source = this.GetUrlPara(weex.config.bundleUrl, 'pay_source') != 'undefined' ? this.GetUrlPara(weex.config.bundleUrl, 'pay_source') : '';
        this.index = 0;
        var self = this;
        refreshEvent.onmessage = function (event) {
            if (!event.data) {
                return;
            }
            if (event.data == self.eventBusTag().REFRESH_TRYOUT) {
                self.$refs['all-order'].refreshOrderStatus(2);
                if (self.showWaitPay) {
                    self.$refs['pay-order'].refreshOrderStatus(2);
                }
                if (self.showWaitSend) {
                    self.$refs['send-order'].refreshOrderStatus(2);
                }
            }
        };
        orderEvent.onmessage = function (event) {
            ...
            switch (event.data) {
                case self.eventBusTag().ORDER_CANCEL:
                    self.$refs['all-order'].refreshOrderStatus(-1);
                    if (self.showWaitPay) {
                        self.$refs['pay-order'].refreshOrderStatus(-1);
                    }
                    if (self.showWaitSend) {
                        self.$refs['send-order'].refreshOrderStatus(-1);
                    }
                    break;
                case self.eventBusTag().ORDER_FINISH:
                    self.$refs['all-order'].refreshOrderStatus(1);
                    if (self.showDelivers) {
                        self.$refs['take-order'].refreshOrderStatus(1);
                    }
                    if (self.showFinish) {
                        self.$refs['finish-order'].refreshOrderStatus(1);
                    }
                    break;
            }
        };
    ...
    updated: function updated() {
        if (this.showSelect == 0 || !this.isFirst) {
            return;
        }
        this.isFirst = false;
        this.index = this.showSelect;
        var show = parseInt(this.showSelect);
        this.$refs['wxc-tab-page'].setPage(show, null, false);
    },
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\order\\orderManager.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-76b0e8ea"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
    components: { refresher: _refresh2.default },
    props: {
        status: {
            default: -1
        },
    ...
    methods: {
        ...
        getData: function getData() {
            ...
            self.Get(self.api().list_order, param, function (res) {
                if (res.data.code != '0') {
                    //请求数据失败
                    ...
                    self.isAllLoadFinish = self.data.length < self.size;
                }
            });
        },
        clickItem: function clickItem(item, index) {
            this.changePosition = index;
            this.jumpTo2('order/orderDetail', '订单详情', { sn_id: item.id });
        },
        bt_copy_sn: function bt_copy_sn(sn) {
            comModal.copy({ value: sn }, function () {});
        },
        bt_order_cancel: function bt_order_cancel(item, index) {
            var self = this;
        ...
        cancelOrder: function cancelOrder(item) {
            var self = this;
            loadingModule.show({ msg: '正在取消...' });
            self.Post(self.api().cancel_order, { order_id: item.id }, function (res) {
                if (res.data.code == '0') {
                    self.refreshOrderStatus(-1);
                    self.$emit('cancel', {});
                }
                loadingModule.cancel(function (ret) {
                    ...
                    if (result == 'dismiss') {
                        if (res.data.code == '0') {
                            promptModule.showPrompt({ msg: res.data.msg, type: 'success' });
                        } else {
                            promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                        }
                    }
        ...
        bt_order_continue: function bt_order_continue(item, index) {
            this.changePosition = index;
            this.interceptPush('order/payOrder', { b_order_ids: item.b_order_ids, type: 'continue' }, 'intercept');
        },
        bt_order_confirm: function bt_order_confirm(item, index) {
            var self = this;
            self.changePosition = -1;
            promptModule.show({ title: '温馨提示',
                message: '是否确认收货',
                ...
                if (result == 'success') {
                    self.changePosition = index;
                    self.confirmOrder(item);
                }
            });
        },

        //查看物流
        bt_show_pd: function bt_show_pd(id) {
            this.jumpTo2('order/orderLogistics', '', { sn_id: id });
        },
        confirmOrder: function confirmOrder(item) {
            var self = this;
            loadingModule.show({ msg: '正在确认...' });
            self.Get(self.api().confirm_order, { order_id: item.id }, function (res) {
                if (res.data.code == '0') {
                    self.refreshOrderStatus(1);
                    self.$emit('finish', {});
                }
        ...
        getOrderStatusInfo: function getOrderStatusInfo(item) {
            var self = this;
            self.Get(self.api().order_status_info, {
                id: item.id
            }, function (res) {
                if (res.data.code == '0') {
                    if (res.data.data.status == item.status) {
                        self.changePosition = -1;
                        return;
                    }
                    if (self.status == -1) {
                        item.status = res.data.data.status;
                        item.status_name = res.data.data.status_name;
                        if (item.source == 0 && item.status == 2 && item.is_push_warehouse == 0) {
                            item.cancel_order = 1;
                        }
                    } else {
                        if (self.data.length == 1) {
                            ...
                            self.data.splice(self.changePosition, 1);
                        }
                    }
                    self.changePosition = -1;
                } else {
        ...
        resetSelectPosition: function resetSelectPosition() {
            this.changePosition = -1;
        },
        refreshOrderStatus: function refreshOrderStatus(orderStatus) {
            var self = this;
            if (self.changePosition >= 0 && self.changePosition < self.data.length) {
                if (self.status == -1) {
                    if (orderStatus == 2) {
                        self.getOrderStatusInfo(self.data[self.changePosition]);
                    } else if (orderStatus == -1) {
                        self.data[self.changePosition].status = orderStatus;
                        self.data[self.changePosition].status_name = "已取消";
                        self.changePosition = -1;
                    } else if (orderStatus == 1) {
                        self.data[self.changePosition].status = orderStatus;
                        self.data[self.changePosition].status_name = "已完成";
                        self.changePosition = -1;
                    }
                } else {
                    if (orderStatus == 2) {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["line"]
  }), (_vm.data.length >= 0) ? _c('list', {
    attrs: {
      "loadmoreoffset": "10"
    },
    on: {
      "loadmore": _vm.loadingMore
    }
  }, [_c('refresher', {
    on: {
    ...
    return _c('cell', {
      ...
      staticClass: ["dv_order_status"]
    }, [_c('text', {
      staticClass: ["order_sn"]
    }, [_vm._v("订单号：" + _vm._s(item.sn))]), _c('text', {
      staticClass: ["order_copy"],
      on: {
        "click": function($event) {
          _vm.bt_copy_sn(item.sn)
        }
      }
    }, [_vm._v("复制")])]), _c('text', {
      staticClass: ["order_status"]
    }, [_vm._v(_vm._s(item.status_name))])]), _c('div', {
      staticClass: ["order_depot"]
    }, [_c('text', {
      staticClass: ["depot_name"]
    }, [_vm._v(_vm._s(item.depot_name))]), _c('text', {
      staticClass: ["order_slipping"]
    }, [_vm._v(_vm._s(item.shipping_detail.shipping_name) + "  " + _vm._s(item.shipping_detail.shipping_mobile) + "  " + _vm._s(item.created_at))])]), _c('div', {
      staticClass: ["line"]
    }), _vm._l((item.goods_list), function(goods, position) {
      return _c('div', {
        key: position
      }, [_c('div', {
      ...
      }, [_vm._v(_vm._s(goods.goods_name))]), _c('div', {
        staticClass: ["price_info"]
      }, [_c('text', {
        staticClass: ["goods_price"]
      }, [_vm._v("优惠价  ¥" + _vm._s(goods.in_price))]), _c('div', {
      ...
      }, [_vm._v("售价  ¥" + _vm._s(goods.price))]), _c('text', {
        staticClass: ["goods_market_price"]
      }, [_vm._v("(¥" + _vm._s(goods.market_price) + ")")])]), _c('text', {
        staticClass: ["goods_num"]
      }, [_vm._v("x  " + _vm._s(goods.goods_count))])])])])]), _c('div', {
        staticClass: ["line"]
      })])])
    }), _c('div', {
      staticClass: ["order_count"]
    }, [_c('div', {
      staticClass: ["order_count_info"]
    }, [(item.depot_id != 20) ? _c('text', {
      staticClass: ["goods_count_price"]
    }, [_vm._v("(含运费 ¥" + _vm._s(item.shipping_fee) + "  税费¥" + _vm._s(item.tariff_fee) + ")")]) : _c('text', {
      staticClass: ["goods_count_price"]
    }, [_vm._v("(含运费 ¥" + _vm._s(item.shipping_fee) + "  进口综合费¥" + _vm._s(item.tariff_fee) + ")")]), _c('text', {
      staticClass: ["goods_price"]
    }, [_vm._v("合计 ¥" + _vm._s(item.amount))])]), _c('text', {
      staticClass: ["order_num"],
      style: {
        marginTop: '12px'
      }
    }, [_vm._v("共" + _vm._s(item.goods_count) + "件商品")])]), (item.status == 0 || (item.status == 2 && item.cancel_order == 1) || item.status == 3) ? _c('div', [_c('div', {
      staticClass: ["line"]
    }), _c('div', {
      staticClass: ["order_bt"]
    }, [(item.status == 0 || (item.status == 2 && item.cancel_order == 1)) ? _c('text', {
      staticClass: ["bt_cancel_order"],
      on: {
        "click": function($event) {
          _vm.bt_order_cancel(item, index)
        }
      }
    }, [_vm._v("取消订单")]) : _vm._e(), (item.status == 0) ? _c('text', {
      staticClass: ["bt_continue_order"],
      on: {
        "click": function($event) {
          _vm.bt_order_continue(item, index)
        }
      }
    }, [_vm._v("继续付款")]) : _vm._e(), (item.status == 3) ? _c('text', {
      staticClass: ["bt_cancel_order"],
      on: {
        "click": function($event) {
          _vm.bt_show_pd(item.id)
        }
      }
    }, [_vm._v("查看物流")]) : _vm._e(), (item.status == 3) ? _c('text', {
      staticClass: ["bt_confirm_order"],
      on: {
        "click": function($event) {
          _vm.bt_order_confirm(item, index)
        }
      }
    }, [_vm._v("确认收货")]) : _vm._e()])]) : _vm._e(), _c('div', {
      staticClass: ["space"]
    })], 2), (item.is_billing == 2) ? _c('image', {
      staticClass: ["invoice-icon"],
      attrs: {
        "src": "http://img.abmau.com/abmau/c85a184a2679f1419f8e2e599a6c658c.png"
      }
    }) : _vm._e()])
  }), (_vm.data.length > 3 && _vm.isLoading) ? _c('cell', {
    staticClass: ["more-list"],
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [(_vm.isAllLoadFinish) ? _c('text', {
    staticClass: ["loading-over-txt"]
  }, [_vm._v("已经全部加载完毕")]) : _c('text', {
    staticClass: ["loading-over-txt"]
  }, [_vm._v("加载中...")])]) : _vm._e(), (_vm.data.length <= 0 && _vm.isAllLoadFinish) ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_vm._m(0)]) : _vm._e()], 2) : _vm._e()])
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["empty_content"]
  }, [_c('image', {
    staticClass: ["empty_icon"],
    attrs: {
      "src": "http://img.abmau.com/abmau/6f6b5a8bd76238cc1862404d07d4df48.png"
    }
  }), _c('text', {
    staticClass: ["empty_hint"]
  }, [_vm._v("您暂时没有订单")])])
}]}
module.exports.render._withStripped = true
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    attrs: {
      "title": "我的订单",
      "line": false,
      "rightButton": "assets://icon_search",
      "isFinish": false
    },
    on: {
      "wxcMinibarRightButtonClicked": function($event) {
        _vm.changeSearch(true)
      },
    ...
    staticClass: ["search-edit"],
    style: {
      height: (_vm.titleHeight - 20) + 'px'
    },
    attrs: {
      "placeholder": "订单搜索",
  ...
  }, 'input', _vm.inputValue, false))]), _c('text', {
  ...
  }, [_vm._v("取消")])]) : _vm._e(), _c('wxc-tab-page', {
    ref: "wxc-tab-page",
    attrs: {
    ...
    staticClass: ["item-container"]
  }, [(_vm.showWaitPay) ? _c('order-manager', {
    ref: "pay-order",
    style: {
      height: (_vm.height - _vm.tabStyles.height) + 'px'
    },
    ...
    staticClass: ["item-container"]
  }, [(_vm.showWaitSend) ? _c('order-manager', {
    ref: "send-order",
    style: {
      height: (_vm.height - _vm.tabStyles.height) + 'px'
    },
    ...
    staticClass: ["item-container"]
  }, [(_vm.showDelivers) ? _c('order-manager', {
    ref: "take-order",
    style: {
      height: (_vm.height - _vm.tabStyles.height) + 'px'
    },
    ...
    staticClass: ["item-container"]
  }, [(_vm.showFinish) ? _c('order-manager', {
    ref: "finish-order",
    style: {
      height: (_vm.height - _vm.tabStyles.height) + 'px'
    },
