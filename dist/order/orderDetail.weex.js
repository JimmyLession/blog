...
var _orderDetail = __webpack_require__(2);
...
_orderDetail2.default.el = '#root';
new Vue(_orderDetail2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\order\\orderDetail.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-532086b8"
...
var _itemDecoration = __webpack_require__(9);
...
var result = new BroadcastChannel('applyInvoiceResult'); //发票申请成功的广播
var height = 0;
var bottom = 0;
exports.default = {
    components: {
        itemDecoration: _itemDecoration2.default,
        line: _line2.default,
        title: _titleBar2.default,
    ...
    data: function data() {
        return {
            ...
            isChangeData: false,
            changeMsg: '',
            isShowDialog: false, //默认不显示dialog弹框
            order_id: '' //订单id
        };
    },
    ...
    methods: {
        ...
        bt_order_cancel: function bt_order_cancel(id) {
            var _this = this;
        ...
        cancelOrder: function cancelOrder(id) {
            ...
            _this.Post(_this.api().cancel_order, { order_id: id }, function (res) {
                if (res.data.code == '0') {
                    _this.isChangeData = true;
                    _this.changeMsg = res.data.msg;
                    orderEvent.postMessage(_this.eventBusTag().ORDER_CANCEL);
                    _this.getOrderStatusInfo();
                } else {
        ...
        //点击进入发票页面  参数=>type:0.申请发票  1.开票中  2.已开发票;  invoice_id:发票id;  order_id:订单id
        clickToInvoice: function clickToInvoice(type, invoice_id, order_id) {
            if (type == 0) {
                this.order_id = order_id;
                this.isShowDialog = true;
            } else if (type == 1) {
                this.jumpTo2('electronicInvoice/applyInvoiceResult', '', { id: invoice_id });
            } else if (type == 2) {
                this.jumpTo2('electronicInvoice/invoiceDetails', '', { id: invoice_id });
            }
        },
        ...
        cancel: function cancel() {
            //去开单笔订单 发票
            this.isShowDialog = false;
            this.jumpTo2('electronicInvoice/applyInvoice', '', { order_id: this.order_id });
        },
        confirm: function confirm() {
            //去开多笔订单 发票
            this.isShowDialog = false;
            this.jumpTo2('electronicInvoice/invoiceOrder', '', {});
        },
        bt_order_continue: function bt_order_continue(item) {
            this.interceptPush('order/payOrder', { b_order_ids: item.b_order_ids, type: 'continue' }, 'intercept');
        },
        bt_order_confirm: function bt_order_confirm(id) {
            var _this = this;
            promptModule.show({ title: '温馨提示',
                message: '是否确认收货',
                cancel: '取消',
        ...
        confirmOrder: function confirmOrder(id) {
            ...
            _this.Get(_this.api().confirm_order, { order_id: id }, function (res) {
                if (res.data.code == '0') {
                    _this.isChangeData = true;
                    _this.changeMsg = res.data.msg;
                    orderEvent.postMessage(_this.eventBusTag().ORDER_FINISH);
                    _this.getOrderStatusInfo();
                } else {
                    loadingModule.cancel(function (ret) {
                        var result = ret.result;
        ...
        getOrderStatusInfo: function getOrderStatusInfo() {
            var self = this;
            self.Get(self.api().order_status_info, {
                id: self.data.id
            }, function (res) {
                if (res.data.code == '0') {
                    self.data.status = res.data.data.status;
                    self.data.status_name = res.data.data.status_name;
                    if (self.data.source == 0 && self.data.status == 2 && self.data.is_push_warehouse == 0) {
                        self.data.cancel_order = 1;
                    }
                    if (self.isChangeData) {
                        self.isChangeData = false;
                        loadingModule.cancel(function (ret) {
                            var result = ret.result;
                            if (result == 'dismiss' && self.changeMsg) {
                                promptModule.showPrompt({ msg: self.changeMsg, type: 'success' });
                            }
                        });
                    }
                } else {
        ...
        fetch: function fetch() {
            var _this = this;
            _this.Get(_this.api().order_detail, {
                version: '3.0',
                code: 'ABM',
                order_id: _this.sn_id,
                device: 'android',
                from: 'ABM'
            }, function (res) {
                if (res.data.code != 0) {
                    //请求数据失败
                    modal.toast({ message: res.data.msg, duration: 3 });
                } else {
                    _this.data = res.data.data;
                    _this.shipping_detail = _this.data.shipping_detail;
                }
            });
        }
    },
    created: function created() {
        ...
        this.isIpx(this);
        _this.sn_id = _this.GetUrlPara(weex.config.bundleUrl, 'sn_id');
        _this.fetch();
        refreshEvent.onmessage = function (event) {
            if (!event.data) {
                return;
            }
            if (event.data == _this.eventBusTag().REFRESH_TRYOUT) {
                _this.getOrderStatusInfo();
            }
        };
        result.onmessage = function (event) {
            if (event.data.applyResult) {
                console.log("接收applyInvoiceResult返回的结果");
                _this.fetch();
            }
        };
    },
...
var _wxcOverlay = __webpack_require__(23);
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v("订单号：" + _vm._s(_vm.data.sn))]), _c('text', {
    staticClass: ["order_copy"],
    on: {
      "click": function($event) {
        _vm.bt_copy_sn(_vm.data.sn)
      }
    }
  }, [_vm._v("复制")])]), _c('text', {
    staticClass: ["order_status"]
  }, [_vm._v(_vm._s(_vm.data.status_name))])]), _c('div', {
    staticClass: ["order_depot"]
  }, [_c('text', {
    staticClass: ["depot_name"]
  }, [_vm._v(_vm._s(_vm.data.depot_name))]), _c('text', {
    staticClass: ["order_slipping"]
  }, [_vm._v(_vm._s(_vm.shipping_detail.shipping_name) + "  " + _vm._s(_vm.shipping_detail.shipping_mobile) + "  " + _vm._s(_vm.data.created_at))])]), _c('line'), _vm._l((_vm.data.goods_list), function(goods, position) {
    return _c('div', {
      key: position,
      staticClass: ["list"]
    }, [_c('div', {
    ...
    }, [_vm._v("x  " + _vm._s(goods.goods_count))])])])])]), (position != (_vm.data.goods_list.length - 1)) ? _c('line') : _vm._e()], 1)])
  }), _c('item-decoration'), _c('div', {
    staticClass: ["address"]
  }, [_c('text', {
  ...
  }, [_vm._v(_vm._s(_vm.shipping_detail.shipping_name))]), _c('text', {
    staticClass: ["address_mobile"]
  }, [_vm._v(_vm._s(_vm.shipping_detail.shipping_mobile))])]), _c('text', {
    staticClass: ["address_detail"]
  }, [_vm._v(_vm._s(_vm.shipping_detail.shipping_province) + _vm._s(_vm.shipping_detail.shipping_city) + _vm._s(_vm.shipping_detail.shipping_district) + _vm._s(_vm.shipping_detail.shipping_address))])])]), _c('item-decoration'), _c('div', {
    staticClass: ["order_price"]
  }, [_c('text', {
    staticClass: ["order_in_price_title"]
  }, [_vm._v("应付金额")]), _c('text', {
    staticClass: ["order_in_price"]
  }, [_vm._v("¥" + _vm._s(_vm.data.amount))])]), _c('line'), _c('div', {
    staticClass: ["order_market_price"]
  }, [_c('text', {
    staticClass: ["order_market_price_title"]
  }, [_vm._v("商品货款")]), _c('text', {
    staticClass: ["market_price"]
  }, [_vm._v("¥" + _vm._s(_vm.data.goods_amount))])]), _c('line'), _c('div', {
    staticClass: ["order_fee_price"]
  }, [_c('text', {
    staticClass: ["order_fee_price_title"]
  }, [_vm._v("运费")]), _c('text', {
    staticClass: ["fee_price"]
  }, [_vm._v("¥" + _vm._s(_vm.data.shipping_fee))])]), _c('line'), _c('div', {
    staticClass: ["order_tax_price"]
  }, [(_vm.data.depot_id != 20) ? _c('text', {
    staticClass: ["order_tax_price_title"]
  }, [_vm._v("税费")]) : _c('text', {
    staticClass: ["order_tax_price_title"]
  }, [_vm._v("进口综合费")]), _c('text', {
    staticClass: ["tax_price"]
  }, [_vm._v("¥" + _vm._s(_vm.data.tariff_fee))])]), (_vm.data.billing_status == 1) ? _c('div', {
    staticClass: ["order_fee_price", "apply_invoice"],
    on: {
      "click": function($event) {
        _vm.clickToInvoice(_vm.data.is_billing, _vm.data.invoice_id, _vm.data.id)
      }
    }
  }, [(_vm.data.is_billing == 0) ? _c('text', {
    staticClass: ["order_fee_price_title"]
  }, [_vm._v("申请发票")]) : _vm._e(), (_vm.data.is_billing == 1) ? _c('text', {
    staticClass: ["order_fee_price_title"]
  }, [_vm._v("开票中")]) : _vm._e(), (_vm.data.is_billing == 2) ? _c('text', {
    staticClass: ["order_fee_price_title"]
  }, [_vm._v("已开发票")]) : _vm._e(), _c('image', {
    staticClass: ["more"],
    attrs: {
  ...
  })], 2), (_vm.data.status == 0 || (_vm.data.status == 2 && _vm.data.cancel_order == 1) || _vm.data.status == 3) ? _c('div', {
    staticClass: ["order_bt"],
    style: {
      bottom: _vm.bottom + 'px'
    }
  }, [(_vm.data.status == 0 || (_vm.data.status == 2 && _vm.data.cancel_order == 1)) ? _c('text', {
    staticClass: ["bt_cancel_order"],
    on: {
      "click": function($event) {
        _vm.bt_order_cancel(_vm.data.id)
      }
    }
  }, [_vm._v("取消订单")]) : _vm._e(), (_vm.data.status == 0) ? _c('text', {
    staticClass: ["bt_continue_order"],
    on: {
      "click": function($event) {
        _vm.bt_order_continue(_vm.data)
      }
    }
  }, [_vm._v("继续付款")]) : _vm._e(), (_vm.data.status == 3) ? _c('text', {
    staticClass: ["bt_confirm_order"],
    on: {
      "click": function($event) {
        _vm.bt_order_confirm(_vm.data.id)
      }
    }
  }, [_vm._v("确认收货")]) : _vm._e()]) : _vm._e(), _c('dialog', {
    attrs: {
      "show": _vm.isShowDialog,
      "confirmBackgroundColor": "#fff",
      "confirmTextColor": "#333333",
      "content": "您最近有多笔订单可以集中进行开票，是否选择多笔订单集中开票。",
      "title": "提示",
      "confirmText": "是",
      "cancelText": "否",
      "isCancelEvent": true
