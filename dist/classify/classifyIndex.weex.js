...
var _classifyIndex = __webpack_require__(2);
...
_classifyIndex2.default.el = '#root';
new Vue(_classifyIndex2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\classify\\classifyIndex.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-b2445e1a"
...
exports.default = {
    components: { line: _line2.default },
    data: function data() {
        return {
            height: height,
            bottom: bottom,
            ...
            noreadcount: 0,
            keywordMarquee: [],
            cateId: 0, //这个tab切换时候用到的id
            category_type: 'brand',
            tabs: [], //chooseType 左边tab的name数据
            allProduct: '',
            data: { //热卖单品和品牌的数据
                // brand_lists:[],//新版本不再添加品牌模块
                goods_list: [],
                header: []
            },
    ...
    methods: {
        ...
        logoUrl: function logoUrl(name, url, goods_id, type, intro, link, image_url) {
            if (type == undefined) type = 0;
            switch (type) {
                case 0:
                    this.jumpToBanner(name, url, link, image_url, name, intro);
                    break;
                case 1:
                    if (this.compareVersion(weex.config.env.appVersion, '3.4.0')) {
                        this.jumpTo2('shop/goodsDetailsParent', '', { goods_id: goods_id });
                    } else {
                        this.jumpTo2('shop/goodsDetails', '', { goods_id: goods_id });
                    }
                    break;
            }
        },
        ...
        tabChoosed: function tabChoosed(i, tabId, category_type) {
            //tab页签切换 tabId是用于请求数据时候发送过去和切换;(tabId比所以大1)
            var self = this;
            self.activeTab = i;
            ...
            self.goodsBrandData(tabId, category_type);
        },
        tabDataLoad: function tabDataLoad() {
            //tab标签的标题
            var self = this;
            self.Get(self.api().order_center_category_list_new, {}, function (res) {
                if (res.data.code != '0') {
                    //请求数据失败
                    ...
                    var exists = true;
                    for (var arr in self.tabs) {
                        if (self.tabs[arr].id == self.cateId) {
                            exists = true;
                            break;
                        }
                        exists = false;
                    ...
                    if (!exists) {
                        self.activeTab = 0;
                        self.cateId = self.tabs[0].id;
                        self.category_type = self.tabs[0].category_type;
                    }
                    self.goodsBrandData(self.cateId, self.category_type);
                    self.tabs[self.activeTab].choseType = "choosed"; //刚进来默认给第一个tab处于选中状态
                }
            });
        },
        goodsBrandData: function goodsBrandData(cateId, category_type) {
            //品牌和产品的数据请求
            var self = this;
            this.Get(self.api().order_center_category_getCategoryDetail_new, {
                cate_id: cateId,
                ...
                if (res.data.code != '0') {
                    ...
                    //请求数据成功
                    self.data.goods_list = res.data.data.data; //品牌或者商品的数据
                    self.data.header = res.data.data.header; //商品header数据
                }
            });
        },

        goodsDetail: function goodsDetail(goodsId) {
            //点击跳转---到商品详情页 带参数 商品id
            if (this.compareVersion(weex.config.env.appVersion, '3.4.0')) {
        ...
        brandDetail: function brandDetail(title, brandId, url) {
            //点击跳转---到品牌详情页
            if (this.compareVersion(weex.config.env.appVersion, '3.2.2')) {
                this.jumpTo2('classify/brandDetail', '', { title: title, brandId: brandId });
            } else {
                titleBar.startWeb({
                    title: title,
                    url: url
        ...
        searchData: function searchData() {
            var self = this;
            this.Get(self.api().search_hot, {}, function (res) {
                if (res.data.code != '0') {
                    //请求数据失败
                    promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                } else {
                    ...
                    if (keyword != '' && keyword.length > 0) {
                        ...
                        for (var i = 0; i < keyword.length; i++) {
                            if (weex.config.env.platform.toLowerCase() === 'ios') {
                                data.push("         " + keyword[i].name);
                            } else {
                                data.push(keyword[i].name);
                            }
                        }
                        self.keywordMarquee = data;
                    }
                }
            });
        }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["active-red-line"],
    style: {
      top: _vm.activeTab * 140 + 'px'
    }
    ...
    return _c('div', {
      ...
      on: {
        "click": function($event) {
          _vm.tabChoosed(i, tab.id, tab.category_type)
        }
      }
    }, [_c('div', {
      class: ['tab-detail', tab.choseType == 'choosed' ? 'active' : '']
    }, [_c('text', {
      class: ['title', tab.choseType == 'choosed' ? 'active2' : '']
    }, [_vm._v(_vm._s(tab.name))])]), _c('line', {
      style: {
        width: '131px',
        paddingLeft: '30px'
      }
    ...
    on: {
      "click": function($event) {
        _vm.logoUrl(_vm.data.header.name, _vm.data.header.title_url, _vm.data.header.goods_id, _vm.data.header.type, _vm.data.header.intro, _vm.data.header.link, _vm.data.header.image_thumb_url)
      }
    }
  }, [_c('image', {
    ...
    attrs: {
      "resize": "stretch",
      "src": _vm.data.header.logo_img
    }
  })]) : _vm._e(), (_vm.data.header) ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('div', {
    staticClass: ["hotSell-title"]
  }, [(_vm.category_type != 'brand') ? _c('text', {
    staticClass: ["hotSell-title-text"]
  }, [_vm._v("— " + _vm._s(_vm.data.header.name) + " —")]) : _vm._e(), (_vm.category_type == 'brand') ? _c('text', {
    staticClass: ["hotSell-title-text"]
  }, [_vm._v("— 所有品牌 —")]) : _vm._e(), (_vm.category_type != 'brand') ? _c('text', {
    staticClass: ["hotSell-title-text2"]
  }, [_vm._v(_vm._s(_vm.data.header.name_1))]) : _vm._e()])]) : _vm._e(), (_vm.category_type != 'brand') ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('div', {
    staticClass: ["hotSell-content"]
  }, _vm._l((_vm.data.goods_list), function(goods, i) {
    return _c('div', {
      on: {
        "click": function($event) {
          _vm.goodsDetail(goods.goods_id)
        }
      }
    }, [_c('div', {
      staticClass: ["hotSell-content-detail"]
    }, [_c('image', {
      staticClass: ["img"],
      attrs: {
        "src": goods.title_img
      ...
      staticClass: ["hotSell-content-detail-text"]
    }, [_vm._v(_vm._s(goods.name))])])])
  }))]) : _vm._e(), (_vm.category_type == 'brand') ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('div', {
    staticClass: ["hotSell-content2"]
  }, _vm._l((_vm.data.goods_list), function(goods, i) {
    return _c('div', {
      staticClass: ["hotSell-content-detail2"],
      on: {
        "click": function($event) {
          _vm.brandDetail(goods.title, goods.brand_id, goods.brand_url)
        }
      }
    }, [_c('image', {
      staticClass: ["img2"],
      ...
      staticClass: ["hotSell-content-detail-text2"]
    }, [_vm._v(_vm._s(goods.name))])])
  }))]) : _vm._e()])])], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
