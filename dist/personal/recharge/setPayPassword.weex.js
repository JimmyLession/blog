...
var _setPayPassword = __webpack_require__(2);

var _setPayPassword2 = _interopRequireDefault(_setPayPassword);
...
_setPayPassword2.default.el = '#root';
new Vue(_setPayPassword2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\recharge\\setPayPassword.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-353a5137"
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            password: '',
            inputValue: '',
            //type = set       未绑卡第一次设置支付密码
            //type = set_pay   绑卡后未设置支付密码  确认支付密码
            //type = sure      未绑卡确认设置支付密码
            //type = sure_pay  绑卡后未确认设置支付密码    第一次设置支付密码并且发送支付请求
            //type = recharge  绑卡后并且设置过支付密码发送支付请求
            type: ''
        };
    },
    ...
    methods: {
        ...
        //确认支付
        applyPost: function applyPost(isPay) {
            var self = this;
            if (weex.config.env.platform.toLowerCase() === 'android') utilModule.hideKeyBorad();
            loadingModule.show({ msg: '' });
            self.Post(self.api().bank_recharge, {
                type_str: 'confirm',
                id: self.id,
                recharge_amount: self.recharge_amount,
                recharge_sn: self.recharge_sn,
                smscode: self.smscode,
                thpinfo: self.thpinfo,
                pay_password: self.inputValue
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    ...
                    if (result == 'dismiss') {
                        ...
                        if (res.data.code == 0) {
                            promptModule.showListener({ msg: '充值成功', type: 'success' }, function (ev) {
                                eventBank.postMessage(self.eventBusTag().REFRESH_WALLET);
                                if (isPay) {
                                    event.postMessage(self.eventBusTag().REFRESH_RECHARGE);
                                    self.kill(4);
                                } else {
                                    self.kill(3);
                                }
                            ...
                            promptModule.show({ title: '温馨提示',
                                message: '支付密码错误，请重试',
                                cancel: '取消',
                                confirm: '重试' }, function (ret) {
                                var result = ret.result;
                                if (result == 'success') {
                                    self.$refs.password.resetPassWord();
                                    if (weex.config.env.platform.toLowerCase() === 'android') self.$refs.password.showKeyBorad();
                                }
                            });
                        } else {
        ...
        //设置支付密码
        setPassword: function setPassword(isPay) {
            var _this = this;
            if (_this.inputValue == _this.password) {
                if (weex.config.env.platform.toLowerCase() === 'android') utilModule.hideKeyBorad();
                _this.Post(_this.api().set_pay_password, {
                    new_password: _this.inputValue,
                    confirm_password: _this.inputValue
                }, function (ret) {
                    if (ret.data.code == 0) {
                        if (isPay) {
                            //支付密码设置成功发送充值请求
                            _this.applyPost(isPay);
                        } else {
                            promptModule.showListener({ msg: '支付密码设置成功', type: 'success' }, function (res) {
                                //支付密码设置成功返回充值页面 并且刷新数据
                                event.postMessage(_this.eventBusTag().REFRESH_RECHARGE);
                                _this.kill(4);
                            });
                        }
                    } else {
                        promptModule.showPrompt({ msg: ret.data.message, type: 'err' });
                    }
                });
            } else {
                promptModule.showPrompt({ msg: '两次输入的密码不一致，请重新输入', type: 'err' });
            }
        },
        ...
        getValue: function getValue(event) {
            var _this = this;_this.inputValue = event.value;
            switch (_this.type) {
                case 'set':
                    _this.password = _this.inputValue;
                    _this.jumpTo2('personal/recharge/setPayPassword', '', { type: 'sure', mobile: _this.mobile, price: _this.price, password: _this.inputValue });
                    break;
                case 'set_pay':
                    _this.password = _this.inputValue;
                    _this.jumpTo2('personal/recharge/setPayPassword', '', { type: 'sure_pay', mobile: _this.mobile, price: _this.price, password: _this.inputValue, bank_id: _this.id, recharge_amount: _this.money, recharge_sn: encodeURIComponent(_this.recharge_sn), smscode: _this.smscode, thpinfo: encodeURIComponent(_this.thpinfo) });
                    break;
                case 'sure':
                    //第一次添加银行卡 去设置支付密码
                    _this.setPassword(false);
                    break;
                case 'sure_pay':
                    //非第一次添加银行卡 去设置支付密码并且确认支付
                    _this.setPassword(true);
                    break;
                case 'recharge':
                    //无需设置支付密码 确认支付
                    _this.applyPost(false);
                    break;
            }
        }
    },
    created: function created() {
        ...
        _this.GetUrlPara(weex.config.bundleUrl, 'mobile') ? _this.mobile = _this.GetUrlPara(weex.config.bundleUrl, 'mobile') : '';
        _this.GetUrlPara(weex.config.bundleUrl, 'price') ? _this.price = _this.GetUrlPara(weex.config.bundleUrl, 'price') : '';
        _this.GetUrlPara(weex.config.bundleUrl, 'password') ? _this.password = _this.GetUrlPara(weex.config.bundleUrl, 'password') : '';
        _this.id = _this.GetUrlPara(weex.config.bundleUrl, 'bank_id');
        _this.recharge_amount = _this.GetUrlPara(weex.config.bundleUrl, 'recharge_amount');
        _this.recharge_sn = decodeURIComponent(_this.GetUrlPara(weex.config.bundleUrl, 'recharge_sn'));
        _this.smscode = _this.GetUrlPara(weex.config.bundleUrl, 'smscode');
        _this.thpinfo = decodeURIComponent(_this.GetUrlPara(weex.config.bundleUrl, 'thpinfo'));
    },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [(_vm.type == 'set' || _vm.type == 'set_pay') ? _c('title', {
    attrs: {
      "title": "设置支付密码"
    }
  }) : _vm._e(), (_vm.type == 'sure' || _vm.type == 'sure_pay') ? _c('title', {
    attrs: {
      "title": "确认支付密码"
    }
  }) : _vm._e(), (_vm.type == 'recharge') ? _c('title', {
    attrs: {
      "title": "支付密码"
    }
  }) : _vm._e(), _c('div', {
  ...
  })], 1), (_vm.type == 'set' || _vm.type == 'sure' || _vm.type == 'set_pay' || _vm.type == 'sure_pay') ? _c('text', {
    staticClass: ["text-content"]
  }, [_vm._v("请设置6位数字支付密码，建议勿与银行卡取款密码相同")]) : _vm._e()])], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
