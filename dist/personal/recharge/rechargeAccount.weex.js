...
var _rechargeAccount = __webpack_require__(2);

var _rechargeAccount2 = _interopRequireDefault(_rechargeAccount);
...
_rechargeAccount2.default.el = '#root';
new Vue(_rechargeAccount2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\recharge\\rechargeAccount.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-d9af3940"
...
var _rechargePay = __webpack_require__(23);

var _rechargePay2 = _interopRequireDefault(_rechargePay);

var _wxButton = __webpack_require__(27);
...
var event = new BroadcastChannel('rechargeBingCard');
var eventBank = new BroadcastChannel('wallet');
var comModal = weex.requireModule('ComModule'); //原生提供的ComModule模块
exports.default = {
    components: { title: _titleBar2.default, line: _line2.default, WxcPopup: _wxcPopup2.default, rechargePop: _rechargePay2.default, WxButton: _wxButton2.default },
    data: function data() {
        return {
            isParam: false,
            version: false,
    ...
    methods: {
        ...
        selected_Wx: function selected_Wx() {
            this.id = 0;
            this.selectPOS = false;
            this.selectAlipay = false;
            if (this.selectWx) return;
            this.selectWx = !this.selectWx;
        },
        selected_Alipay: function selected_Alipay() {
        ...
        selected_POS: function selected_POS() {
            this.id = 0;
            this.selectWx = false;
            this.selectAlipay = false;
            if (this.selectPOS) return;
            this.selectPOS = !this.selectPOS;
        },
        close: function close() {
            this.showPay = false;
        },
        selected_bank: function selected_bank(item) {
            this.id = item.id;
            this.selectWx = false;
            this.selectPOS = false;
            this.selectAlipay = false;
            this.bank_mobile = item.mobile;
        },
        limitAmount: function limitAmount() {
            this.jumpTo2('personal/recharge/rechargeQuota', '', {});
        },

        //添加银行卡
        addPayBank: function addPayBank() {
            this.showPay = false;
            this.jumpTo2('personal/recharge/rechargeAddBankCard', '', {});
        },
        rechargeWx: function rechargeWx() {
            var self = this;
            loadingModule.show({ msg: '正在支付...' });
            self.Post(self.api().recharge_wx, {
                recharge_amount: self.money
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    var result = ret.result;
                    if (result == 'dismiss') {
                        if (res.data.code == 0) {
                            comModal.pay({ pay_type: 1, json: res.data.data, pay_source: 'pay' }, function (event) {
                                switch (event.result) {
                                    case 'success':
                                        promptModule.showListener({ msg: '充值成功', type: 'success' }, function (evt) {
                                            eventBank.postMessage(self.eventBusTag().REFRESH_WALLET);
                                            self.popBar();
                                        });
                                        break;
                                    case 'failed':
                                        promptModule.showPrompt({ msg: '充值失败', type: 'err' });
                                        break;
                                    case 'unknow':
                                        eventBank.postMessage(self.eventBusTag().REFRESH_WALLET);
                                        break;
        ...
        rechargePOS: function rechargePOS() {
            var self = this;
            loadingModule.show({ msg: '' });
            self.Post(self.api().pos_recharge, {
                recharge_amount: self.money
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    var result = ret.result;
                    if (result == 'dismiss') {
                        if (res.data.code == 0) {
                            self.interceptPush('personal/recharge/rechargePOS', { money: res.data.data.amount, recharge_sn: res.data.data.recharge_sn, user_name: res.data.data.user_name, img_url: res.data.data.img_url }, 'intercept');
                        } else {
                            promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                        }
                    }
        ...
        goToPay: function goToPay() {
            ...
            if (this.bank_list.length > 0) {
                if (this.id != 0) {
                    this.showPay = false;
                    this.jumpTo2('personal/recharge/rechargeConfirmPay', '', { id: this.id, mobile: this.bank_mobile, price: this.money });
                } else {
                    promptModule.showPrompt({ msg: "请先选择添加银行卡", type: 'err' });
                }
            } else {
                //如果没有添加银行 跳转确认支付
                this.addPayBank();
        ...
        rechargeAlipay: function rechargeAlipay() {
            var self = this;
            loadingModule.show({ msg: '正在支付...' });
            self.Post(self.api().recharge_alipay, {
                recharge_amount: self.money
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    var result = ret.result;
                    if (result == 'dismiss') {
                        if (res.data.code == 0) {
                            comModal.pay({ pay_type: 2, json: JSON.stringify(res.data.data), pay_source: 'pay' }, function (event) {
                                switch (event.result) {
                                    case 'success':
                                        promptModule.showListener({ msg: '充值成功', type: 'success' }, function (evt) {
                                            eventBank.postMessage(self.eventBusTag().REFRESH_WALLET);
                                    ...
                                    case 'unknow':
                                        loadingModule.show({ msg: '' });
                                        self.testRecharge(1, res.data.data.out_trade_no);
                                        break;
                                }
                            });
                        } else {
        ...
        testRecharge: function testRecharge(index, out_trade_no) {
            var self = this;
            self.Get(self.api().get_notify_result, {
                ...
                if (res.data.data == 1) {
                    ...
                    if (index == 1) self.testRecharge(2, out_trade_no);else {
                        loadingModule.cancel(function (ret) {});
                    }
                }
            });
        ...
        getMoney: function getMoney(e) {
            this.money = e.value;
            this.isParam = this.isValueParam(this, [this.money]);
        },
        recharge: function recharge() {
            var _this = this;
            if (_this.money == '') return;
            if (_this.showPay) return;
            _this.showPay = true;
        ...
        request: function request() {
            var _this = this;
            _this.Get(_this.api().get_allinpay_bank_list, {}, function (res) {
                if (res.data.code == 0) {
                    _this.mobile = res.data.data.recharge_account;
                    _this.bank_list = res.data.data.bank_list;
                } else {
                    promptModule.showPrompt({ msg: res.data.message, type: 'err' });
                }
            });
        }
    },
    created: function created() {
        ...
        _this.isIpx(_this);
        _this.iPhoneX(_this);
        _this.version = !_this.compareVersion(weex.config.env.appVersion, '3.6.0');
        if (_this.isIphoneX) _this.popHeight = 768;else _this.popHeight = 700;
        _this.request();
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\components\\rechargePay.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-5a64ac80"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var animation = weex.requireModule('animation');
var modal = weex.requireModule('modal');
var height = 750 / weex.config.env.deviceWidth * weex.config.env.deviceHeight;
var promptModule = weex.requireModule('promptModule');
exports.default = {
    props: ['rules', 'title', 'isset_pay_password', 'mobile'],
    components: {},
    data: function data() {
        return {
            isIphoneX: false,
    ...
    methods: {
        ...
        //添加银行卡
        addPayBank: function addPayBank() {
            if (this.isset_pay_password) {
                this.jumpTo2('personal/recharge/rechargeAddBankCard', '', {});
            } else {
                this.jumpTo2('personal/recharge/setPayPassword', '', { type: 'set', mobile: this.mobile, price: this.title, password: '' });
            }
            this.close();
        },
        selected_bank: function selected_bank() {
            this.isSelected = !this.isSelected;
        },

        //限额说明
        limitAmount: function limitAmount() {
            promptModule.showPrompt({ msg: '链接我也不知道是哪个', type: "err" });
        },
        goToPay: function goToPay() {
            //如果有添加银行 跳转确认支付
            if (this.rules.length > 0) {
                if (this.isSelected) {
                    this.jumpTo2('personal/recharge/rechargeConfirmPay', '', { mobile: this.mobile, price: this.title });
                    this.close();
                } else {
                    promptModule.showPrompt({ msg: "请先选择添加银行卡", type: 'err' });
                }
    ...
    created: function created() {
        ...
        if (weex.config.env.platform == 'iOS') {
            self.scrollHeightStyle = { height: '497px' };
        } else if (weex.config.env.platform == 'android') {
            self.scrollHeightStyle = { maxHeight: '700px' };
        }
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [(_vm.rules.length != 0) ? _c('scroller', {
    staticClass: ["bank_list"]
  }, [_vm._l((_vm.rules), function(item) {
    return _c('div', {
      staticClass: ["bank"]
    }, [_c('div', {
      staticClass: ["bank_name_bg"]
    }, [_c('div', {
      staticClass: ["left_name"],
      on: {
        "click": function($event) {
          _vm.selected_bank()
        }
      }
    }, [(!_vm.isSelected) ? _c('image', {
      staticClass: ["select_image"],
      attrs: {
        "src": "assets://icon_pay_unselect"
      }
    }) : _vm._e(), (_vm.isSelected) ? _c('image', {
      staticClass: ["select_image"],
      attrs: {
    ...
    }, [_vm._v(_vm._s(item.bankname.substr(0, item.bankname.length - 6)))]), _c('text', {
      staticClass: ["bank_end_name"]
    }, [_vm._v(_vm._s(item.bankname.substr(item.bankname.length - 6, 6)))])]), _c('div', {
      staticClass: ["limit_bg"],
      on: {
        "click": function($event) {
          _vm.limitAmount()
        }
      ...
      staticClass: ["amount"]
    }, [_vm._v("限额说明")]), _c('image', {
      staticClass: ["limit_image"],
      attrs: {
        "src": "assets://icon_explain"
      }
    })])]), _c('text', {
      staticClass: ["bank_explain"],
      on: {
        "click": function($event) {
          _vm.selected_bank()
        }
      }
    }, [_vm._v(_vm._s(item.quota_des + '*单笔限额10万单月50万'))]), _c('div', {
      staticClass: ["line"]
    })])
  }), _c('div', {
    ...
    staticClass: ["add_bank_image"],
    attrs: {
      "src": "assets://icon_add_circle"
    }
  }), _c('text', {
    staticClass: ["add_bank_text"]
  }, [_vm._v("使用银行卡支付")])])]), _c('div', {
    staticClass: ["line"]
  })])], 2) : _vm._e(), (_vm.rules.length == 0) ? _c('div', {
    staticClass: ["add_bank"]
  }, [_c('div', {
    staticClass: ["item_bg"]
  }, [_c('div', {
  ...
  }, [_vm._v("去付款")])])])]), (_vm.isIphoneX) ? _c('div', {
    staticStyle: {
      height: "68px",
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
  ...
  }, [_vm._v(_vm._s(_vm.mobile))])]), _c('line'), _c('div', {
    staticClass: ["content-bg"]
  }, [_c('text', {
    staticClass: ["content"]
  }, [_vm._v("充值金额")]), _c('edit', {
    staticClass: ["content-money"],
    attrs: {
      "placeholder": "请输入充值金额",
      "type": "decimal",
      "returnKeyType": "done"
    ...
    on: {
      "onInput": _vm.getMoney
    }
  })], 1), _c('line'), _vm._m(0), _c('div', [_c('wx-button', {
    attrs: {
      "marginTop": "160",
      "content": "确定",
      "isValueParam": _vm.isParam
    },
    on: {
    ...
    staticClass: ["top-title-price"]
  }, [_vm._v("  ¥ " + _vm._s(_vm.money))]), _c('image', {
    staticClass: ["top-cancle"],
    attrs: {
      "src": "assets://ic_goods_details_remove"
    },
    ...
    staticClass: ["left_name-alipay"]
  }, [(_vm.version) ? _c('image', {
    staticClass: ["select_image"],
    attrs: {
      "src": "http://img.abmau.com/abmau/5698c91b332d79ffd53f6135514356c0.png"
    }
  }) : _vm._e(), (!_vm.selectAlipay && !_vm.version) ? _c('image', {
    staticClass: ["select_image"],
    attrs: {
      "src": "assets://icon_pay_unselect"
    }
    ...
    staticClass: ["bank_name"]
  }, [_vm._v("支付宝支付")]), (_vm.version) ? _c('text', {
    staticClass: ["bank_name_des"]
  }, [_vm._v("当前版本不支持支付宝支付，请升级到最新版本")]) : _vm._e()])])]), _c('div', {
    staticClass: ["line"]
  })]), _c('cell', {
    staticClass: ["bank"],
    appendAsTree: true,
    ...
    staticClass: ["left_name"]
  }, [(!_vm.selectPOS) ? _c('image', {
    staticClass: ["select_image"],
    attrs: {
      "src": "assets://icon_pay_unselect"
    }
  }) : _vm._e(), (_vm.selectPOS) ? _c('image', {
    staticClass: ["select_image"],
    attrs: {
      "src": "assets://icon_pay_selected"
    }
    ...
    staticClass: ["bank_name"]
  }, [_vm._v("线下POS机支付")])])]), _c('div', {
    staticClass: ["line"]
  })]), _vm._l((_vm.bank_list), function(item) {
    return _c('cell', {
      staticClass: ["bank"],
      appendAsTree: true,
      attrs: {
      ...
      on: {
        "click": function($event) {
          _vm.selected_bank(item)
        }
      }
    }, [_c('div', {
      staticClass: ["left_name"]
    }, [(item.id != _vm.id) ? _c('image', {
      staticClass: ["select_image"],
      attrs: {
        "src": "assets://icon_pay_unselect"
      }
    }) : _vm._e(), (item.id == _vm.id) ? _c('image', {
      staticClass: ["select_image"],
      attrs: {
        "src": "assets://icon_pay_selected"
      }
  ...
  }, [_vm._v("去付款")])], 1), (_vm.isIphoneX) ? _c('div', {
    staticStyle: {
      height: "68px",
      backgroundColor: "white"
    }
  ...
  return _c('div', {
    ...
    staticClass: ["alter-text"]
  }, [_vm._v("银行对于单笔、单日支付有限额。银行卡支付限额请查看《限额说明》，微信支付限额请以微信支付页面提示为准。")]), _c('text', {
    staticClass: ["alter-text"]
  }, [_vm._v("* 超过单笔限额：您可以分多笔充值到账户")]), _c('text', {
    staticClass: ["alter-text"]
  }, [_vm._v("* 超过单日限额：您可以使用其他银行卡（可以使用多个不同持卡人的银行卡）充值到账户")])])
}]}
module.exports.render._withStripped = true
