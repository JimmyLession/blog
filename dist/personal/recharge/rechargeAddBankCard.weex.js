...
var _rechargeAddBankCard = __webpack_require__(3);

var _rechargeAddBankCard2 = _interopRequireDefault(_rechargeAddBankCard);
...
_rechargeAddBankCard2.default.el = '#root';
new Vue(_rechargeAddBankCard2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\recharge\\rechargeAddBankCard.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-022f1240"
...
module.exports = {
  ...
  "edit-item-alter-image": {
    "width": "20",
    "height": "24"
  },
  "edit-item-alter-text": {
    "paddingLeft": "10",
    "lineHeight": "70",
    "height": "70",
    "textAlign": "center",
  ...
  "edit_item_bottom-image": {
    "width": "140",
    "height": "40"
  },
  "edit_item_bottom-line": {
    "width": "1",
    "height": "26",
    "backgroundColor": "#e1e1e1",
    "marginLeft": "30",
...
var _wxButton = __webpack_require__(28);
...
exports.default = {
    components: { title: _titleBar2.default, line: _line2.default, WxcMask: _wxcMask2.default, WxcPopup: _wxcPopup2.default, WxButton: _wxButton2.default },
    data: function data() {
        return {
            isParam: false,
            bankId: 0,
    ...
    methods: {
        tabBank: function tabBank(id, name) {
            this.bankId = id;
            this.bank_info = name;
            this.bankColor = '#333333';
            this.isBottomShow = false;
        },
        popupOverlayBottomClick: function popupOverlayBottomClick() {
            this.isBottomShow = false;
        },
        chooseBank: function chooseBank() {
            this.isBottomShow = true;
        },
        onBankInput: function onBankInput(event) {
            this.bank_id = event.value;
            this.isParam = this.isValueParam(this, [this.bank_id, this.bank_name, this.user_id, this.user_mobile]);
        },
        onBankNameInput: function onBankNameInput(event) {
            this.bank_name = event.value;
            this.isParam = this.isValueParam(this, [this.bank_id, this.bank_name, this.user_id, this.user_mobile]);
        },
        onUserIDInput: function onUserIDInput(event) {
            this.user_id = event.value;
            this.isParam = this.isValueParam(this, [this.bank_id, this.bank_name, this.user_id, this.user_mobile]);
        },
        onUserMobileInput: function onUserMobileInput(event) {
            this.user_mobile = event.value;
            if (this.user_mobile != '' && this.user_mobile.length >= 4) this.verify_mobile = this.user_mobile.substring(this.user_mobile.length - 4, this.user_mobile.length);
            this.isParam = this.isValueParam(this, [this.bank_id, this.bank_name, this.user_id, this.user_mobile]);
        },
        onUserCodeInput: function onUserCodeInput(event) {
            this.mobile_code = event.value;
            if (this.mobile_code == '') {
                this.verify_backgroundColor = '#999999';
            } else {
                this.verify_backgroundColor = '#333333';
            }
        },
        clickCheck: function clickCheck() {
            this.isCheck = !this.isCheck;
        },
        clickCloseVerify: function clickCloseVerify() {
            this.showVerify = false;
        },
        clickMobile: function clickMobile(event) {
            this.showExplain = true;
        },
        clickClose: function clickClose() {
            this.showExplain = false;
        },
        clickCode: function clickCode(event) {
            var self = this;
            if (!self.isClick) return;
            if (event.timestamp - self.lastTimestampCode < 3000) return;
            self.lastTimestampCode = event.timestamp;
            self.mobile_code = '';
            loadingModule.show({ msg: '正在发送...' });
            self.applyPost('resend');
        },
        submitBind: function submitBind(event) {
            var self = this;
            if (self.mobile_code == '') return;
            if (event.timestamp - self.lastTimestampConfirm < 3000) return;
            self.lastTimestampConfirm = event.timestamp;
            loadingModule.show({ msg: '正在添加...' });
            self.applyPost('confirm');
        },
        sendCode: function sendCode() {
            var _this = this;
            _this.isClick = false;
            _this.countDownObject = _this.countDown(60, function (t) {
                _this.verify_content = t + '秒';
            }, function () {
                _this.isClick = true;
                _this.verify_content = '发送';
            });
        },
        applyPost: function applyPost(type) {
            var self = this;
            self.Post(self.api().bind_bank, {
                type_str: type,
                accttype: 0,
                type: '',
                acctno: self.bank_id,
                idno: self.user_id,
                acctname: self.bank_name,
                mobile: self.user_mobile,
                smscode: self.mobile_code,
                thpinfo: self.thpinfo
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    if (res.data.code == 0) {
                        if (type == 'apply') {
                            self.verify_content = "";
                            self.thpinfo = res.data.data.thpinfo;
                            self.showVerify = true;
                            if (self.countDownObject != '') clearInterval(self.countDownObject);
                            self.sendCode();
                        } else if (type == 'resend') {
                            self.verify_content = "";
                            self.sendCode();
                        } else if (type == 'confirm') {
                            self.interceptPush('personal/recharge/rechargeBindSuccess', {}, 'intercept');
                        }
                    } else {
        ...
        submitBank: function submitBank() {
            var self = this;
            /*if(event.timestamp - self.lastTimestamp < 3000)return;
            self.lastTimestamp = event.timestamp;*/
            if (self.bankId == 0) {
                promptModule.showPrompt({ msg: '请选择发卡银行', type: 'prompt' });return;
            } else if (self.bank_id == '') {
                promptModule.showPrompt({ msg: '请输入银行卡号(仅支持储蓄卡)', type: 'prompt' });return;
            } else if (self.bank_name == '') {
                promptModule.showPrompt({ msg: '请输入银行卡户名', type: 'prompt' });return;
            } else if (self.user_id == '') {
                promptModule.showPrompt({ msg: '请输入持卡对应身份证号', type: 'prompt' });return;
            } else if (self.user_mobile == '') {
                promptModule.showPrompt({ msg: '请输入银行预留手机号', type: 'prompt' });return;
            } else if (!self.isCheck) {
                promptModule.showPrompt({ msg: '请同意快捷支付协议', type: "prompt" });return;
            }
            loadingModule.show({ msg: '正在申请...' });
            self.applyPost('apply');
        },
        bankInfo: function bankInfo() {
            var _this = this;
            _this.Get(_this.api().allinpay_allinpay_bank_list, {}, function (res) {
                if (res.data.code == "0") {
    ...
    created: function created() {
        ...
        _this.bankInfo();
        _this.GetUrlPara(weex.config.bundleUrl, 'type') ? _this.type = _this.GetUrlPara(weex.config.bundleUrl, 'type') : '';
    },
    destroyed: function destroyed() {
        clearInterval(this.countDownObject);
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
    ...
    staticClass: ["edit-item-edit-bank"],
    style: {
      color: _vm.bankColor
    }
  }, [_vm._v(_vm._s(_vm.bank_info))]), _c('image', {
    staticClass: ["next"],
    attrs: {
    ...
    ref: "bank_id",
    staticClass: ["edit-item-edit"],
    attrs: {
      "type": "number",
      "placeholder": "请输入银行卡号(仅支持储蓄卡)"
    },
    on: {
      "onInput": _vm.onBankInput
    }
  })], 1), _vm._m(0), _c('div', {
    staticClass: ["edit-item"]
  }, [_c('text', {
    ...
    staticClass: ["edit-item-edit"],
    attrs: {
      "placeholder": "请输入持卡对应身份证号",
      "type": "text"
    },
    on: {
      "input": _vm.onUserIDInput
    }
  })]), _c('div', {
    staticStyle: {
      width: "750px",
    ...
    staticClass: ["edit-item-alter-text"]
  }, [_vm._v("*提醒后续仅支持绑定该持卡人的银行卡")]) : _vm._e()]), _c('div', {
    staticClass: ["edit-item"]
  }, [_c('text', {
    staticClass: ["edit-item-title"]
  }, [_vm._v("手机号")]), _c('edit', {
    ...
    on: {
      "onInput": _vm.onUserMobileInput
    }
  }), _c('image', {
    staticClass: ["edit-item-edit-image"],
    attrs: {
      "src": "assets://icon_explain"
    },
    on: {
      "click": _vm.clickMobile
    }
  })], 1), _c('line'), _vm._m(1), _c('wx-button', {
    attrs: {
      "marginTop": "50",
      "content": "确定",
    ...
    staticClass: ["edit_item_bottom"]
  }, [_c('image', {
    staticClass: ["edit_item_bottom-image"],
    attrs: {
      "src": "assets://icon_allin_pay"
    }
  }), _c('div', {
    staticClass: ["edit_item_bottom-line"]
  }), _c('text', {
    staticClass: ["edit-item-alter-text"]
  }, [_vm._v("关于通联支付?")])]) : _vm._e(), _c('wxc-mask', {
    attrs: {
      "height": "414",
      "width": "570",
      "borderRadius": "10",
  ...
  }, [_vm._v("手机号说明")]), _c('text', {
    staticClass: ["mask-content"]
  }, [_vm._v("银行预留手机号码是在线办理该银行卡时所填写的手机号码。")]), _c('text', {
    staticClass: ["mask-content"]
  }, [_vm._v("没有预留手机号码，手机号忘记或停用，请联系银行客服更新处理。")]), _c('text', {
    staticClass: ["mask-alter"]
  }, [_vm._v("*请注意区别绑定单创的手机号码")])])])]), _c('wxc-mask', {
    attrs: {
      "height": "480",
      "width": "570",
      "borderRadius": "10",
    ...
    ref: "mobile_code",
    staticClass: ["verify-edit"],
    attrs: {
      "type": "number",
      "placeholder": "请输入验证码"
    },
    on: {
      "onInput": _vm.onUserCodeInput
    }
  }), _c('div', {
    staticClass: ["verify-line"]
  }), _c('text', {
    staticClass: ["verify-code-btn"],
    on: {
      "click": _vm.clickCode
    }
  }, [_vm._v(_vm._s(_vm.verify_content))])], 1), _c('text', {
    staticClass: ["verify-alter"]
  }, [_vm._v("已向尾号为" + _vm._s(_vm.verify_mobile) + "的手机发送短信验证码,输入后完成绑定")]), _c('text', {
    staticClass: ["verify-submit"],
    style: {
      backgroundColor: _vm.verify_backgroundColor
    },
    on: {
      "click": _vm.submitBind
    }
  }, [_vm._v("确定")])])])])], 1), _c('wxc-popup', {
    attrs: {
    ...
    style: {
      height: (_vm.popHeight - 80) + 'px'
    }
  }, _vm._l((_vm.bankData), function(item, i) {
    return _c('cell', {
      key: i,
      staticClass: ["cell"],
      appendAsTree: true,
      ...
      staticClass: ["bank_list", "row"],
      on: {
        "click": function($event) {
          _vm.tabBank(item.id, item.name)
        }
      }
    }, [_c('div', {
      staticClass: ["bank_list_left", "column", "center"]
    }, [_c('image', {
      staticClass: ["bank_list_image"],
      attrs: {
        "src": item.url
      }
    })]), _c('div', {
      ...
      staticClass: ["bank_list_right", "column", "between"]
    }, [_c('text', {
      staticClass: ["bank_list_name"]
    }, [_vm._v(_vm._s(item.name))]), _c('text', {
      staticClass: ["bank_list_quota"]
    }, [_vm._v(_vm._s(item.des))])])]), _c('div', {
      staticClass: ["center"],
      staticStyle: {
        marginRight: "30px"
      }
    }, [(item.id == _vm.bankId) ? _c('image', {
  ...
  return _c('div', {
    staticClass: ["edit-item-alter"]
  }, [_c('image', {
    staticClass: ["edit-item-alter-image"],
    attrs: {
      "src": "assets://icon_green_info"
    }
  }), _c('text', {
    staticClass: ["edit-item-alter-text"]
  }, [_vm._v("信息加密处理，保障个人隐私不泄露")])])
},function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["agreement"]
  }, [_c('text', {
    staticClass: ["agreement_text"]
  }, [_vm._v("*目前系统仅支持储蓄卡充值，暂不支持信用卡充值")])])
}]}
module.exports.render._withStripped = true
