...
var _rechargeConfirmPay = __webpack_require__(2);

var _rechargeConfirmPay2 = _interopRequireDefault(_rechargeConfirmPay);
...
_rechargeConfirmPay2.default.el = '#root';
new Vue(_rechargeConfirmPay2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\recharge\\rechargeConfirmPay.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-4658ed36"
...
exports.default = {
    components: { title: _titleBar2.default, line: _line2.default, WxButton: _wxButton2.default },
    data: function data() {
        return {
            isParam: false,
            id: 1,
            ...
            lastTimestampCode: 0,
            lastTimestamp: 0,
            isset_pay_password: false
        };
    },

    methods: {
        onMobileCodeInput: function onMobileCodeInput(event) {
            this.mobile_code = event.value;
            this.isParam = this.isValueParam(this, [this.mobile_code]);
        },
        ...
        submitRecharge: function submitRecharge() {
            ...
            if (self.isset_pay_password) {
                //有支付密码 直接确认密码支付
                self.jumpTo2('personal/recharge/setPayPassword', '', { type: 'recharge', bank_id: self.id, recharge_amount: self.money, recharge_sn: encodeURIComponent(self.recharge_sn), smscode: self.mobile_code, thpinfo: encodeURIComponent(self.thpinfo) });
            } else {
                //无支付密码 设置支付密码完成支付
                self.jumpTo2('personal/recharge/setPayPassword', '', { type: 'set_pay', bank_id: self.id, recharge_amount: self.money, recharge_sn: encodeURIComponent(self.recharge_sn), smscode: self.mobile_code, thpinfo: encodeURIComponent(self.thpinfo) });
            }
        },
        fetch: function fetch() {
            var _this = this;
            this.Get(_this.api().isset_pay_password, {}, function (ret) {
                if (ret.data.code == 0) {
                    _this.isset_pay_password = ret.data.data == 1 ? true : false;
                } else {
                ...
                loadingModule.cancel(function (ret) {
                    if (res.data.code == 0) {
                        if (type == 'apply') {
                            ...
                            self.recharge_sn = res.data.data.recharge_sn;
                            self.sendCode();
                        } else if (type == 'resend') {
                            self.verify_content = "";
                            self.sendCode();
    ...
    created: function created() {
        ...
        _this.isIpx(_this);
        _this.money = this.GetUrlPara(weex.config.bundleUrl, 'price');
        _this.mobile = this.GetUrlPara(weex.config.bundleUrl, 'mobile');
        _this.id = this.GetUrlPara(weex.config.bundleUrl, 'id');
        _this.applyPost('apply');
        _this.fetch();
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
    ...
    staticClass: ["pay-item-title"]
  }, [_vm._v("充值金额")]), _c('text', {
    staticClass: ["pay-item-money"]
  ...
  }, [_vm._v(_vm._s(_vm.verify_content))])], 1), _c('line'), _c('wx-button', {
    attrs: {
      "marginTop": "160",
      "content": "确定",
