...
var _rechargeLevelConfirm = __webpack_require__(2);

var _rechargeLevelConfirm2 = _interopRequireDefault(_rechargeLevelConfirm);
...
_rechargeLevelConfirm2.default.el = '#root';
new Vue(_rechargeLevelConfirm2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\recharge\\rechargeLevelConfirm.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-8fa4c20e"
...
exports.default = {
    ...
    methods: {
        ...
        confirmUpgrade: function confirmUpgrade() {
            var self = this;
            self.Get(self.api().check_activity_button, {
                type: 'confirm_upgrade'
            }, function (res) {
                if (res.data.code == 0) {
                    promptModule.show({ title: '温馨提示',
                        message: '您本次升级等级：' + self.name,
                        cancel: '取消',
                    ...
                    promptModule.showPrompt({ msg: res.data.msg, type: 'prompt' });
                }
            });
        },
        confirm: function confirm() {
            ...
            self.Post(self.api().submit_new_brand_provider, {
                level: self.id
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    if (res.data.code == 0) {
                        utilModule.toggleAccount();
                        eventAccount.postMessage(self.eventBusTag().REFRESH_HOME);
                        self.kill(3);
                    } else {
        ...
        getLevelInfo: function getLevelInfo() {
            var self = this;
            self.Get(self.api().brand_provider_level, {
                level: self.id
            }, function (res) {
                if (res.data.code == 0) {
                    self.name = res.data.data.item.name;
                    self.parent_name = res.data.data.parent_name;
                    self.apply_money = res.data.data.item.apply_money;
                    self.update_payment = res.data.data.update_payment;
                    if (parseFloat(self.apply_money) <= parseFloat(self.update_payment)) self.isSatisfy = true;else self.isSatisfy = false;
                    self.diff_money = (parseFloat(self.apply_money) - parseFloat(self.update_payment)).toFixed(2);
                    if (self.diff_money < 1.00) self.placeholder = '还需要充值1.00';else self.placeholder = '还需要充值' + self.diff_money;
                } else {
                    promptModule.showPrompt({ msg: res.data.msg, type: "err" });
                }
            });
    ...
    created: function created() {
        ...
        _this.isIpx(_this);
        _this.id = _this.GetUrlPara(weex.config.bundleUrl, 'id');
        _this.getLevelInfo();
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v(_vm._s(_vm.name))])]), _c('line'), _c('div', {
    staticClass: ["level-item"]
  }, [_c('text', {
    staticClass: ["level-title"]
  }, [_vm._v("充值金额")]), _c('text', {
    staticClass: ["level-content"]
  }, [_vm._v(_vm._s(_vm.apply_money))])]), _c('line'), _c('div', {
    staticClass: ["level-item"]
  }, [_c('text', {
    staticClass: ["level-title"]
  }, [_vm._v("实际充值金额")]), _c('text', {
    staticClass: ["level-content"]
  }, [_vm._v(_vm._s(_vm.placeholder))])]), _c('line'), _c('wx-button', {
    attrs: {
      "marginTop": "160",
      "content": "去充值",
      "isValueParam": true
  ...
  })], 1) : _vm._e(), (_vm.isSatisfy) ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
    ...
    staticClass: ["level-content"]
  }, [_vm._v(_vm._s(_vm.name))])]), _c('line'), _c('wx-button', {
    attrs: {
      "marginTop": "160",
      "content": "确认升级",
      "isValueParam": true
