...
var _businessSchoolLv = __webpack_require__(4);

var _businessSchoolLv2 = _interopRequireDefault(_businessSchoolLv);
...
_businessSchoolLv2.default.el = '#root';
new Vue(_businessSchoolLv2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\businessSchool\\businessSchoolLv.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-d7003f72"
...
var _orderPop = __webpack_require__(24);

var _orderPop2 = _interopRequireDefault(_orderPop);

var _line = __webpack_require__(28);

var _line2 = _interopRequireDefault(_line);

var _empty = __webpack_require__(32);
...
var promptModule = weex.requireModule('promptModule');
var dom = weex.requireModule('dom'); //dom组件
var modal = weex.requireModule('modal'); //modal组件
var comModal = weex.requireModule('ComModule'); //用于获取设备的高度
var height = 0;
exports.default = {
    components: {
        titleBar: _titleBar2.default, refresher: _refresh2.default, WxcTabPage: _wxcTabPage2.default, line: _line2.default, empty: _empty2.default, orderPop: _orderPop2.default
    },
    data: function data() {
        return {
            popArray: [{
    ...
    methods: {
        ...
        updateScrollHeright: function updateScrollHeright() {
            //滚动的监听事件
            var _this = this;
            //获取导航栏和状态栏的高度
            _this.topHeight = parseInt(_this.getTitleHeight(_this)) + parseInt(_this.getStatusBarHeight(_this));
            dom.getComponentRect(_this.$refs.header[_this.index], function (option) {
                //当导航栏和状态栏的高度大于option.size.top
                if (_this.topHeight > option.size.top) {

                    _this.scrollHeight = _this.topHeight + 80;
                } else {

                    _this.scrollHeight = option.size.top + 80;
                }
                // modal.toast({ message: '123', duration: 5.0});
            });
        },
        showPop: function showPop() {
            //显示弹出层
            var _this = this;
            _this.showArray[_this.index].show = true;
        },
        closeOrderPop: function closeOrderPop() {
            //点击的遮罩层的时候影藏整个弹出层
            this.showArray[this.index].show = false;
        },
        sortFetch: function sortFetch(val, text) {
            //排序请求
            this.tabArrya[this.index].sort = val;
            this.tabArrya[this.index].text = text;
            this.sort[this.index].type = val;
            this.showArray[this.index].show = false;
            this.page[this.index] = 1;
            this.list = [];
            this.tabText = text;
            this.initArticle(this.index);
        },
        onloading: function onloading() {
            if (this.tabList[this.index].length < this.size) return;
            this.page[this.index] = this.page[this.index] + 1;
            this.initArticle(this.index);
        },
        clickArticle: function clickArticle(article_id) {
            this.jumpTo2('personal/businessSchool/businessSchoolDetail', '', { article_id: article_id });
        },
        ...
        wxcTabPageCurrentTabSelected: function wxcTabPageCurrentTabSelected(e) {
            var _this = this;
            _this.showArray[_this.index].show = false;
            var index = e.page;
            _this.sort[index].type = _this.tabArrya[index].sort;
            _this.index = index;
            _this.article_id = _this.category[index].category_id;
            _this.page[index] = 1;
            _this.empty[_this.index] = false;
            if (_this.tabList[index].length == 0) {
                _this.list = [];
                _this.initArticle(index);
            }
        },
        initCategory: function initCategory() {
            var _this = this;
            _this.Get(_this.api().college_category, { sort: 'time', category_id: _this.category_id }, function (res) {
                if (res.data.code == '0') {
                    _this.category = res.data.data;
                    for (var arr = 0; arr < _this.category.length; arr++) {
                        _this.tabTitles.push({ 'title': _this.category[arr].category_title });
                        _this.tabArrya.push({ 'sort': 'time', text: "按发布时间排序" });
                        _this.sort.push({ 'type': 'time' });
                        _this.showArray.push({ 'show': false });
                        _this.page.push(1);
                        _this.loading.push(false);
                        _this.empty.push(false);
                        _this.allLoadingFinish.push(false);
                    }
                    _this.tabList = [].concat(_toConsumableArray(Array(_this.tabTitles.length).keys())).map(function (i) {
                        return [];
                    });
                    if (_this.category.length == 0) {
                        _this.article_id = _this.category_id;
                        ...
                        _this.showArray.push({ 'show': false });
                        _this.sort.push({ 'type': 'time' });
                    } else _this.article_id = _this.category[0].category_id;
                    comModal.getHeight({}, function (data) {
                        _this.scrollHeight = parseInt(data.topHeight) + parseInt(_this.tabStyles.height) + 80;
                    });
                    _this.initArticle(0);
                } else promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
            });
        },
        initArticle: function initArticle(index) {
            var _this = this;
            _this.Get(_this.api().college_article_list, {
                page: _this.page[index],
                size: _this.size,
                sort: _this.sort[index].type,
                category_id: _this.article_id
            }, function (res) {
                if (res.data.code == '0') {
                    if (res.data.data.length == 0 && _this.page[index] == 1) _this.empty[index] = true;else _this.empty[index] = false;
                    ...
                    for (var arr in res.data.data) {
                        _this.list.push(res.data.data[arr]);
                    }
                    Vue.set(_this.tabList, index, _this.list);
                } else promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
            });
        }
    },
    created: function created() {
        ...
        _this.category_title = _this.GetUrlPara(weex.config.bundleUrl, 'category_title');
        _this.category_id = _this.GetUrlPara(weex.config.bundleUrl, 'category_id');
        _this.initCategory();
    }
};
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\components\\orderPop.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-48d4d698"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
    props: ['scrollHeight', 'popArray', 'sort'],
    data: function data() {
        return {
            height: height
        };
    ...
    methods: {
        ...
        sortFetch: function sortFetch(value, text) {
            //触发排序请求
            this.$emit('sortFetch', value, text);
        }
    },
    created: function created() {
        this.getWindowHeight(this);
    }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["content"]
  }, _vm._l((_vm.popArray), function(item, index) {
    return _c('cell', {
      staticClass: ["item"],
      appendAsTree: true,
      attrs: {
      ...
      on: {
        "click": function($event) {
          _vm.sortFetch(item.key, item.text)
        }
      }
    }, [(_vm.sort == item.key) ? _c('text', {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('list', {
    ...
    }, [_vm._v(_vm._s(_vm.tabArrya[index].text))]), _c('image', {
      staticClass: ["tab_image"],
      attrs: {
        "src": "http://img.abmau.com/abmau/6f44b102e9cd9b91f7c3f538d1d3e8f6.png"
      }
    })]) : _vm._e(), (_vm.showArray[index].show) ? _c('order-pop', {
      ref: "orderPop",
      refInFor: true,
      attrs: {
        "popArray": _vm.popArray,
        "sort": _vm.sort[index].type,
      ...
      on: {
        ...
        "sortFetch": _vm.sortFetch
      }
    }) : _vm._e()], 1), _c('refresher', {
      on: {
      ...
      return _c('div', {
        ...
        on: {
          "click": function($event) {
            _vm.clickArticle(item.article_id)
          }
        }
      }, [_c('image', {
        staticClass: ["article_image"],
        attrs: {
          "resize": "cover",
          "src": item.cover_thumbnail_url
        }
      }), _c('div', {
        staticClass: ["vertical_text"]
      }, [_c('text', {
        staticClass: ["title"]
      }, [_vm._v(_vm._s(item.article_title))]), _c('div', {
        staticClass: ["linear_text"]
      }, [_c('text', {
        staticClass: ["article_title"]
      }, [_vm._v(_vm._s(item.category_title))]), _c('text', {
        staticClass: ["article_time"]
      }, [_vm._v(_vm._s(item.created_at))])])]), _c('image', {
        staticClass: ["image"],
        attrs: {
          "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
        }
      })]), _c('line')], 1)
    })) : _vm._e(), (_vm.empty[index]) ? _c('cell', {
      style: {
        height: (_vm.height - _vm.tabStyles.height) + 'px'
      },
      appendAsTree: true,
    ...
    }, [_c('empty')], 1) : _vm._e(), (_vm.loading[index] && v.length > 10) ? _c('cell', {
      appendAsTree: true,
      attrs: {
        "append": "tree"
      }
    ...
    }, [(_vm.allLoadingFinish[index] = false) ? _c('text', {
      staticClass: ["loading-txt"]
    }, [_vm._v("加载中...")]) : _vm._e(), (_vm.allLoadingFinish[index] = true) ? _c('text', {
      staticClass: ["loading-over-txt"]
    }, [_vm._v("已经全部加载完毕")]) : _vm._e()])]) : _vm._e()], 1)
  }))], 1)], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
