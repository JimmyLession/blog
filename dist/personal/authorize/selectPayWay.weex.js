...
var _selectPayWay = __webpack_require__(2);

var _selectPayWay2 = _interopRequireDefault(_selectPayWay);
...
_selectPayWay2.default.el = '#root';
new Vue(_selectPayWay2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\authorize\\selectPayWay.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-50549aa5"
...
exports.default = {
    ...
    methods: {
        ...
        clickTo: function clickTo(item) {
            var _this = this;
            if (item.type == 1) {
                comModal.pay({ pay_type: 1, json: _this.pay_info.payinfo, pay_source: 'pay' }, function (event) {
                    switch (event.result) {
                        case 'success':
                            promptModule.showListener({ msg: '支付成功', type: 'success' }, function (evt) {
                                applyEvent.postMessage(_this.eventBusTag().REFRESH_AUTHORIZE);
                                _this.popBar();
                            });
                            break;
                        case 'failed':
                            promptModule.showPrompt({ msg: '支付失败', type: 'err' });
                            break;
                        case 'unknow':
                            applyEvent.postMessage(this.eventBusTag().REFRESH_AUTHORIZE);
                            break;
                    }
                });
            } else {}
        ...
        getData: function getData() {
            var _this = this;
            _this.utilPost(_this.api().user_brand + _this.apply_id + '/pay', {
                user_token: _this.mobile
            }, function (res) {
                if (res.data.code == 0) {
                    _this.pay_info = res.data.data.pay_info;
                }
            });
        }
    },
    created: function created() {
        ...
        this.apply_id = this.GetUrlPara(weex.config.bundleUrl, 'apply_id');
        this.getData();
    }
};
