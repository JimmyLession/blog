...
var _editAuthorize = __webpack_require__(2);

var _editAuthorize2 = _interopRequireDefault(_editAuthorize);
...
_editAuthorize2.default.el = '#root';
new Vue(_editAuthorize2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\authorize\\editAuthorize.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-6c25bd0d"
...
exports.default = {
    ...
    filters: {
        getPlaceholder: function getPlaceholder(value) {
            return '点击输入' + value;
        }
    },
    components: {
    ...
    methods: {
        ...
        clickTo: function clickTo(name) {},
        onInput: function onInput(e, item) {
            item.value = e.value;
        },
        apply: function apply() {
            ...
            try {
                for (var _iterator = _this.data[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var item = _step.value;

                    if (!item.value && item.is_required == 1) {
                        promptModule.showPrompt({ msg: '请输入' + item.name, type: 'prompt' });
                        isFinish = false;
                        return;
                    } else {
                        var input = {};
                        input.field_id = item.field_id;
                        if (!item.value) {
                            item.value = '';
                        }
            ...
            _this.utilPost(_this.api().brand_apply, {
                ...
                loadingModule.cancel(function (ret) {
                    ...
                    if (result == 'dismiss') {
                        if (res.data.code == 0) {
                            ...
                            if (_this.price != 0) {
                                var apply_id = res.data.data.apply_id;
                                _this.jumpTo2('personal/authorize/selectPayWay', '', { apply_id: apply_id, mobile: _this.mobile });
                                _this.popBar();
                            } else {
                                modal.toast({ message: '申请授权成功,请等待审核', duration: 5 });
                                _this.popBar();
                            }
                        } else {
    ...
    created: function created() {
        ...
        this.isIpx(this);
        var json = this.GetUrlPara(weex.config.bundleUrl, 'data');
        this.data = JSON.parse(json);
        this.price = this.GetUrlPara(weex.config.bundleUrl, 'price');
        this.mobile = this.GetUrlPara(weex.config.bundleUrl, 'mobile');
        this.brand_id = this.GetUrlPara(weex.config.bundleUrl, 'brand_id');
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_c('div', [_vm._l((_vm.data), function(item, i) {
    return _c('div', {
      key: i,
      staticClass: ["home_li"]
    }, [_c('text', {
      staticClass: ["home_li_text"]
    }, [_vm._v(_vm._s(item.name))]), _c('edit', {
      staticClass: ["authorize-edit"],
      attrs: {
        "placeholder": _vm._f("getPlaceholder")(item.name),
        "returnKeyType": "done"
      },
      on: {
        "onInput": function($event) {
          _vm.onInput($event, item)
        }
      }
    }), (item.is_required == 1) ? _c('text', {
      staticClass: ["home_li_text"]
    }, [_vm._v("*")]) : _vm._e()], 1)
  }), (_vm.price != 0) ? _c('div', {
    staticClass: ["home_li"],
    ...
    staticClass: ["authorize-edit"]
  }, [_vm._v(_vm._s(_vm.price))])]) : _vm._e(), (_vm.price != 0) ? _c('text', {
