...
var _authorizeDetails = __webpack_require__(2);

var _authorizeDetails2 = _interopRequireDefault(_authorizeDetails);
...
_authorizeDetails2.default.el = '#root';
new Vue(_authorizeDetails2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\authorize\\authorizeDetails.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-bad9e10a"
...
var applyEvent = new BroadcastChannel('authorize');
...
exports.default = {
    ...
    methods: {
        getData: function getData() {
            var _this = this;
            _this.utilGet(_this.api().user_brand + _this.brandId, {
                user_token: _this.mobile
            }, function (res) {
                if (res.data.code == 0) {
                    _this.data = res.data.data;
                    if (_this.data.images) {
                        _this.indexImage = 0;
                        _this.image = _this.data.images[_this.indexImage];
                        _this.showPrevious = false;
                        _this.showNext = _this.data.images.length > 1;
                    }
                } else {
        ...
        previousImage: function previousImage() {
            if (!this.showPrevious) {
                promptModule.showPrompt({ msg: '当前为第一张', type: 'prompt' });
                return;
            }
            this.indexImage--;
            this.image = this.data.images[this.indexImage];
            this.showPrevious = this.indexImage > 0;
            this.showNext = this.indexImage < this.data.images.length - 1;
        },
        nextImage: function nextImage() {
            if (!this.showNext) {
                promptModule.showPrompt({ msg: '当前为最后一张', type: 'prompt' });
                return;
            }
            this.indexImage++;
            this.image = this.data.images[this.indexImage];
        ...
        showImage: function showImage() {
            utilModule.showImageWatcher(this.data.images, this.indexImage);
        },
        apply: function apply() {
            this.jumpTo2('personal/authorize/editAuthorize', '', { data: JSON.stringify(this.data.fields),
                price: this.data.need_pay && this.data.need_pay == 1 ? this.data.price : 0, mobile: this.mobile, brand_id: this.brandId });
        },
        pay: function pay() {
            this.jumpTo2('personal/authorize/selectPayWay', '', { apply_id: this.data.apply_id, mobile: this.mobile });
        },
        downloadImage: function downloadImage() {
            var _this = this;
            var names = [];
            _this.data.images.forEach(function (item, index, array) {
                names.push(new Date().getTime() + index);
            });
            utilModule.download(_this.data.images, names, 'image', new Date().getTime(), function (res) {
                if (res.result == 'success') {}
            });
        },
        showCancelPop: function showCancelPop() {
            this.showCancel = true;
        },
        cancelApply: function cancelApply(type) {
            var _this = this;
            loadingModule.show({ msg: '请稍候...' });
            _this.utilPost(_this.api().user_brand + _this.data.apply_id + '/apply_refund', {
                user_token: _this.mobile
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    ...
                    if (result == 'dismiss') {
                        if (res.data.code == 0) {
                            applyEvent.postMessage(_this.eventBusTag().REFRESH_AUTHORIZE);
                            if (type == 1) {
                                _this.close();
                                _this.jumpTo2('personal/authorize/cancelAuthorize', '', {});
                            } else if (type == 2) {
                                promptModule.showPrompt({ msg: '申请退款成功,审核后支付金额会退款您的账户', type: 'prompt' });
                            }
                            _this.getData();
                        } else {
                            promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
        ...
        reApply: function reApply() {
            this.jumpTo2('personal/authorize/editAuthorize', '', { data: JSON.stringify(this.data.fields),
                price: 0, mobile: this.mobile, brand_id: this.brandId });
        },
        refund: function refund() {
            var _this = this;
            promptModule.show({ title: '温馨提示',
                message: '您确定要申请退款吗？申请退款后，再次申请授权将需要重新支付',
                cancel: '取消',
    ...
    created: function created() {
        ...
        this.brandId = this.GetUrlPara(weex.config.bundleUrl, 'brandId');
        this.getData();
        var _this = this;
        applyEvent.onmessage = function (event) {
            if (!event.data) {
                return;
            }
            switch (event.data) {
                case _this.eventBusTag().REFRESH_AUTHORIZE:
                    _this.getData();
                    break;
            }
        };
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["home_li_text"]
  }, [_vm._v("授权书名称")]), _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v(_vm._s(_vm.data.name))])])]), _c('div', [(_vm.data.status || _vm.data.status == 0 || _vm.data.refund_status || _vm.data.refund_status == 0) ? _c('div', {
    staticClass: ["home_li"]
  }, [_c('text', {
    staticClass: ["home_li_text"]
  }, [_vm._v("状态")]), (_vm.data.refund_status) ? _c('div', [(_vm.data.refund_status == 1 || _vm.data.refund_status == 3) ? _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v("退款中")]) : _vm._e(), (_vm.data.refund_status == 2) ? _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v("退款失败")]) : _vm._e(), (_vm.data.refund_status == 4) ? _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v("已退款")]) : _vm._e()]) : _c('div', [(_vm.data.status == 0) ? _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v("待支付")]) : _vm._e(), (_vm.data.status == 1) ? _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v("审核中")]) : _vm._e(), (_vm.data.status == 2) ? _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v("已拒绝")]) : _vm._e(), (_vm.data.status == 3) ? _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v("已授权")]) : _vm._e()])]) : _vm._e()]), _c('div', [_c('div', {
    staticClass: ["home_li"]
  }, [_c('text', {
    staticClass: ["home_li_text"]
  }, [_vm._v("授权期限")]), _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v(_vm._s(_vm.data.expiry_month) + "个月")])])]), _c('div', [(_vm.data.status == 3) ? _c('div', {
    staticClass: ["home_li"]
  }, [_c('text', {
    staticClass: ["home_li_text"]
  }, [_vm._v("剩余时间")]), _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v(_vm._s(_vm.data.remaining_time) + "天")])]) : _vm._e()]), _c('div', [(_vm.data.need_pay == 1) ? _c('div', {
    staticClass: ["home_li"]
  }, [_c('text', {
    staticClass: ["home_li_text"]
  }, [_vm._v("定金金额")]), _c('text', {
    staticClass: ["home_li_status"]
  }, [_vm._v(_vm._s(_vm.data.price))])]) : _vm._e()]), (_vm.data.images && _vm.data.images.length > 0) ? _c('div', [_c('div', {
    style: {
      paddingLeft: '50px'
    }
  }, [_c('text', {
    ...
    staticClass: ["previous-text"],
    on: {
      "click": function($event) {
        _vm.previousImage()
      }
    }
  }, [_vm._v("上一张")]), (_vm.image) ? _c('image', {
    staticClass: ["img"],
    attrs: {
      "src": _vm.image
    },
  ...
  }, [_vm._v("下一张")])]), (_vm.data.status && _vm.data.status == 3 && !_vm.data.refund_status) ? _c('div', {
    style: {
      justifyContent: 'center',
      width: '750px',
      alignItems: 'center'
    ...
    staticClass: ["download-authorize"],
    on: {
      "click": function($event) {
        _vm.downloadImage()
      }
  ...
  })]) : _vm._e()]), (_vm.data.status && (_vm.data.status == 3 || _vm.data.status == 1) && !_vm.data.refund_status) ? _c('div', {
    style: {
      justifyContent: 'center',
      width: '750px',
      alignItems: 'center'
  ...
  }, [_vm._v("申请取消授权")])]) : _vm._e(), (_vm.data.status == 2) ? _c('text', {
    staticClass: ["reject-text"]
  }, [_vm._v("拒绝理由:" + _vm._s(_vm.data.reject_reason))]) : _vm._e(), (!_vm.data.status && _vm.data.status != 0) ? _c('text', {
  ...
  }, [_vm._v("立即申请")]) : _vm._e(), (_vm.data.status == 0 && !_vm.data.refund_status) ? _c('text', {
    staticClass: ["apply-btn"],
    on: {
      "click": function($event) {
        _vm.pay()
  ...
  }, [_vm._v("立即支付")]) : _vm._e(), ((_vm.data.status == 0 || _vm.data.status == 2) && (_vm.data.refund_status == 0 || _vm.data.refund_status == 4 || _vm.data.refund_status == 2)) ? _c('text', {
    staticClass: ["apply-btn"],
    on: {
      "click": function($event) {
        _vm.reApply()
  ...
  }, [_vm._v("重新申请授权")]) : _vm._e(), (_vm.data.status == 2 && _vm.data.need_pay == 1 && (_vm.data.refund_status == 2 || _vm.data.refund_status == 0)) ? _c('text', {
    staticClass: ["apply-btn"],
    on: {
      "click": function($event) {
        _vm.refund()
    ...
    staticClass: ["cancel-pop-content"]
  }, [_vm._v("取消授权后，定金会原路退还支付账户 是否确定取消授权")])]), _c('text', {
    staticClass: ["cancel-pop-apply"],
    on: {
      "click": function($event) {
        _vm.cancelApply(1)
      }
