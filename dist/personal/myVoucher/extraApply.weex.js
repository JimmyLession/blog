...
var _extraApply = __webpack_require__(2);

var _extraApply2 = _interopRequireDefault(_extraApply);
...
_extraApply2.default.el = '#root';
new Vue(_extraApply2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\myVoucher\\extraApply.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-a6c2a9f2"
...
var _wxButton = __webpack_require__(9);
...
var picker = weex.requireModule('pickerSheet'); //picker组件
var stream = weex.requireModule('stream'); //stream组件
var modal = weex.requireModule('modal'); //modal组件
var comModal = weex.requireModule('ComModule'); //ComModule组件
var token = "";
var height = 0;
var bottom = 0;
exports.default = {
    filters: {
        maxNumFilters: function maxNumFilters(value) {
            return '最大申请数量' + value;
        }
    },
    components: { title: _titleBar2.default, WxButton: _wxButton2.default },
    data: function data() {
        //data数据
        return {
            version: false,
            ...
            bottom: bottom,
            index: 0,
            applyTypeValue: '选择类型', //选择类型
            applyNumber: '', //申请数量
            applyReason: '', //申请理由
            applyWeixin: '', //申请的联系方式(微信)
            couponId: '', //申请的一个couponId
            maxNum: '', //最大申请数量
            data: { //返回的数据
                items: [],
                coupon_id: [],
    ...
    computed: { //计算属性  作为计算切换体验券类型
        applyTypeValue: function applyTypeValue() {
    ...
    methods: {
        onInputApplyNumber: function onInputApplyNumber(e) {
            this.applyNumber = e.value;
            this.isParam = this.isValueParam(this, [this.applyNumber, this.applyReason, this.applyWeixin]);
        },
        onInputApplyWeixin: function onInputApplyWeixin(e) {
            this.applyWeixin = e.value;
            this.isParam = this.isValueParam(this, [this.applyNumber, this.applyReason, this.applyWeixin]);
        },
        onInputApplyReason: function onInputApplyReason(event) {
            this.applyReason = event.value;
            this.isParam = this.isValueParam(this, [this.applyNumber, this.applyReason, this.applyWeixin]);
        },
        ...
        chooseType: function chooseType() {
            ...
            picker.pick({
                ...
                if (event.result === 'success') {
                    _this2.applyTypeValue = _this2.data.items[event.data];
                    _this2.index = event.data;
                    _this2.couponId = _this2.data.coupon_id[event.data];
                    _this2.maxNum = _this2.data.max_num[event.data];
                }
            });
        },
        applyRecord: function applyRecord() {
            //点击进入——>申请记录
            this.jumpTo2('personal/myVoucher/applyRecord', '申请记录', {});
        },
        toParams: function toParams(obj) {
            //当post提交申请的时候，需要把参数转换为a=1&b=2...这种参数形式
            var param = "";
            for (var name in obj) {
                if (typeof obj[name] != 'function') {
                    param += "&" + name + "=" + encodeURI(obj[name]);
        ...
        applySure: function applySure() {
            //点击事件——>确认申请，并且提交给后台
            if (this.applyTypeValue == '选择类型') {
                modal.toast({
                    message: '请选择申请券类型',
                    duration: 3
                });
                return;
            } else if (this.applyNumber.length < 1 || !(this.applyNumber % 1 === 0) || this.applyNumber.replace(/(^\s*)|(\s*$)/g, "") == "") {
                modal.toast({
                    message: '请输入申请数量',
                    duration: 3
                });
                return;
            } else if (this.applyReason.length < 1 || this.applyReason.replace(/(^\s*)|(\s*$)/g, "") == "") {
                modal.toast({
                    message: '请输入申请原因',
                    duration: 3
                });
                return;
            } else if (this.applyWeixin.length < 1 || this.applyWeixin.replace(/(^\s*)|(\s*$)/g, "") == "") {
                modal.toast({
                    message: '请输入申请联系方式',
                    duration: 3
                });
            ...
            var _this = this;
            _this.Post(_this.api().apply_coupon, {
                coupon_id: _this.couponId,
                apply_num: _this.applyNumber,
                apply_reason: _this.applyReason,
                contact_way: _this.applyWeixin
            }, function (res) {
                if (res.data.code != '0') {
                    //申请失败
                    modal.toast({ message: res.data.msg, duration: 5 });
                } else {
                    //申请成功
                    modal.toast({ message: res.data.msg, duration: 5 });
                    _this.popBar();
    ...
    created: function created() {
        //当进入额外申请页面的时候请求部分数据(例如：最大申请数量、申请券类型等)
        var _this = this;
        _this.version = _this.compareVersion(weex.config.env.appVersion, '3.0.5');
        _this.getTopHeight(_this);
        _this.isIpx(_this);
        _this.Get(_this.api().coupon_limit, {
            ...
            if (res.data.code != '0') {
                ...
                for (var i = 0; i < res.data.data.length; i++) {
                    _this.data.items.push(res.data.data[i].title);
                    _this.data.coupon_id.push(res.data.data[i].coupon_id);
                    _this.data.max_num.push(res.data.data[i].max_num);
                }
            }
        });
    }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [(_vm.version) ? _c('title', {
    attrs: {
      "title": "超额申请",
      "rightText": "申请记录"
    },
    ...
    staticClass: ["apply-item", "apply-type"],
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('text', {
    staticClass: ["apply-type-text", "text"]
  }, [_vm._v("申请体验券类型")]), _c('div', {
    staticClass: ["right"]
  }, [_c('text', {
    staticClass: ["apply-type-choose"],
    attrs: {
  ...
  }, [_vm._v(_vm._s(_vm.applyTypeValue))]), _c('image', {
    staticClass: ["item_image"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
  })])]), _c('cell', {
    staticClass: ["apply-item", "apply-number"],
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('text', {
    staticClass: ["apply-number-text", "text"]
  }, [_vm._v("申请数量")]), _c('edit', {
    ref: "input1",
    staticClass: ["input"],
    attrs: {
      "type": "number",
      "placeholder": _vm._f("maxNumFilters")(_vm.maxNum)
    },
    on: {
      "onInput": _vm.onInputApplyNumber
    }
  })], 1), _c('cell', {
    staticClass: ["apply-item", "apply-reason"],
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('text', {
    staticClass: ["apply-reason-text", "text"]
  }, [_vm._v("申请原因")]), _c('textarea', {
    ...
    on: {
      "input": _vm.onInputApplyReason
    }
  })]), _c('cell', {
    staticClass: ["apply-item", "apply-weixin"],
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('text', {
    staticClass: ["apply-weixin-text", "text"]
  }, [_vm._v("联系方式(微信号)")]), _c('edit', {
    ref: "input1",
    staticClass: ["input"],
    attrs: {
      "type": "text",
    ...
    on: {
      "onInput": _vm.onInputApplyWeixin
    }
  })], 1), _c('cell', {
    appendAsTree: true,
    attrs: {
