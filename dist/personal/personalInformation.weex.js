...
var _personalInformation = __webpack_require__(2);
...
_personalInformation2.default.el = '#root';
new Vue(_personalInformation2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\personal\\personalInformation.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-2e6c9be6"
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            qr_code_url: '', //二维码地址
            showCodeDot: false,
            showAddressDot: false,
            qr_code_show: '上传微信二维码',
            bottom: bottom
        };
    },
    ...
    methods: {
        ...
        requestToGetUserInfor: function requestToGetUserInfor() {
            ...
            _this.Get(_this.api().get_userinfo, {}, function (res) {
                if (res.data.code != '0') {
                    ...
                    _this.qr_code_url = res.data.data.qr_code_url;
                    //地区判断
                    ...
                    //微信二维码判断
                    if (res.data.data.qr_code_url == '') {
                        _this.qr_code_show = '上传微信二维码';
                    } else {
                        _this.qr_code_show = '修改微信二维码';
                    }
                    storage.getItem('abm_wx_qr_' + _this.userInfo.id, function (ret) {
                        _this.showCodeDot = ret.data != _this.userInfo.id && _this.qr_code_url == '';
                    });
                    storage.getItem('abm_user_address_' + _this.userInfo.id, function (ret) {
                        _this.showAddressDot = ret.data != _this.userInfo.id && _this.address == '未填写';
                    });
                }
            });
        },
        ...
        //操作个人头像
        clickPersonalPhoto: function clickPersonalPhoto() {
            ...
            var _this = this; // 0：拍照  1：从手机相册选择 3：保存图片
            var titlesDict = { 'caram': '拍照', 'photoFile': '从手机相册选择', 'save': '保存图片' };
            comModal.uploadPhoto({ 'titlesDict': titlesDict, 'save_imageUrl': _this.headimg }, function (ret) {
                _this.Post(_this.api().personal_updateImage, { 'headimg': ret.url }, function (res) {
                    if (res.data.code == '0') {
                        _this.headimg = ret.url;
                        _this.userInfo.headimg = ret.url;
                        promptModule.showPrompt({ msg: '更新头像成功', type: 'success' });
                    } else {
                        promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                    }
                });
        ...
        //跳转-修改密码页面
        jumpToChangePassword: function jumpToChangePassword() {
            this.jumpTo2('setting/changePsd', '', { mobile: this.userInfo.mobile });
        },

        //跳转-微信名片页面
        jumpToWeiXinCard: function jumpToWeiXinCard() {
            var _this = this;
            if (!_this.userInfo.id) {
                return;
            }
            storage.setItem('abm_wx_qr_' + _this.userInfo.id, _this.userInfo.id, function (res) {
                if (res.result == 'success') {
                    event.postMessage(_this.eventBusTag().REFRESH_MINE_INFO_DOT);
                    _this.showCodeDot = false;
                }
            });
            if (weex.config.env.platform.toLowerCase() === 'ios' && !this.compareVersion(weex.config.env.appVersion, '3.4.2')) {
                promptModule.showPrompt({ msg: "请到AppStore更新最新版本", type: 'prompt' });
            } else this.jumpTo2('personal/personalInfoCode', '', { qr_code_url: this.userInfo.qr_code_url });
        },
        ...
        chooseAddress: function chooseAddress() {
            ...
            storage.setItem('abm_user_address_' + _this.userInfo.id, _this.userInfo.id, function (res) {
                if (res.result == 'success') {
                    event.postMessage(_this.eventBusTag().REFRESH_MINE_INFO_DOT);
                    _this.showAddressDot = false;
                }
            });
            var _this = this;
            var province;
        ...
        //展示分享页面
        showABMShareView: function showABMShareView() {
            if (this.userInfo.level_id == 1) {
                promptModule.showPrompt({ msg: '会员无法邀请好友', type: 'prompt' });
            } else {
                if (!this.userInfo) return;
                if (!this.userInfo.share) return;
                var shareData = this.userInfo.share;
                var invite_code = this.userInfo.invite_code;
                if (!shareData.title) return;
                utilModule.getABMShareView({
                    ...
                    'invite_qr': JSON.stringify({ headimg: this.userInfo.headimg, name: this.userInfo.invite_share_title, mobile: this.userInfo.invite_share_mobile, qr: shareData.qrcode, des: shareData.qrcode_des })
                });
            }
        }
    },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    on: {
      "click": function($event) {
        _vm.clickPersonalPhoto()
      }
    }
  }, [_c('text', {
    staticClass: ["photo_tittle_name"]
  }, [_vm._v("个人头像")]), _c('div', {
    staticClass: ["right_info"]
  }, [(_vm.headimg != '') ? _c('image', {
    staticClass: ["photo_img"],
    attrs: {
      "resize": "cover",
      "src": _vm.headimg
    }
  }) : _vm._e(), (_vm.headimg == '') ? _c('image', {
    staticClass: ["photo_img"],
    attrs: {
      "resize": "cover",
    ...
    staticClass: ["form_item_tittle"]
  }, [_vm._v("登录手机号")]), _c('text', {
    staticClass: ["mobile"]
  }, [_vm._v(_vm._s(_vm.userInfo.mobile))])]), _c('line'), _c('div', {
    staticClass: ["form_item"],
    on: {
      "click": function($event) {
        _vm.jumpToChangePassword()
      }
    }
  }, [_c('text', {
    staticClass: ["form_item_tittle"]
  }, [_vm._v("修改密码")]), _c('image', {
    staticClass: ["form_item_more"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
  })]), _c('line'), _c('div', {
  ...
  }, [_vm._v(_vm._s(_vm.qr_code_show))]), (_vm.showCodeDot) ? _c('div', {
    staticClass: ["red_dot"]
  }) : _vm._e(), _c('image', {
    staticClass: ["form_item_more"],
    attrs: {
    ...
    staticClass: ["right_content"]
  }, [_vm._v(_vm._s(_vm.address))]), (_vm.showAddressDot) ? _c('div', {
    staticClass: ["red_dot"]
  }) : _vm._e(), _c('image', {
    staticClass: ["form_item_more"],
    attrs: {
  ...
  })])]), (_vm.userInfo.level_id > 1) ? _c('div', {
    staticClass: ["bottom_item"]
  }, [_c('div', {
    staticClass: ["form_item"],
    on: {
      "click": function($event) {
        _vm.showABMShareView()
      }
    }
  }, [_c('text', {
    staticClass: ["form_item_tittle"]
    ...
    staticClass: ["right_content"]
  }, [_vm._v(_vm._s(_vm.userInfo.invite_code))]), _c('image', {
    staticClass: ["form_item_more"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
