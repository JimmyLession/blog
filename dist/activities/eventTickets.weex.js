...
var _eventTickets = __webpack_require__(3);

var _eventTickets2 = _interopRequireDefault(_eventTickets);
...
_eventTickets2.default.el = '#root';
new Vue(_eventTickets2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\activities\\eventTickets.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-02c478a2"
...
exports.default = {
    components: { title: _titleBar2.default, refresher: _refresh2.default, WxcMask: _wxcMask2.default, pop: _WxcMask3.default, empty: _empty2.default },
    data: function data() {
        return {
            height: height,
            bottom: bottom,
    ...
    methods: {
        clickArea: function clickArea() {
            this.jumpTo2('region/region', '选择所在国家及区号', { from: 'ticket' });
        },
        clickPresented: function clickPresented(index, id, is_own, own_mobile, is_exchange) {
            this.index = index;
            this.id = id;
            this.is_own = is_own;
            if (is_own == 0) this.to_mobile = own_mobile;else if (is_own == 1) this.to_mobile = '';
            if (is_own == 0) this.showRebate();else {
                if (is_exchange == 0) {
                    this.areaCode = '+86';
                    this.showPresented = true;
                }
            }
        },
        showRebate: function showRebate() {
            var self = this;
            promptModule.show({ title: '温馨提示',
                message: '返回门票给赠送人，您将无权使用该门票',
                cancel: '取消',
        ...
        sendRebate: function sendRebate() {
            var self = this;
            loadingModule.show({ msg: '返回门票...' });
            self.utilPost(_jsMd2.default, self.api().Rollback_Presented_ticket, {
                user_token: self.mobile,
                id: self.id
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    if (res.data.code == 0) {
                        self.items.splice(self.index, 1);
                        if (self.items.length == 0) self.emptyShow = true;else self.emptyShow = false;
                    } else {
        ...
        onMobileInput: function onMobileInput(event) {
            this.to_mobile = event.value;
        },
        clickConfirm: function clickConfirm() {
            utilModule.hideKeyBorad();
            var self = this;
            self.showPresented = false;
            var areaCodeMobile = self.to_mobile;
            if (self.areaCode != '+86') areaCodeMobile = self.areaCode + self.to_mobile;
            if (areaCodeMobile.startsWith('+86')) areaCodeMobile = areaCodeMobile.replace('+86', '');
            if (areaCodeMobile.startsWith("+")) areaCodeMobile = areaCodeMobile.replace("+", "%2B");
            areaCodeMobile = areaCodeMobile.replace(/\s+/g, "");
            loadingModule.show({ msg: '正在赠送...' });
            self.utilPost(_jsMd2.default, self.api().Presented_ticket, {
                user_token: self.mobile,
                id: self.id,
                to_mobile: areaCodeMobile
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    if (res.data.code == 0) {
                        self.getTickets();
        ...
        clickCancel: function clickCancel() {
            utilModule.hideKeyBorad();
            this.showPresented = false;
        },
        loadingDown: function loadingDown() {
            this.getTickets();
        },
        clickItem: function clickItem(status, url, is_exchange, is_own, is_master) {
            if (status != 1 || is_exchange == 1) return;
            this.qrUrl = url;
            this.showQr = true;
        },
        ...
        getTickets: function getTickets() {
            var self = this;
            self.utilGet(_jsMd2.default, self.api().Tickets_Lv, {
                user_token: self.mobile
            }, function (res) {
                if (res.data.code == 0) {
                    self.items = res.data.data.items;
                    if (res.data.data.total == 0) self.emptyShow = true;else self.emptyShow = false;
                } else {
                    promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                }
            });
    ...
    created: function created() {
        ...
        _this.getTickets();
        region.onmessage = function (event) {
            if (!event.data) {
                return;
            }
            _this.areaCode = event.data;
        };
        if (weex.config.env.platform.toLowerCase() === 'android') {
            ...
            globalEvent.addEventListener("geolocation", function (e) {
                if (_this.showQr) {
                    _this.showQr = false;
                    _this.getTickets();
                } else if (_this.showPresented) {
                    _this.showPresented = false;
                } else {
                    _this.popBar();
                }
            });
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('cell', {
    ...
    }) : _vm._e(), (item.is_master == 1) ? _c('div', {
      staticClass: ["tickets_item"],
      on: {
        "click": function($event) {
          _vm.clickItem(item.status, item.qrcode_url, item.is_exchange, item.is_own, item.is_master)
        }
      }
    }, [(item.bg) ? _c('image', {
      staticClass: ["tickets_bg"],
      attrs: {
        "src": item.bg
      }
    }) : _c('image', {
      staticClass: ["tickets_bg"],
      attrs: {
        "src": "http://img.abmau.com/abmau/333cb5757c4ea94d552b813dd50f962a.png"
      }
    }), _c('div', {
      staticClass: ["tickets_left"]
    }, [_c('div', [_c('text', {
      staticClass: ["tickets_title"]
    }, [_vm._v(_vm._s(item.subtitle))]), _c('text', {
      staticClass: ["tickets_des"]
    }, [_vm._v(_vm._s(item.title))])]), _c('div', [(item.is_exchange == 1) ? _c('text', {
      staticClass: ["tickets-son-from"],
      style: {
        color: '#DDC196'
      }
    }, [_vm._v("已赠送：" + _vm._s(item.receiver_mobile))]) : _vm._e(), (item.is_own == 0) ? _c('text', {
      staticClass: ["tickets-son-from"],
      style: {
        color: '#DDC196'
      }
    }, [_vm._v("来自" + _vm._s(item.own_mobile) + "的赠送")]) : _vm._e(), _c('div', {
      style: {
        flexDirection: 'row',
        marginLeft: '55px',
        alignItems: 'flex-end'
      ...
      staticClass: ["tickets_time"]
    }, [_vm._v(_vm._s(item.showtime))]), (item.location != '') ? _c('text', {
      staticClass: ["tickets_time"],
      style: {
        fontWeight: 'bold',
        marginLeft: '5px',
    ...
    }, [_vm._v("(" + _vm._s(item.location) + ")")]) : _vm._e()])])]), _c('div', {
      staticClass: ["tickets_right"]
    }, [_c('text', {
      staticClass: ["tickets-right-title"]
    }, [_vm._v("入场门票")]), _c('div', {
      staticStyle: {
        width: "136px",
        height: "136px",
      ...
      staticClass: ["tickets-image-shade"]
    }), _c('div', {
      staticClass: ["tickets-image-see"]
    }, [(item.status == 1 && item.is_exchange == 0) ? _c('text', {
      staticStyle: {
        color: "#fff",
        fontSize: "22px"
      }
    }, [_vm._v("点击查看")]) : _vm._e()])]), (item.is_own == 0 && item.status == 1) ? _c('div', {
      staticClass: ["tickets-right-title-send"],
      style: {
        borderColor: '#DDC196'
      },
      on: {
        "click": function($event) {
          _vm.clickPresented(key, item.id, item.is_own, item.own_mobile, item.is_exchange)
        }
      }
    }, [_c('text', {
      staticStyle: {
    ...
    }, [_vm._v("返回门票")])]) : _vm._e(), (item.is_own == 1 && item.is_exchange == 0 && item.status == 1) ? _c('div', {
      staticClass: ["tickets-right-title-send"],
      style: {
        borderColor: '#DDC196'
      },
    ...
    }, [_vm._v("赠送他人")])]) : _vm._e(), (item.is_own == 1 && item.is_exchange == 1) ? _c('div', {
      staticClass: ["tickets-right-title-send"],
      style: {
        borderColor: '#DDC196'
      },
    ...
    }, [_vm._v("已赠送")])]) : _vm._e()]), (item.status == 2) ? _c('div', {
      staticClass: ["tickets_status"]
    }, [_c('image', {
      staticClass: ["tickets_status-image"],
      attrs: {
        "src": "http://img.abmau.com/abmau/6b8815fe3022ff9abc42d3a2c1d6777c.png"
      }
    })]) : _vm._e(), (item.status == 3) ? _c('div', {
      staticClass: ["tickets_status"]
    }, [_c('image', {
      staticClass: ["tickets_status-image"],
      attrs: {
        "src": "http://img.abmau.com/abmau/d4696b2f88ec983931fe9dbabc79efea.png"
      }
    })]) : _vm._e()]) : _vm._e(), (item.is_master == 0) ? _c('div', {
      staticClass: ["tickets_item"],
      on: {
        "click": function($event) {
          _vm.clickItem(item.status, item.qrcode_url, item.is_exchange, item.is_own, item.is_master)
      ...
      staticClass: ["tickets_bg"],
      attrs: {
        "src": "http://img.abmau.com/abmau/1e538636ef1e0979ef488141e6e1729c.png"
      }
    }), _c('div', {
      staticClass: ["tickets_left"]
    }, [_c('div', [_c('text', {
      staticClass: ["tickets_title-son"]
    }, [_vm._v(_vm._s(item.subtitle))]), _c('text', {
      staticClass: ["tickets_des-son"]
    }, [_vm._v(_vm._s(item.title))])]), _c('div', [(item.is_exchange == 1) ? _c('text', {
      staticClass: ["tickets-son-from"]
    }, [_vm._v("已赠送：" + _vm._s(item.receiver_mobile))]) : _vm._e(), (item.is_own == 0) ? _c('text', {
      staticClass: ["tickets-son-from"]
    ...
    }, [_vm._v("(" + _vm._s(item.location) + ")")]) : _vm._e()])])]), (item.status == 2) ? _c('div', {
      staticClass: ["tickets_status"]
    }, [_c('image', {
      staticClass: ["tickets_status-image"],
      attrs: {
      ...
      staticClass: ["tickets-right-title-son"]
    }, [_vm._v("入场门票·副票")]), _c('div', {
      staticStyle: {
        width: "136px",
        height: "136px",
        justifyContent: "center",
    ...
    }), (item.is_own == 1) ? _c('div', {
      staticClass: ["tickets-image-shade"]
    }) : _vm._e(), (item.is_own == 0) ? _c('div', {
      staticClass: ["tickets-image-shade"]
    }, [(item.status == 1) ? _c('text', {
      staticStyle: {
        color: "#fff",
        fontSize: "22px"
      }
    }, [_vm._v("点击查看")]) : _vm._e()]) : _vm._e()]), (item.is_own == 0 && item.status == 1) ? _c('div', {
      staticClass: ["tickets-right-title-send"],
      on: {
        "click": function($event) {
          _vm.clickPresented(key, item.id, item.is_own, item.own_mobile, item.is_exchange)
    ...
    }, [_vm._v("已赠送")])]) : _vm._e()])]) : _vm._e(), _c('div', {
      staticClass: ["line"]
    })])
    ...
    on: {
      "click": _vm.wxcOverlayBodyClicked
    }
  }, [_c('div', {
    staticClass: ["guide-image-parent"]
  }, [_c('image', {
    ...
    attrs: {
      ...
      "showClose": false,
      "show": _vm.showPresented
    }
  }, [_c('div', {
    staticClass: ["mask-Presented"]
  }, [_c('text', {
    staticClass: ["mask-Presented-title"]
  }, [_vm._v("赠送门票")]), _c('div', {
    staticStyle: {
      width: "540px",
      height: "1px",
    ...
    staticClass: ["mask-Presented-mobile"]
  }, [_c('div', {
    staticClass: ["presented-edit-code"],
    on: {
      "click": function($event) {
        _vm.clickArea()
      }
  ...
  }, [_vm._v(_vm._s(_vm.areaCode))]), _c('image', {
    staticClass: ["image"],
    attrs: {
    ...
    ref: "mobile",
    staticClass: ["presented-edit"],
    attrs: {
      "type": "number",
      "placeholder": "被赠送人手机号"
    },
    on: {
      "onInput": _vm.onMobileInput
    }
  })], 1), _c('text', {
    staticStyle: {
      marginTop: "20px",
