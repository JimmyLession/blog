...
var _elevenBrandRank = __webpack_require__(4);

var _elevenBrandRank2 = _interopRequireDefault(_elevenBrandRank);
...
_elevenBrandRank2.default.el = '#root';
new Vue(_elevenBrandRank2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\activities\\elevenBrandRank.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-89c7b880"
...
var Steve = new BroadcastChannel("remarkMessage");

var height = 0;
exports.default = {
    components: { WxcTabPage: _wxcTabPage2.default, line: _line2.default, titleBar: _titleBar2.default, refresher: _refresh2.default, empty: _empty2.default },
    data: function data() {
        return {
            strTitle: '双十一各品牌排行榜', //导航标题
            height: 1334, //页面需要一个高度，在这里先暂时定义一个高度，初始化的时候会加载一个
            // limit : 20,//当前请求数据的条数
            // offset:[],//数据请求的页数
            lastTimestamp: 0,
            bigImgURL: 'http://img.abmau.com/abmau/b231f94e91661a38f16591cb62c088cd.png', //顶部大图
            webTittle: '八大品牌方打Call', //顶部大图-跳转web的title
            webUrl: 'https://wap.abmau.com//app/recommend_list?article_id=331&token=47f5df60b845caf741b919efd8bdd97a', //顶部大图-跳转web的url
            tabList: [], //数据列表，是一个二维数组
            tabTitles: [], //tab标题
            tabStyles: { //tab标题的样式
                bgColor: '#FFFFFF',
                titleColor: '#333333',
    ...
    methods: {
        ...
        clickToCheckRankDetail: function clickToCheckRankDetail(event) {
            ...
            this.jumpTo2('web/webMovieView', '', { url: encodeURIComponent(this.webUrl), title: this.webTittle });
        },
        loadingDown: function loadingDown() {
            ...
            _this.data = [];
            _this.startLoading(_this.index, _this.id);
        },
        loadTabTitle: function loadTabTitle() {
            //请求品牌名称
            var _this = this;
            _this.Get(_this.api().getBrandByRank, {}, function (res) {
                if (res.data.code == 0) {
                    //请求数据成功
                    for (var i = 0; i < res.data.data.length; i++) {
                        _this.tabTitles.push({ 'title': res.data.data[i].name });
                        _this.brandId.push(res.data.data[i].id);
                        _this.id = _this.brandId[0];
                        _this.empty.push(false);
                        _this.tabList = [].concat(_toConsumableArray(Array(_this.tabTitles.length).keys())).map(function (i) {
                            return [];
                        });
                        _this.startLoading(0, _this.id);
                    }
                } else {
                    //请求数据失败
        ...
        wxcTabPageCurrentTabSelected: function wxcTabPageCurrentTabSelected(e) {
            //tab标签的切换数据加载
            var _this = this;
            _this.id = _this.brandId[e.page];
            _this.data = [];
            _this.startLoading(e.page, _this.id);
        },
        startLoading: function startLoading(index, id) {
            //初始化加载数据
            var _this = this;
            _this.isRequesting = true;
            _this.Get(_this.api().brandPaymentRankList, {
                brand_id: id
            }, function (res) {
                if (res.data.code != 0) {
                    //请求数据失败
                    ...
                    if (res.data.data.list.length == 0) {
                        _this.empty[index] = true;
                    } else {
                        _this.empty[index] = false;
                    }
                    for (var arr in res.data.data.list) {
                        _this.data.push(res.data.data.list[arr]);
                    }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title-bar', {
    ...
    return _c('list', {
      ...
      style: {
        height: (_vm.height - _vm.tabStyles.height - 360) + 'px'
      },
      attrs: {
        "loadmoreoffset": "10"
      },
      ...
      staticClass: ["head-title-left"]
    }, [_vm._v("销售排行榜")]), _c('text', {
      staticClass: ["head-title-right"]
    }, [_vm._v("金额")])])]), _c('refresher', {
      on: {
      ...
      return _c('cell', {
        ...
        staticClass: ["order_item-left"]
      }, [_c('text', {
        staticClass: ["order-rank"]
      }, [_vm._v(_vm._s(item.rank))]), (item.head_image != '') ? _c('image', {
        staticClass: ["head-img"],
        attrs: {
          "resize": "cover",
          "src": item.head_image
        }
      }) : _vm._e(), (item.head_image == '') ? _c('image', {
        staticClass: ["head-img"],
        attrs: {
          "resize": "cover",
        ...
        staticClass: ["order-user-name"]
      }, [_vm._v(_vm._s(item.user_name))])]), _c('text', {
        staticClass: ["order_item-right"]
      }, [_vm._v("¥" + _vm._s(item.total))])])
    }), (_vm.empty[index]) ? _c('cell', {
      style: {
        height: (_vm.height - _vm.tabStyles.height - 360 - 150) + 'px'
      },
      appendAsTree: true,
      attrs: {
        "append": "tree"
