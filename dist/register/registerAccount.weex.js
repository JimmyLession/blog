...
var _registerAccount = __webpack_require__(2);
...
_registerAccount2.default.el = '#root';
new Vue(_registerAccount2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\register\\registerAccount.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-cf6e60a4"
...
var height = 0;
exports.default = {
    components: { line: _line2.default, WxButton: _wxButton2.default },
    data: function data() {
        return {
            is_show: false,
            area_code: '+86',
    ...
    methods: {
        onInputNickName: function onInputNickName(e) {
            this.applyNickName = e.value;
            this.isParam = this.isValueParam(this, [this.applyNickName, this.applyMobile, this.applyMobileCode, this.applyPsw]);
        },
        onInputMobile: function onInputMobile(e) {
            this.applyMobile = e.value;
            this.isParam = this.isValueParam(this, [this.applyNickName, this.applyMobile, this.applyMobileCode, this.applyPsw]);
        },
        onInputMobileCode: function onInputMobileCode(e) {
            this.applyMobileCode = e.value;
            this.isParam = this.isValueParam(this, [this.applyNickName, this.applyMobile, this.applyMobileCode, this.applyPsw]);
        },
        onInputPsw: function onInputPsw(e) {
            this.applyPsw = e.value;
            this.isParam = this.isValueParam(this, [this.applyNickName, this.applyMobile, this.applyMobileCode, this.applyPsw]);
        },
        viewstart: function viewstart() {
            var _this = this;
            storage.getItem('area_code', function (event) {
                _this.area_code = event.data;
            });
            storage.getItem('send_voice', function (event) {
                if (event.data == 'true') {
                    _this.is_show = true;
                } else {
                    _this.is_show = false;
        ...
        alterCode: function alterCode() {
            this.jumpTo2('other/codeHelp', '帮助', { mobile: this.area_code + this.applyMobile, type: this.applyMobile });
        },
        finish: function finish() {
            this.finishAnim();
        },
        clickAgreement: function clickAgreement(title, url) {
            this.jumpTo2('web/webView', title, { title: title, url: encodeURIComponent(url) });
        },
        clickArea: function clickArea() {
            this.jumpTo2('region/region', '选择所在国家及区号', {});
        },
        sendCode: function sendCode(isClick) {
            var _this = this;
            if (!isClick) return;
            if (_this.applyMobile == "") {
                promptModule.showPrompt({ msg: '请输入手机号码', type: 'prompt' });
                return;
            }
            var areaCode = _this.area_code.replace('+', '%2B');
            _this.Get(_this.api().get_code, {
                mobile: areaCode + _this.applyMobile
            }, function (res) {
                if (res.data.code == '0') {
                    _this.isClick = false;
                    _this.countDown(60, function (t) {
                        _this.codeName = t + '秒后重发';
                    }, function () {
                        _this.isClick = true;
                        _this.codeName = '发送验证码';
                    });
                    promptModule.showPrompt({ msg: '发送成功', type: 'success' });
                } else {
                    _this.isClick = true;
        ...
        clickNext: function clickNext() {
            var _this = this;
            var areaCode = _this.area_code.replace('+', '%2B');
            if (_this.applyNickName == "") {
                promptModule.showPrompt({ msg: '请设置您的昵称', type: 'prompt' });
                return;
            }
            if (_this.applyMobile == "") {
                promptModule.showPrompt({ msg: '请输入手机号码', type: 'prompt' });
            ...
            if (_this.applyMobileCode == "") {
                promptModule.showPrompt({ msg: '请输入验证码', type: 'prompt' });
                return;
            }
            if (_this.applyPsw == "") {
                promptModule.showPrompt({ msg: '请设置您的登录密码', type: 'prompt' });
                return;
            }
            loadingModule.show({ msg: '正在检测...' });
            _this.md5(_this.applyPsw, function (ps) {
                _this.Post(_this.api().check_mobile, {
                    mobile: areaCode + _this.applyMobile,
                    code: _this.applyMobileCode
                }, function (res) {
                    loadingModule.cancel(function (ret) {
                        var result = ret.result;
                        if (result == 'dismiss') {
                            if (res.data.code == '0') {
                                _this.startAnim('register/registerInviteCode', {
                                    type: res.data.data.type,
                                    nick_name: _this.applyNickName,
                                    area_code: _this.area_code,
                                    mobile: _this.applyMobile,
                                    psw: ps.md5
                                }, 2);
                            } else {
                                promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
    ...
    created: function created() {
        ...
        storage.setItem('send_voice', 'false', function (event) {
            if (event.result == 'success') {}
        });
        storage.setItem('area_code', this.area_code, function (event) {
            if (event.result == 'success') {}
        });
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    ref: "inputName",
    staticClass: ["input_nickName"],
    attrs: {
      "type": "text",
      "placeholder": "请设置您的昵称"
    },
    on: {
      "onInput": _vm.onInputNickName
    }
  }), _c('line'), _c('div', {
    staticClass: ["mobile"]
  }, [_c('div', {
    staticClass: ["areaCode"],
    on: {
      "click": function($event) {
        _vm.clickArea()
      }
    ...
    staticClass: ["areaCode_num"]
  }, [_vm._v(_vm._s(_vm.area_code))]), _c('image', {
    staticClass: ["image"],
    attrs: {
      "src": "assets://icon_expand_more"
    }
  })]), _c('div', {
    ...
    on: {
      "onInput": _vm.onInputMobile
    }
  })], 1), _c('line'), _c('div', {
    staticClass: ["mobileCode"]
  }, [_c('edit', {
    ref: "inputMobileCode",
    ...
    on: {
      "click": function($event) {
        _vm.sendCode(_vm.isClick)
      }
    }
  }, [_vm._v(_vm._s(_vm.codeName))])], 1), _c('line'), _c('edit', {
    ref: "inputPsw",
    staticClass: ["input_nickName"],
    attrs: {
      "type": "password",
    ...
    on: {
      "onInput": _vm.onInputPsw
    }
  }), _c('line'), (_vm.is_show) ? _c('text', {
    staticClass: ["voice_hint"]
  }, [_vm._v("*验证码已发送至" + _vm._s(_vm.area_code) + _vm._s(_vm.applyMobile) + ",请注意接听")]) : _vm._e(), _c('wx-button', {
    attrs: {
      "marginTop": "140",
      "content": "下一步",
    ...
    staticClass: ["code_err"],
    on: {
      "click": function($event) {
        _vm.alterCode()
      }
    }
  }, [_vm._v("无法收到验证码?")])])], 1), _c('div', {
    ...
    on: {
      "click": function($event) {
        _vm.clickAgreement('单创用户服务协议', 'https://wap.abmau.com/abm_protocol.html')
      }
    }
  }, [_vm._v("《单创用户服务协议》")]), _c('text', {
    staticClass: ["agm"],
    on: {
      "click": function($event) {
        _vm.clickAgreement('单创用户隐私权政策', 'https://wap.abmau.com/privacy_of_individual.html')
      }
    }
  }, [_vm._v("《单创用户隐私权政策》")])])])])
},staticRenderFns: []}
