...
var _registerInviteCode = __webpack_require__(2);

var _registerInviteCode2 = _interopRequireDefault(_registerInviteCode);
...
_registerInviteCode2.default.el = '#root';
new Vue(_registerInviteCode2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\register\\registerInviteCode.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-267cc625"
...
exports.default = {
    ...
    methods: {
        onInputInviteCode: function onInputInviteCode(e) {
            this.applyInviteCode = e.value;
            this.isParam = this.isValueParam(this, [this.applyInviteCode]);
        },
        ...
        clickNext: function clickNext() {
            var _this = this;
            if (_this.applyInviteCode == "") {
                if (_this.type == '1') {
                    promptModule.showPrompt({ msg: '请输入优惠码', type: 'prompt' });
                    return;
                } else {
                    _this.register();
                    return;
            ...
            _this.Get(_this.api().invite_code, {
                invite_code: _this.applyInviteCode
            }, function (res) {
                if (res.data.code == '0') {
                    _this.startAnim('register/register', { name: res.data.data.name, tel: res.data.data.mobile, psw: _this.applyPsw, nick_name: _this.applyNickName, invite_code: _this.applyInviteCode, area_code: _this.area_code, mobile: _this.applyMobile }, 3);
        ...
        register: function register() {
            ...
            _this.md5('上一个页面加密过了', function (ret) {
                _this.register_id = ret.register_id;
                loadingModule.show({ msg: '正在注册...' });
                _this.Post(_this.api().register, {
                    mobile: areaCode + _this.applyMobile,
                    nickname: _this.applyNickName,
                    password: _this.applyPsw,
                    invite_code: _this.applyInviteCode,
                    ...
                    loadingModule.cancel(function (ret) {
                        ...
                        if (result == 'dismiss') {
                            if (res.data.code == '0') {
                                ...
                                titleBar.registerSuccess({ id: res.data.data.id, name: res.data.data.name, invite_code: res.data.data.invite_code, nick_name: res.data.data.nickname, heading: res.data.data.heading, verify: res.data.data.verify, token: res.data.data.token, mobile: res.data.data.mobile, level_name: res.data.data.level_name, status: res.data.data.status });
                            } else promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                        }
                    });
                });
    ...
    created: function created() {
        ...
        this.area_code = this.GetUrlPara(weex.config.bundleUrl, 'area_code');
        this.applyNickName = this.GetUrlPara(weex.config.bundleUrl, 'nick_name');
        this.applyMobile = this.GetUrlPara(weex.config.bundleUrl, 'mobile');
        this.applyPsw = this.GetUrlPara(weex.config.bundleUrl, 'psw');
