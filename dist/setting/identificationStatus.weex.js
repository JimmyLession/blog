...
var _identificationStatus = __webpack_require__(2);

var _identificationStatus2 = _interopRequireDefault(_identificationStatus);
...
_identificationStatus2.default.el = '#root';
new Vue(_identificationStatus2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\setting\\identificationStatus.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-72dbf6e9"
...
exports.default = {
    ...
    data: function data() {
        return {
            height: height,
            bottom: bottom,
            faceImg: 'assets://ic_empty',
            backImg: 'assets://ic_empty',
            name: '',
            showAgain: false,
            id: '',
            result: '',
    ...
    created: function created() {
        ...
        _this.isIpx(_this);
        _this.statusFrom = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'statusFrom'));
        _this.getStatusData();
        event.onmessage = function (res) {
            if (res.data == _this.eventBusTag().REFRESH_CARD_ID) {
                _this.getStatusData();
            }
        };
    },
    methods: {
        ...
        wxcMinibarLeftButtonClicked: function wxcMinibarLeftButtonClicked() {
            //未实名第一次认证后显示认证状态
            if (this.statusFrom === 'identification') {
                if (weex.config.env.platform.toLowerCase() === 'ios') {
                    //返回设置列表
                    this.kill(2);
                } else if (weex.config.env.platform.toLowerCase() === 'android') {
                ...
                //已认证过,查看状态
            } else if (this.statusFrom === 'settingList') {
                this.popBar();
            }
        },
        getStatusData: function getStatusData() {
            var _this = this;
            _this.Get(_this.api().certify_userinfo, {}, function (res) {
                if (res.data.code == 0) {
                    var data = res.data.data;

                    if (data.card_valid == 1) {
                        _this.showAgain = false;
                        _this.result = "请稍等，实名认证进行中！";
                        _this.resultImg = "assets://icon_approving";
                    } else if (data.card_valid == 2) {
                        _this.showAgain = false;
                        _this.result = "恭喜，实名认证成功！！";
                        _this.resultImg = "assets://icon_approve_success";
                    } else {
                        _this.showAgain = true;
                        _this.result = "抱歉，实名认证未通过！";
                        _this.resultImg = "assets://icon_approve_err";
                    }
                    if (data.card_img_f) {
                        _this.showFace = true;
                        _this.faceImg = data.card_img_f;
                    }
                    if (data.card_img_b) {
                        _this.showBack = true;
                        _this.backImg = data.card_img_b;
                    }
                    _this.name = data.name;
                    _this.id = data.card_no;
                } else {
                    promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                }
            });
        },
        reIdentification: function reIdentification() {
            this.jumpTo2('setting/identification', '', { isReIdentification: true, jumpFrom: 'identificationStatus' });
        }
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
    ...
    staticClass: ["value"]
  }, [_vm._v(_vm._s(_vm.id))])]), _c('line'), _c('div', {
  ...
  }) : _vm._e()])]), _vm._m(0), (_vm.showAgain) ? _c('text', {
    staticClass: ["re_identification"],
    on: {
      "click": function($event) {
        _vm.reIdentification()
      }
    }
  }, [_vm._v("重新认证")]) : _vm._e()], 1)], 1)
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["hint_item"]
  }, [_c('text', {
    staticClass: ["img_hint"]
  }, [_vm._v("身份证正面(头像)")]), _c('text', {
    staticClass: ["img_hint"]
  }, [_vm._v("身份证反面(国徽)")])])
}]}
module.exports.render._withStripped = true
