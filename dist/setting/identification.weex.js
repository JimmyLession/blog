...
var _identification = __webpack_require__(2);
...
_identification2.default.el = '#root';
new Vue(_identification2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\setting\\identification.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-187f2f17"
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            name: '',
            id: '',
            faceImg: 'assets://icon_add_face',
            faceId: '',
            backImg: 'assets://icon_add_back',
            backId: '',
            defaultImg: 'assets://ic_empty',
            showFace: false,
            showBack: false,
            isReIdentification: false,
            jumpFrom: ''
        };
    },
    ...
    created: function created() {
        ...
        _this.isIpx(_this);
        _this.isReIdentification = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'isReIdentification'));
        _this.jumpFrom = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'jumpFrom'));
    },

    methods: {
        inputName: function inputName(e) {
            this.name = e.value;
            this.isParam = this.isValueParam(this, [this.name, this.id]);
        },
        inputId: function inputId(e) {
            this.id = e.value;
            this.isParam = this.isValueParam(this, [this.name, this.id]);
        },
        ...
        commit_identification: function commit_identification() {
            var _this = this;
            if (!_this.name) {
                promptModule.showPrompt({ msg: '请输入姓名', type: 'prompt' });
            } else if (!_this.id) {
                promptModule.showPrompt({ msg: '请输入身份证号码', type: 'prompt' });
            } else if (!_this.faceId) {
                promptModule.showPrompt({ msg: '请上传身份证正面照', type: 'prompt' });
            } else if (!_this.backId) {

                promptModule.showPrompt({ msg: '请上传身份证反面照', type: 'prompt' });
            } else {
                promptModule.show({
                    title: '确定提交审核',
                    message: '我们将在1-2个工作日内帮您完成审核',
                    cancel: '取消',
                    confirm: '确定'
                }, function (ret) {
                    var result = ret.result;
                    if (result == 'success') {
                        _this.Post(_this.api().update_certify, {
                            card_no: _this.id,
                            name: _this.name,
                            card_img_f: _this.faceId,
                            card_img_b: _this.backId
                        }, function (res) {
                            if (res.data.code == '0') {
                                promptModule.showListener({ msg: '提交成功', type: 'success' }, function (ret) {
                                    event.postMessage(_this.eventBusTag().REFRESH_CARD_ID);
                                    //未实名第一次认证
                                    if (_this.jumpFrom == 'settingList') {
                                        _this.jumpTo2('setting/identificationStatus', '', { statusFrom: 'identification' });
                                        if (weex.config.env.platform.toLowerCase() === 'android') _this.popBar();
                                    }
                                    //重新认证 - 返回设置列表
                                    else if (_this.jumpFrom == 'identificationStatus') {
                                            _this.popBar();
                                        }
                                        //重新提交 - 返回提现
                                        else if (_this.jumpFrom == 'bank') {
                                                _this.popBar();
                                            }
                                });
                            } else {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
    ...
    staticClass: ["img"],
    attrs: {
      "src": "assets://icon_add_back"
    }
  }) : _vm._e()])]), _vm._m(0), _c('wx-button', {
    attrs: {
      "marginTop": "120",
    ...
    on: {
      "onClick": function($event) {
        _vm.commit_identification()
      }
    }
  })], 1), _c('text', {
    staticClass: ["hint"]
  }, [_vm._v("上传身份证照片，注意避免反光，确保内容清晰可见")])])], 1)
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["hint_item"]
  }, [_c('text', {
    staticClass: ["img_hint"]
  }, [_vm._v("添加身份证正面(头像)")]), _c('text', {
    staticClass: ["img_hint"]
  }, [_vm._v("添加身份证反面(国徽)")])])
}]}
module.exports.render._withStripped = true
