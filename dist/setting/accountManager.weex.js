...
var _accountManager = __webpack_require__(2);
...
_accountManager2.default.el = '#root';
new Vue(_accountManager2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\setting\\accountManager.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-55aa3469"
...
var add = new BroadcastChannel('add_account');
var height = 0;
var bottom = 0;
exports.default = {
    components: { titleBar: _titleBar2.default, line: _line2.default, WxButton: _wxButton2.default },
    data: function data() {
        return {
            height: height,
            bottom: bottom,
    ...
    methods: {
        fetch: function fetch() {
            var _this = this;
            utilModule.getAccountData(function (res) {
                _this.data = [];
                _this.userInfo(res.account);
            });
        },
        userInfo: function userInfo(json) {
            var _this = this;
            var items = [];
            _this.list = JSON.parse(json);

            var _loop = function _loop(arr) {
                _this.Get(_this.api().user_info_msg, {
                    token: _this.list[arr].token
                }, function (res) {
                    if (res.data.code == 0) {
                ...
                comModal.getToken({}, function (value) {
                    if (value.token == _this.list[arr].token) _this.token = value.token;
                });
            };
        ...
        clickAccount: function clickAccount(token) {
            if (!this.edit) return;
            this.setToken(token);
            event.postMessage(this.eventBusTag().REFRESH_HOME);
            if (this.token != token) utilModule.toggleAccount();
            this.kill(2);
        },
        clickEdit: function clickEdit(id, index) {
            var _this = this;
            if (index == _this.is_select) {
                //点击同一个按钮
                console.log(_this.is_select);
                if (id == _this.id) var value = 180;else value = 90;
                this.shake(_this.$refs.edit_image[_this.is_select], value, function () {
                    _this.is_select = index;
                    if (id == _this.id) {
                        _this.show = false;
                        _this.id = 0;
                    } else {
                    ...
                    //_this.transition(_this.show,index);
                });
            } else {
                //点击不同按钮
                console.log(_this.is_select);
                if (_this.is_select != -1) _this.shake(_this.$refs.edit_image[_this.is_select], 180, function () {}); //
                _this.shake(_this.$refs.edit_image[index], 90, function () {
                    _this.is_select = index;
                    if (id == _this.id) {
                        _this.show = false;
                        _this.id = 0;
        ...
        transition: function transition(show, index) {
            var x = 0;
            if (show) {
                x = 140;
            } else x = 0;
            animation.transition(this.$refs.remove[index], {
                styles: {
        ...
        shake: function shake(el, value, callback) {
        ...
        clickRemove: function clickRemove(id, token) {
            var _this = this;
            promptModule.show({ title: '温馨提示',
                ...
                if (result == 'success') {
                    _this.removeAccount(token);
                }
            });
        },
        removeAccount: function removeAccount(token) {
            var _this = this;
            _this.Post(_this.api().logout, {
                user_token: token
            }, function (res) {
                if (res.data.code == 0) {
                    if (_this.is_select != -1 && _this.is_select < _this.data.length - 1) _this.shake(_this.$refs.edit_image[_this.is_select], 180, function () {
                        _this.is_select = -1;
                    });

                    var _loop2 = function _loop2(arr) {
                        if (_this.data[arr].token == token) {
                            utilModule.removeAccount({ id: _this.data[arr].id + "", token: _this.data[arr].token }, function (ret) {
                                if (ret.result == 'success') {
                                    _this.data.splice(arr, 1);
                                    if (_this.token == token) {
                                        if (_this.data.length != 0) {
                                            _this.token = _this.data[0].token;
                                            _this.setToken(_this.token);
                                            utilModule.logoutUnicorn();
                                            event.postMessage(_this.eventBusTag().REFRESH_HOME);
                                            if (_this.token != token) utilModule.toggleAccount();
                                        } else {
                                            _this.setToken("");
                                            utilModule.logout();
                                        }
                    ...
                    for (var arr in _this.data) {
                        _loop2(arr);
                    }
                } else promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
            });
        },
        clickOutLogin: function clickOutLogin() {
            var _this = this;
            ...
            promptModule.show({ title: '温馨提示',
                ...
                if (result == 'success') {
                    for (var arr in _this.data) {
                        _this.tokenArray.push(_this.data[arr].token);
                    }
                    _this.logoutLogin(_this.tokenArray.join(','));
                }
            });
        },
        logoutLogin: function logoutLogin(user_token) {
            var _this = this;
            _this.Post(_this.api().logout, {
        ...
        addAccount: function addAccount() {
            if (!this.edit) return;
            this.jumpTo2('login/dc_login', '', { type: 'add' });
        }
    },
    created: function created() {
        ...
        this.fetch();
        var _this = this;
        add.onmessage = function (event) {
            if (!event.data) {
                return;
            }
            switch (event.data) {
                case _this.eventBusTag().REFRESH_ACCOUNT:
                    _this.fetch();
                    break;
            }
        };
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('div', {
      ...
      staticClass: ["account_item"],
      on: {
        "click": function($event) {
          _vm.clickAccount(item.token)
        }
      }
    }, [(!_vm.edit) ? _c('image', {
      ref: "edit_image",
      ...
      attrs: {
        "src": "assets://ic_remove_circle_black_36dp"
      },
      on: {
        "click": function($event) {
          _vm.clickEdit(item.id, index)
        }
      }
    }) : _vm._e(), (item.headimg == '' || item.headimg == null) ? _c('image', {
      ...
      staticClass: ["account_content"]
    }, [_c('text', {
      staticClass: ["user_name"]
    }, [_vm._v(_vm._s(item.name))]), _c('text', {
      staticClass: ["user_mobile"]
    }, [_vm._v(_vm._s(item.mobile))])]), _c('div', [(_vm.id == item.id && _vm.show) ? _c('text', {
      ref: "remove",
      refInFor: true,
      staticClass: ["removeItem"],
      on: {
        "click": function($event) {
          _vm.clickRemove(item.id, item.token)
        }
      }
    }, [_vm._v("删除")]) : _vm._e(), (_vm.token == item.token && _vm.edit) ? _c('image', {
      staticClass: ["add_image"],
      attrs: {
        "src": "assets://icon_account_selected"
      }
    }) : _vm._e()])]), _c('line')], 1)
  })), _c('cell', {
    appendAsTree: true,
    ...
    staticClass: ["add_text"]
  }, [_vm._v("添加账号")])]), _c('line')], 1)]), _c('text', {
