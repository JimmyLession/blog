...
var _classifyLabel = __webpack_require__(2);

var _classifyLabel2 = _interopRequireDefault(_classifyLabel);
...
_classifyLabel2.default.el = '#root';
new Vue(_classifyLabel2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\material\\classifyLabel.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-3e418df8"
...
var bottom = 0;
exports.default = {
  components: { title: _titleBar2.default, empty: _empty2.default },
  data: function data() {
    return {
      title: "添加标签",
      height: height,
  ...
  methods: {
    ...
    getData: function getData() {
      ...
      var allidArrName = new Array();
      allidArrName = this.GetUrlPara(weex.config.bundleUrl, "allName").split(",");
      if (allidArrName != "") {
        allidArrName.forEach(function (element) {
          _this.classifyName.push(element);
        });
      }
      this.Get(this.api().material_label, {
        brand_id: this.brand_id
      }, function (res) {
        if (res.data.code == "0") {
          if (res.data.data.all_tag == "" && res.data.data.recommend_tag == "") {
            _this.emptyShow = true;
          } else {
            _this.emptyShow = false;
          }
          res.data.data.all_tag.forEach(function (element) {
            var checkValue = true;
            if (_this.classifyName.indexOf(element.tag_name) < 0) {
              checkValue = false;
            }
            _this.all_tag.push({
          ...
          res.data.data.recommend_tag.forEach(function (element) {
            var checkValue = true;
            if (_this.classifyName.indexOf(element.tag_name) < 0) {
              checkValue = false;
            }
            _this.recommend_tag.push({
              tag_name: element.tag_name,
              check: checkValue
            });
          });
    ...
    //选择综合标签
    clickAll: function clickAll(item) {
      item.check = !item.check;
      var index = this.classifyName.indexOf(item.tag_name);
      if (index < 0) {
        this.classifyName.push(item.tag_name);
      } else {
        this.classifyName.splice(index, 1);
      }
    },

    //确定
    submit: function submit() {
      if (this.all_tag == "" && this.recommend_tag == "") {
        promptModule.showPrompt({ msg: "该商品没有标签", type: "prompt" });
      } else {
        if (this.classifyName.length == 0) {
          promptModule.showPrompt({ msg: "请添加标签", type: "prompt" });
        } else {
          storage.setItem("classify", JSON.stringify({
            classifyName: this.classifyName.toString()
          }), function (event) {});
          classifylabel.postMessage(this.eventBusTag().REFRESH_MATERIAL_TWO);
  ...
  created: function created() {
    ...
    this.isIpx(this);
    this.brand_id = this.GetUrlPara(weex.config.bundleUrl, "materialBrandId");
    this.getData();
  },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  ...
  }, _vm._l((_vm.all_tag), function(item, i) {
    return _c('div', {
      key: i,
      class: [item.check ? 'item_sure' : 'item'],
      on: {
      ...
      class: [item.check ? 'item_text_sure' : 'item_text']
    }, [_vm._v(_vm._s(item.tag_name))])])
  }))]) : _vm._e(), (_vm.recommend_tag != '') ? _c('div', {
    staticClass: ["content"]
  }, [_vm._m(1), _c('div', {
    staticClass: ["list", "row", "wrap"]
  }, _vm._l((_vm.recommend_tag), function(item, i) {
    return _c('div', {
      key: i,
      class: [item.check ? 'item_sure' : 'item'],
      on: {
  ...
  }))]) : _vm._e(), (_vm.emptyShow) ? _c('div', [_c('empty')], 1) : _vm._e()])], 1)
