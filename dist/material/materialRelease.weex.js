...
var _materialRelease = __webpack_require__(2);

var _materialRelease2 = _interopRequireDefault(_materialRelease);
...
_materialRelease2.default.el = '#root';
new Vue(_materialRelease2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\material\\materialRelease.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-5ab9e7f0"
...
var relationcommodity = new BroadcastChannel("relationcommodity");
var classifylabel = new BroadcastChannel("classifylabel");
...
exports.default = {
  ...
  data: function data() {
    return {
      ...
      addImgShow: true,
      type: "",
      materialComShow: true, //控制关联商品
      materialId: 0, //选中商品id
      materialName: "", //选中商品Name
      materialBrand_id: 0, //选中商品品牌id
      classifyShow: true, //控制标签
      classifyName: [], //标签名
      id: 0, //素材id
      selectionShow: false
    };
  ...
  methods: {
    ...
    clickdel: function clickdel(index) {
      var _this = this;
      var items = [{ type: 0, message: "删除" }, { type: 1, message: "取消" }];
      actionSheet.create({
        items: items
      }, function (ret) {
        var result = ret.result;
        if (result == "success") {
          if (ret.data.message == "删除") {
            _this.delImg(index);
          }
        }
      });
    ...
    submit: function submit() {
      var _this = this;
      var coverIdArray = [];
      _this.cover.forEach(function (element) {
        coverIdArray.push(element.id);
      });
      var coverIdString = coverIdArray.toString();
      if (_this.content == "") {
        promptModule.showPrompt({ msg: "请填写内容", type: "prompt" });
      } else if (coverIdString.length == 0) {
        promptModule.showPrompt({ msg: "请上传图片", type: "prompt" });
      }
      // else if (_this.materialId < 1) {
      //   promptModule.showPrompt({ msg: "请关联商品", type: "prompt" });
      // } else if (_this.classifyName.length == 0) {
      //   promptModule.showPrompt({ msg: "请关联标签", type: "prompt" });
      // }
      else if (!_this.selectionShow) {
          ...
          _this.Post(_this.api().material_user, {
            id: _this.id,
            // all_tag: _this.classifyName.toString(),
            // goods_id: _this.materialId,
            image_id: coverIdString,
            content: _this.content
          }, function (res) {
            loadingModule.cancel(function (ret) {
              if (res.data.code == "0") {
                // 结果处理
                materialrelease.postMessage(_this.eventBusTag().MATERIAL_RELEASE);
                if (_this.id > 0 || _this.type == "first") {
                  _this.interceptPush("material/releaseSuccess", { type: "change" }, "intercept");
                } else {
                  _this.interceptPush("material/releaseSuccess", { type: "add" }, "intercept");
                }
                if (weex.config.env.platform.toLowerCase() === "android") _this.popBar();
                storage.removeItem("material", function (event) {});
                storage.removeItem("classify", function (event) {});
              } else {
    ...
    //关联商品
    clickRc: function clickRc() {
      this.jumpTo2("material/relationCommodity", "", {
        materialId: this.materialId,
        materialName: this.materialName,
        materialBrand_id: this.materialBrand_id
      });
    },

    //分类标签
    clickCl: function clickCl() {
      if (this.materialId < 1) {
        promptModule.showPrompt({ msg: "请先关联商品", type: "prompt" });
      } else {
        this.jumpTo2("material/classifyLabel", "", {
          allName: this.classifyName.toString(),
          materialBrandId: this.materialBrand_id
        });
      }
    },

    //获取素材
    ifmaterial: function ifmaterial() {
      var _this2 = this;

      var i = this.GetUrlPara(weex.config.bundleUrl, "id");
      if (i > 0) {
        this.id = i;
        this.title = "修改素材";
        this.Get(this.api().material_user_detail, {
          material_id: this.id
        }, function (res) {
          if (res.data.code == "0") {
            res.data.data.forEach(function (element) {
              _this2.content = element.desc;
              element.img_list.forEach(function (element_two) {
                _this2.cover.push({
                  id: element_two.id,
                  src: element_two.img_url
                });
              });
              _this2.materialComShow = false;
              _this2.materialName = element.related_goods.name;
              _this2.materialId = element.related_goods.id;
              _this2.materialBrand_id = element.related_goods.brand_id;
              _this2.classifyShow = false;
              element.tag_list.forEach(function (element_three) {
                _this2.classifyName.push(element_three.name);
              });
            });
          } else {
            promptModule.showPrompt({ msg: res.data.msg, type: "err" });
    ...
    //跳转如何发布素材
    clickHow: function clickHow() {
      this.jumpTo2("material/materialHow", "", {});
    },
  ...
  created: function created() {
    ...
    if (weex.config.env.platform.toLowerCase() == "android") {
      _this.textReportStyle = {
        minHeight: 70 + "px",
        maxHeight: 100 + "px"
      };
    } else {
      _this.textReportStyle = { height: "160px" };
    }
    relationcommodity.onmessage = function (event) {
      if (!event.data) {
        return;
      }
      switch (event.data) {
        case _this.eventBusTag().REFRESH_MATERIAL:
          storage.getItem("material", function (event) {
            if (event.result == "success") {
              var data = JSON.parse(event.data);
              _this.materialName = data.materialName;
              var materialIdNew = data.materialId;
              if (_this.materialId != materialIdNew) {
                _this.classifyShow = true;
                _this.classifyName = [];
              }
              _this.materialId = data.materialId;
              if (_this.materialId < 1) {
                _this.materialComShow = true;
              } else {
                _this.materialComShow = false;
              }
              _this.materialBrand_id = data.materialBrand_id;
            }
          });
          break;
      }
    };
    classifylabel.onmessage = function (event) {
      if (!event.data) {
        return;
      }
      switch (event.data) {
        case _this.eventBusTag().REFRESH_MATERIAL_TWO:
          storage.getItem("classify", function (event) {
            if (event.result == "success") {
              var data = JSON.parse(event.data);
              _this.classifyName = data.classifyName.split(",");
              if (_this.classifyName.length == 0) {
                _this.classifyShow = true;
              } else {
                _this.classifyShow = false;
              }
            }
          });
          break;
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["list_image", "row", "wrap"]
    ...
    staticClass: ["list_type", "row", "between"],
    on: {
      "click": _vm.clickRc
    }
  }, [_c('text', {
    staticClass: ["list_type_name"]
  }, [_vm._v("关联商品")]), _c('div', {
    staticClass: ["row"]
  }, [_c('div', {
    staticClass: ["row"]
  }, [(_vm.materialComShow) ? _c('text', {
    staticClass: ["ifRelation"]
  }, [_vm._v("未关联")]) : _vm._e(), (_vm.materialComShow) ? _c('div', {
    staticClass: ["column", "center"]
  }, [_c('div', {
    staticClass: ["redSpot"]
  })]) : _vm._e(), (!_vm.materialComShow) ? _c('text', {
    staticClass: ["ifRelation"]
  }, [_vm._v(_vm._s(_vm.materialName))]) : _vm._e()]), _vm._m(0)])]) : _vm._e(), (false) ? _c('line') : _vm._e(), (false) ? _c('div', {
    staticClass: ["list_type", "row", "between"],
    on: {
      "click": _vm.clickCl
    }
    ...
    staticClass: ["row"]
  }, [(_vm.classifyShow) ? _c('text', {
    staticClass: ["ifRelation"]
  }, [_vm._v("未关联")]) : _vm._e(), (_vm.classifyShow) ? _c('div', {
    staticClass: ["column", "center"]
  }, [_c('div', {
    staticClass: ["redSpot"]
  })]) : _vm._e(), (!_vm.classifyShow) ? _c('div', {
    staticClass: ["classify_name", "row", "wrap"]
  }, _vm._l((_vm.classifyName), function(item, i) {
    return _c('div', {
      key: i
    }, [_c('div', {
      staticClass: ["classify_list"]
    }, [_c('text', {
      staticClass: ["classify_list_text"]
    }, [_vm._v(_vm._s(item))])])])
  })) : _vm._e(), _vm._m(1)])])]) : _vm._e(), _c('div', {
    staticClass: ["separate"]
  }), _c('div', {
    staticClass: ["list_type", "row", "between"],
    on: {
    ...
    staticClass: ["list_type_name"]
  }, [_vm._v("如何发布一篇优秀的素材")]), _vm._m(2)]), _c('line'), _c('div', {
    staticClass: ["list_bottom", "row"]
  }, [(!_vm.selectionShow) ? _c('image', {
    ...
    on: {
      "click": function($event) {
        _vm.selectionShow = !_vm.selectionShow
      }
    }
  }) : _vm._e(), (_vm.selectionShow) ? _c('image', {
    staticClass: ["selection"],
    attrs: {
    ...
    staticClass: ["selection_text"],
    on: {
      "click": function($event) {
        _vm.selectionShow = !_vm.selectionShow
      }
    }
  }, [_vm._v("同意并接受")]), _c('text', {
    staticClass: ["selection_text_b"],
    on: {
      "click": _vm.clickAgreement
    }
  }, [_vm._v("《发布协议》")])])], 1)], 1)
