...
_collection2.default.el = '#root';
new Vue(_collection2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\material\\collection.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-e238e0b4"
...
var bottom = 0;
exports.default = {
    components: { title: _titleBar2.default, line: _line2.default, titleBar: _titleBar2.default },
    data: function data() {
        return {
            data: [],
            ids: [],
    ...
    methods: {
        wxcMinibarRightButtonClicked: function wxcMinibarRightButtonClicked() {
            ...
            for (var i = 0; i < self.data.length; i++) {
                if (self.data[i].select) {
                    self.ids.push(self.data[i].id);
                }
            }
            var selectNumEnd = self.ids.length; //提交之前选中的文件夹数量
            var idStr = self.ids.join(",");
            self.Post(self.api().add_collect, {
                collect_id: idStr,
                ...
                if (res.data.code != '0') {
                    ...
                    //请求数据成功
                    promptModule.showListener({ msg: res.data.msg, type: 'success' }, function () {

                        if (selectNumEnd > 0) {
                            //取消收藏
                            //孙林说的
                            collectNum.postMessage({ num: selectNumEnd - self.selectNum, type: self.eventBusTag().COLLECTION_MATERIAL });
                        } else {
                            //添加收藏
                            collectNum.postMessage({ num: self.selectNum - selectNumEnd, type: self.eventBusTag().COLLECTION_CANCEL });
                        }
                        self.popBar();
                    });
                }
            ...
            self.Get(self.api().collect, {
                ...
                if (res.data.code != '0') {
                    ...
                    for (var i = 0; i < res.data.data.length; i++) {
                        if (res.data.data[i].is_collect) {
                            res.data.data[i].select = true;
                            res.data.data[i].icon = "assets://xuanzhong";
                            self.selectNum++;
                        } else {
                            res.data.data[i].select = false;
                            res.data.data[i].icon = "assets://weixuanzhong";
                        }
        ...
        jumpToDetail: function jumpToDetail(id, title) {
            //跳转到收藏夹的详情
        ...
        selectItem: function selectItem(i) {
            ...
            if (this.data[i].select) {
                ...
                this.data[i].icon = "assets://weixuanzhong";
            } else {
                this.data[i].select = true;
                this.data[i].icon = "assets://xuanzhong";
            }
        }
    },
    created: function created() {
        var _this = this;
        _this.material_id = _this.GetUrlPara(weex.config.bundleUrl, 'id');
        ...
        event.onmessage = function (event) {
            ...
            switch (event.data) {
                case _this.eventBusTag().CREATE_FAVORITE:
                    _this.data = [];
                    _this.getFavorites();
                    break;
            }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["parent"],
    style: {
      top: (_vm.height + _vm.topHeight) + 'px',
    ...
    }, [_vm._v("共" + _vm._s(item._count) + "个用户口碑")])]), _c('image', {
      staticClass: ["next"],
      attrs: {
        "src": item.icon
      }
