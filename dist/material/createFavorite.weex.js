...
var _createFavorite = __webpack_require__(2);

var _createFavorite2 = _interopRequireDefault(_createFavorite);
...
_createFavorite2.default.el = '#root';
new Vue(_createFavorite2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\material\\createFavorite.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-70f51ec0"
...
var bottom = 0;
exports.default = {
    components: { titleBar: _titleBar2.default, line: _line2.default },
    data: function data() {
        return {
            height: height,
            bottom: bottom,
    ...
    methods: {
        wxcMinibarRightButtonClicked: function wxcMinibarRightButtonClicked() {
            if (this.favoriteName == '') {
                promptModule.showPrompt({ msg: "请输入收藏夹标题", type: 'err' });
            } else {
                var self = this;
                if (self.id) {
                    //编辑文件夹
                    self.Post(self.api().material_collect, {
                        id: self.id,
                        ...
                        if (res.data.code != '0') {
                            ...
                            promptModule.showListener({ msg: res.data.msg, type: 'success' }, function () {
                                event.postMessage(self.favoriteName);
                                self.popBar();
                            });
                        }
                    });
                    ...
                    self.Post(self.api().material_collect, {
                        ...
                        if (res.data.code != '0') {
                            ...
                            //请求数据成功
                            promptModule.showListener({ msg: res.data.msg, type: 'success' }, function () {
                                event.postMessage(self.eventBusTag().CREATE_FAVORITE);
                                self.popBar();
                            });
                        }
                    });
    ...
    created: function created() {
        ...
        this.isIpx(this);
        var self = this;
        if (self.GetUrlPara(weex.config.bundleUrl, 'title') == '') {
            self.title = '创建收藏夹';
        } else {
            self.title = '重命名';
        }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    attrs: {
      "type": "text",
      "placeholder": "收藏夹标题",
      "value": (_vm.favoriteName)
    },
    on: {
      "input": function($event) {
        _vm.favoriteName = $event.target.attr.value
      }
    }
  }), _c('line')], 1)], 1)
},staticRenderFns: []}
