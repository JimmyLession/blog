...
var _relationCommodity = __webpack_require__(2);

var _relationCommodity2 = _interopRequireDefault(_relationCommodity);
...
_relationCommodity2.default.el = '#root';
new Vue(_relationCommodity2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\material\\relationCommodity.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-3e1f0f39"
...
exports.default = {
  ...
  methods: {
    ...
    getData: function getData(type) {
      ...
      this.Get(this.api().relation_goods, {
        limit: this.limit,
        offset: this.offset,
        keywords: this.keywords,
        goods_id: this.isSelectId
      }, function (res) {
        if (res.data.code == "0") {
          if (res.data.count == 0) {
    ...
    //选中
    ifCheck: function ifCheck(id, name, brand_id) {
      this.isSelectId = id;
      this.isSelectName = name;
      this.isSelectBrand_id = brand_id;
    },

    //完成
    submit: function submit() {
      if (this.isSelectId < 1) {
        promptModule.showPrompt({ msg: "请选择关联商品", type: "prompt" });
      } else {
        storage.setItem("material", JSON.stringify({
          materialId: this.isSelectId,
          materialName: this.isSelectName,
          materialBrand_id: this.isSelectBrand_id
        }), function (event) {});
        relationcommodity.postMessage(this.eventBusTag().REFRESH_MATERIAL);
        this.wxcMinibarLeftButtonClicked();
      }
    }
  },
  created: function created() {
    this.isSelectId = this.GetUrlPara(weex.config.bundleUrl, "materialId");
    this.isSelectName = this.GetUrlPara(weex.config.bundleUrl, "materialName");
    this.isSelectBrand_id = this.GetUrlPara(weex.config.bundleUrl, "materialBrand_id");
    this.getTopHeight(this);
    this.isIpx(this);
    if (this.isSelectId >= 1) this.getData(1);
  },
  destroyed: function destroyed() {
    relationcommodity.close();
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('cell', {
      ...
      on: {
        "click": function($event) {
          _vm.ifCheck(item.id, item.name, item.brand_id)
        }
      }
    }, [_c('div', {
    ...
    }, [(_vm.isSelectId == item.id) ? _c('image', {
      staticClass: ["check_img"],
      attrs: {
        "src": "assets://icon_cart_check"
      }
    }) : _vm._e(), (_vm.isSelectId != item.id) ? _c('image', {
      staticClass: ["check_img"],
      attrs: {
        "src": "assets://icon_cart_uncheck"
      }
    }) : _vm._e()]), _c('image', {
      ...
      staticClass: ["column", "between"]
    }, [_c('div', {
      staticClass: ["list_right_goods"]
    }, [_c('text', {
      staticClass: ["list_right_goods_name"]
    }, [_vm._v(_vm._s(item.name))])]), _c('text', {
      staticClass: ["retail"]
    }, [_vm._v(_vm._s(item.retail_title) + " ¥" + _vm._s(item.retail_price))])])]), _c('line')], 1)
  }), (_vm.goods.length > 5) ? _c('cell', {
    ...
    staticClass: ["loading-over-txt"]
  }, [_vm._v("已经全部加载完毕")]) : _vm._e()]) : _vm._e(), (_vm.emptyShow) ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
