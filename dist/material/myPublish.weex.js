...
var _myPublish = __webpack_require__(7);

var _myPublish2 = _interopRequireDefault(_myPublish);

var _mixin = __webpack_require__(55);
...
_myPublish2.default.el = '#root';
new Vue(_myPublish2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\material\\myPublish.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-2927ab6b"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var _wxcTabPage = __webpack_require__(20);
...
var _titleBar = __webpack_require__(3);

var _titleBar2 = _interopRequireDefault(_titleBar);

var _myFavorites = __webpack_require__(35);

var _myFavorites2 = _interopRequireDefault(_myFavorites);

var _refresh = __webpack_require__(42);
...
var _releaseList = __webpack_require__(46);
...
var materialrelease = new BroadcastChannel("materialrelease");
exports.default = {
    components: { title: _titleBar2.default, line: _line2.default, myFavorites: _myFavorites2.default, refresher: _refresh2.default, empty: _empty2.default, WxcTabPage: _wxcTabPage2.default, material: _releaseList2.default, WxcMask: _wxcMask2.default },
    data: function data() {
        return {
            titleObj: [{ name: "我的发布" }, { name: "我的收藏" }],
            bottom: bottom,
            isiOS: true,
            release_type: '',
            ...
            size: 10,
            page: [1, 1, 1, 1],
            loading: [false, false, false, false], //加载中
            allLoadingFinish: [false, false, false, false], //数据是否全部加载完成
            emptyIsShow: [false, false, false, false],
            bgImgSrc: '',
            index: 0,
            type: 0,
            tabList: [],
            backType: '',
            pass_status: 0, //审核通过是否显示小红点 0-否,1-是
            refuse_status: 0, //审核拒绝是否显示小红点 0-否,1-是
            hasData: false, //用于判断是否有发布的数据
            list: [],
            isIphoneX: false,
            collectionData: null,
            collectionIndex: -1
    ...
    methods: {
        leftButtonClicked: function leftButtonClicked() {
            ...
            if (_this.isFinish) {
                if (weex.config.env.platform.toLowerCase() === 'ios') {
                    _this.isiOS = false;
                    if (_this.release_type == 'add') {
                        this.kill(3);
                    } else if (_this.release_type == 'change') {
                        this.kill(4);
                    } else {
                        _this.popBar();
                    }
        ...
        delMaterial: function delMaterial(event, i) {
            //删除素材
            var self = this;
            console.log("index  :" + i);
            promptModule.show({
        ...
        jumpToRelease: function jumpToRelease(id) {
            //跳转到立即发布(数据为空的时候)
            ...
            this.jumpTo2("material/materialRelease", "", { id: id, type: 'first' });
        },
        seeMoreRefuse: function seeMoreRefuse(refuse) {
            //查看拒绝原因
            this.refuse = refuse;
            this.orderMask = true;
        },
        ...
        loadingDown: function loadingDown() {
            //下拉刷新
            var _this = this;
            _this.allLoadingFinish[_this.index] = false;
            _this.type = _this.index;
            _this.emptyIsShow[_this.index] = false;
            _this.loading[_this.index] = false;
            // Vue.set(_this.tabList, _this.index, []);
            _this.page[_this.index] = 1;
            _this.hasData = true;
            _this.fetch(_this.index);
        },
        onloading: function onloading() {
            //上拉加载
            if (this.tabList[this.index].length == 0) {
                return;
            }
            if (!this.loading[this.index] && this.tabList[this.index].length < this.size) return;
            this.loading[this.index] = true;
            if (!this.allLoadingFinish[this.index]) {
        ...
        wxcTabPageCurrentTabSelected: function wxcTabPageCurrentTabSelected(e) {
            ...
            var index = e.page;
            // if (index != 0) {
            //     _this.height  = parseInt(_this.titleHeight) + parseInt(_this.nativeBarHeight) + 2;
            // }
            switch (index) {
                case 0:
                    _this.type = 0; //全部
                ...
                case 1:
                    _this.type = 1; //通过
                    _this.pass_status = 0;
                    if (_this.pass_status == 0 && _this.refuse_status == 0) {
                        event.postMessage(_this.eventBusTag().RED_DOT);
                    }
                    break;
                case 2:
                    _this.type = 2; //待审核
            ...
            if (!_this.tabList[index] || _this.tabList[index].length === 0) {
                _this.fetch(index);
            }
        },
        fetch: function fetch(index) {
            //初始加载数据
            var _this = this;
            _this.Get(_this.api().release_list, {
                // type:2,
                // code:'ABM',
                page: _this.page[index],
                size: _this.size,
                select_type: _this.type
            }, function (res) {
                if (res.data.code == '0') {
                    //请求数据成功
                    var data = _this.tabList[index];
                    if (!data) {
                        data = [];
                    };
                    res.data.data = _this.resetData(res.data.data);
                    if (_this.page[index] === 1) {
                        data = res.data.data;
                    } else {
                    ...
                    Vue.set(_this.tabList, index, data);
                    if (res.data._count === 0) {
                        //没有数据的情况
                        _this.hasData = false;
        ...
        getReleaseDot: function getReleaseDot() {
            ...
            _this.Get(_this.api().material_littlered, {}, function (res) {
                if (res.data.code == 0) {
                    _this.pass_status = res.data.data.pass_status;
                    _this.refuse_status = res.data.data.refuse_status;
                }
            });
        },
        getTop: function getTop() {
            var self = this;
            comModal.getHeight({}, function (data) {
                self.topHeight = parseInt(data.topHeight) + 1.4;
            });
        }
    },
    created: function created() {
        ...
        _this.getReleaseDot();
        _this.height = _this.isIphoneX ? _this.height + 68 : _this.height;
        console.log('_this.height == ' + _this.height);
        _this.backType = _this.GetUrlPara(weex.config.bundleUrl, 'type');
        _this.release_type = _this.GetUrlPara(weex.config.bundleUrl, 'release_type');
        _this.tabList = [].concat(_toConsumableArray(Array(_this.tabTitles.length).keys())).map(function (i) {
            return null;
        });
        _this.fetch(_this.index);
        materialrelease.onmessage = function (event) {
            if (!event.data) {
                return;
            } else {
                if (_this.eventBusTag().MATERIAL_RELEASE == 'material_release') {
                    _this.fetch(_this.index);
                }
            }
        };
        collectNum.onmessage = function (event) {
            ...
            if (_this.collectionData && _this.collectionIndex != -1) {
                ...
                _this.data.splice(_this.collectionIndex, 1, _this.collectionData);
            }
        };
    }
};
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\material\\myFavorites.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-119cd3b3"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
var event = new BroadcastChannel('createFavorite');
exports.default = {
    components: { title: _titleBar2.default, line: _line2.default, empty: _empty2.default },
    data: function data() {
        return {
            data: [],
            isDel: false,
    ...
    methods: {
        jumpTocreateFavorite: function jumpTocreateFavorite() {
            //跳转到创建收藏夹
            this.jumpTo2('material/createFavorite', '', {});
            //this.jumpTo2('material/createFavorite', '', {title:'',id:''});
        },
        jumpToeditFavorite: function jumpToeditFavorite() {
            //编辑收藏夹
            this.isDel = !this.isDel;
        },
        jumpToDelete: function jumpToDelete(i, id) {
            //删除收藏夹
            // promptModule.showPrompt({msg:id,type:'err'});
            var self = this;
            promptModule.show({
                title: '温馨提示',
                message: '确定删除该收藏夹吗?',
                ...
                var result = ret.result;
                if (result == 'success') {
                    self.Delete(self.api().collect_delete, {
                        id: id
                    }, function (res) {
                        if (res.data.code != '0') {
                            //请求数据失败
        ...
        getFavorites: function getFavorites() {
            //获取收藏夹
            var self = this;
            self.Get(self.api().collect, {}, function (res) {
                if (res.data.code != '0') {
                    //请求数据失败
                    promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                } else {
                    ...
                    for (var i = 0; i < res.data.data.length; i++) {
                        self.data.push(res.data.data[i]);
                    }
                }
            });
        },
        jumpToDetail: function jumpToDetail(i, id, title) {
            //跳转到收藏夹的详情
            if (this.isDel) {
                //要对图标判断是删除还是进入详情
                this.jumpToDelete(i, id);
            } else {
                this.jumpTo2('material/favoriteDetail', '', { title: title, id: id });
            }
        }
    },
    created: function created() {
        ...
        event.onmessage = function (event) {
            if (!event.data) {
                ...
                if (event.data == _this.eventBusTag().CREATE_FAVORITE || event.data == _this.eventBusTag().COLLECTION_MATERIAL) {
                    _this.data = [];
                    _this.getFavorites();
                } else {
                    _this.data = [];
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["my-favorites-parent"]
  }, [_c('list', [_vm._l((_vm.data), function(item, i) {
    return (_vm.data.length != 0) ? _c('cell', {
      key: i,
      ...
      on: {
        "click": function($event) {
          _vm.jumpToDetail(i, item.id, item.name)
        }
      }
    }, [_c('div', {
      staticClass: ["favorite-left"]
    }, [_c('text', {
      staticClass: ["favorite-left-title"]
    }, [_vm._v(_vm._s(item.name))]), _c('text', {
      staticClass: ["favorite-left-num"]
    }, [_vm._v("共" + _vm._s(item._count) + "个用户口碑")])]), (!_vm.isDel) ? _c('image', {
      staticClass: ["next"],
      attrs: {
  ...
  }, [_c('empty')], 1) : _vm._e()], 2), _c('line'), _c('div', {
    staticClass: ["handle-btn"]
  }, [_c('div', {
    staticClass: ["create"],
    on: {
      "click": function($event) {
        _vm.jumpTocreateFavorite()
      }
    }
  }, [_c('image', {
    staticClass: ["create-img"],
    attrs: {
      "src": "assets://icon_add_circle"
    }
  }), _c('image'), _c('text', {
    staticClass: ["create-text"]
  }, [_vm._v("创建收藏夹")])]), _c('div', {
    staticClass: ["create"],
    on: {
      "click": function($event) {
        _vm.jumpToeditFavorite()
      }
    }
  }, [_c('image', {
    staticClass: ["create-img"],
...
var _pictureGrid = __webpack_require__(49);
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["left"],
    style: {
      height: _vm.topHeight - _vm.nativeBarHeight + 'px',
      marginTop: _vm.nativeBarHeight + 'px'
    },
    attrs: {
      "ariaLabel": "返回",
  ...
  }, _vm._l((_vm.titleObj), function(item, i) {
    return _c('text', {
      key: i,
      class: ['nav_tittle', i == _vm.slot ? 'active' : ''],
      on: {
        "click": function($event) {
          _vm.changeTab(i)
        }
    ...
    staticClass: ["parent"],
    style: {
      width: 750 * _vm.titleObj.length + 'px',
      flexDirection: 'row',
      top: 0 + 'px',
      bottom: _vm.bottom + 'px',
      left: -750 * _vm.slot + 'px',
      height: _vm.height
    }
  }, [(!_vm.hasData && _vm.tabList[0] && _vm.tabList[0].length == 0 && _vm.isiOS) ? _c('div', {
    staticClass: ["my-publish-parent"]
  }, [_vm._m(0), _c('text', {
    staticClass: ["msg-text"]
  }, [_vm._v("您暂时还没有发布口碑素材")]), _c('div', {
  ...
  }, [_vm._v("立即发布")])])]) : _vm._e(), (_vm.tabList[0] && _vm.tabList[0].length != 0 && _vm.isiOS) ? _c('div', {
    staticClass: ["my-publish-parent"]
  }, [_c('wxc-tab-page', {
    ref: "wxc-tab-page",
    staticClass: ["navbar"],
    ...
    return _c('list', {
      ...
      return (items.length != 0) ? _c('cell', {
        ...
        on: {
          "collection": function($event) {
            _vm.jumpCollection($event, i)
          },
          "delMaterial": function($event) {
            _vm.delMaterial($event, i)
          }
        }
      }), (item.review_status == 2) ? _c('div', {
        staticClass: ["handle", "handle-edit"]
      }, [_c('text', {
        staticClass: ["handle-edit-reason"],
        on: {
          "click": function($event) {
            _vm.seeMoreRefuse(item.refuse)
          }
        }
      }, [_vm._v(_vm._s('*' + item.refuse))]), _c('image', {
        staticClass: ["handle-common-img"],
        attrs: {
          "src": "assets://icon_proxy_edit"
        }
      }), _c('text', {
        staticClass: ["handle-edit-text"],
        on: {
          "click": function($event) {
            _vm.jumpToRelease(item.id)
          }
        }
      }, [_vm._v("修改")])]) : _vm._e(), _c('div', {
        staticClass: ["separate"]
      })], 1) : _vm._e()
    }), (_vm.emptyIsShow[index] && items.length == 0) ? _c('cell', {
      key: 1,
      style: {
        height: _vm.height - 100 + 'px'
      },
    ...
    }, [_c('empty')], 1) : _vm._e(), (_vm.loading[index] && items.length > 1) ? _c('cell', {
      appendAsTree: true,
      attrs: {
        "append": "tree"
      }
      ...
      staticClass: ["loading-over-txt"]
    }, [_vm._v("已经全部加载完毕")]) : _vm._e()])]) : _vm._e()], 2)
  })), _c('wxc-mask', {
    attrs: {
      "height": "600",
      "width": "540",
  ...
  }, [_vm._v(_vm._s(_vm.refuse))]), _c('text', {
    staticClass: ["sure"],
    on: {
      "click": function($event) {
        _vm.closeOrderMask()
  ...
  }, [_vm._v("确定")])])])])], 1) : _vm._e(), (_vm.pass_status == 1) ? _c('div', {
    staticClass: ["redDot1"]
  }) : _vm._e(), (_vm.refuse_status == 1) ? _c('div', {
    staticClass: ["redDot2"]
  }) : _vm._e(), (_vm.slot == 1 && _vm.isiOS) ? _c('my-favorites') : _vm._e()], 1)], 1)
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["msg"]
  }, [_c('image', {
