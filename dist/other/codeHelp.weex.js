...
var _codeHelp = __webpack_require__(2);

var _codeHelp2 = _interopRequireDefault(_codeHelp);
...
_codeHelp2.default.el = '#root';
new Vue(_codeHelp2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\other\\codeHelp.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-8e637692"
...
exports.default = {
    ...
    methods: {
        sendCode: function sendCode() {
            var _this = this;
            if (_this.type == "") {
                promptModule.showPrompt({ msg: '请返回输入正确的手机号', type: 'prompt' });
                return;
            }
            promptModule.show({ title: '语音验证码',
                message: _this.mobile,
                type: 'voice',
                cancel: '不用了',
                confirm: '现在接听' }, function (ret) {
        ...
        voiceCode: function voiceCode() {
            ...
            _this.md5('danchuang' + areaCode, function (ret) {
                _this.mdverify = ret.md5;
                _this.Post(_this.api().get_voice_message, {
                    mobile: areaCode,
                    mdverify: _this.mdverify
                }, function (res) {
                    loadingModule.cancel(function (ret) {
                        var result = ret.result;
                        if (result == 'dismiss') {
                            if (res.data.code == '0') {
                                promptModule.showListener({ msg: '发送成功', type: 'success' }, function (res) {
                                    storage.setItem('send_voice', 'true', function (event) {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [(_vm.version) ? _c('title', {
    ...
    staticClass: ["text"]
  }, [_vm._v("1、请检查您的手机号输入是否正确，港澳台及海外手机请确认国际区号是否选择正确；输入澳洲手机号时，例如：0061282095500，您应该省略任何开头的0字，并选择国际区号（+61）")]), _c('text', {
    staticClass: ["text"]
  }, [_vm._v("2、如果安装了360卫士、安全管家、QQ管家等软件，请进入软件查询拦截记录，并将单创短信设为信任后重试；")]), _c('text', {
    staticClass: ["text"]
  }, [_vm._v("3、手机储存已满，机身内存不足。删掉部分短信或者照片等其他占用内存的文件即可。")]), _c('text', {
    staticClass: ["text"]
  }, [_vm._v("4、请确认您是否退订过（10690329157528）之类的短信，请联系运营商进行解除；")]), _c('text', {
    staticClass: ["text"]
  }, [_vm._v("5、您发送短信验证频率过快导致的，建议您隔一段时间后再重新尝试操作即可")]), _c('text', {
    staticClass: ["text"]
  }, [_vm._v("6、若按以上方法都无法获取验证码，请点击获取验证码获取验证码（语音验证码功能仅支持境内手机号，境外手机号暂不能使用该功能）")])]), _c('footer', {
    staticClass: ["footer"],
    on: {
