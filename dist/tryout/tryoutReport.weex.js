...
var _tryoutReport = __webpack_require__(2);

var _tryoutReport2 = _interopRequireDefault(_tryoutReport);

var _mixin = __webpack_require__(23);
...
_tryoutReport2.default.el = '#root';
new Vue(_tryoutReport2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\tryout\\tryoutReport.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-44ae3dfa"
...
module.exports = {
  ...
  "title-flex-content-alter": {
    "color": "#cf1126",
    "fontSize": "24",
    "lineHeight": "34",
    "borderBottomWidth": "2",
    "borderBottomColor": "#cf1126"
  },
  "list_values_two": {
    "width": "690",
    "flexDirection": "row",
...
var _wxcGridSelect = __webpack_require__(5);
...
var _titleBar = __webpack_require__(18);
...
var promptModule = weex.requireModule("promptModule");
var mytryout = new BroadcastChannel("mytryout");
var loadingModule = weex.requireModule('loadingModule');
var height = 0; //所能用到的最大高度
var bottom = 0;
exports.default = {
  components: { line: _line2.default, title: _titleBar2.default, WxcGridSelect: _wxcGridSelect2.default },
  data: function data() {
    return {
      title: "",
      height: height,
  ...
  methods: {
    ...
    getData: function getData() {
      ...
      this.Get(this.api().activity_report_detail, {
        id: this.id,
        rid: this.rid
      }, function (res) {
        if (res.data.code == "0") {
          res.data.data.base_info.forEach(function (element) {
            _this2.list_one.push({
              title: element.title,
              content: element.content
            });
          });
          _this2.cover = res.data.data.cover;
          if (_this2.cover.length == 9) {
            _this2.addImgShow = false;
          }
          _this2.content = res.data.data.content;
          _this2.goodsImg = res.data.data.image;
          _this2.score = res.data.data.score;
          _this2.score_tips = res.data.data.score_tips;
          for (var i = 0; i < _this2.list_one.length; i++) {
            for (var j = 0; j < _this2.list_one[i].content.length; j++) {
              var checkval = _this2.list_one[i].content[j].checkval.toString();
              _this2.base_info.push({
                id: _this2.list_one[i].content[j].id,
                checkval: _this2.list_one[i].content[j].checkval.toString(),
                is_statistic: _this2.list_one[i].content[j].is_statistic
              });
            }
          }
        } else {
    ...
    //单选事件
    listOneCheckedSingle: function listOneCheckedSingle(id, index, is_statistic) {
      for (var i = 0; i < this.list_one.length; i++) {
        for (var j = 0; j < this.list_one[i].content.length; j++) {
          if (id == this.list_one[i].content[j].id) {
            Vue.set(this.list_one[i].content[j].checkval, 0, index);
            var value = this.list_one[i].content[j].checkval.toString();
            this.addValue(id, value, is_statistic);
          }
        }
      }
    },
    ...
    //多选事件
    listOneCheckedMany: function listOneCheckedMany(id, index, is_statistic) {
      for (var i = 0; i < this.list_one.length; i++) {
        for (var j = 0; j < this.list_one[i].content.length; j++) {
          if (id == this.list_one[i].content[j].id) {
            if (this.list_one[i].content[j].checkval.toString().indexOf(index) == -1) {
              this.list_one[i].content[j].checkval.push(index);
            } else {
              for (var k = 0; k < this.list_one[i].content[j].checkval.length; k++) {
                if (this.list_one[i].content[j].checkval[k] == index) {
                  this.list_one[i].content[j].checkval.splice(k, 1);
                }
              }
            }
            var value = this.list_one[i].content[j].checkval.toString();
    ...
    //添加传值
    addValue: function addValue(i, v, s) {
      var _this3 = this;
      ...
      if (this.base_info.length == 0) {
        ...
        var j = 0;
        this.base_info.forEach(function (element) {
          if (element.id == i) {
            element.checkval = v;
          } else {
            j++;
            if (j == _this3.base_info.length) {
              _this3.base_info.push({
                id: i,
                checkval: v,
                is_statistic: s
              });
    ...
    addImg: function addImg() {
      var _this = this;
      var titlesDict = { caram: "拍照", photoFile: "从手机相册选择" };
      comModal.uploadPhoto({ titlesDict: titlesDict }, function (data) {
        if (data.url) {
          _this.cover.push({
            id: data.id,
            src: data.url
        ...
        if (_this.cover.length == 9) {
          _this.addImgShow = false;
        }
      });
    },

    //删除图片
    delImg: function delImg(id) {
      for (var i = 0; i < this.cover.length; i++) {
        if (id == this.cover[i].id) {
          this.cover.splice(i, 1);
        }
      }
      if (this.cover.length < 9) {
        this.addImgShow = true;
      }
    },
    isValueParams: function isValueParams() {
      for (var i = 0; i < this.base_info.length; i++) {
        if (this.base_info[i].checkval == "") {
          promptModule.showPrompt({ msg: "请全部填写", type: "prompt" });
          return false;
          break;
        }
      }
    ...
    submit: function submit() {
      var coverArray = [];
      this.cover.forEach(function (element) {
        coverArray.push(element.id);
      });
      var coverString = coverArray.toString();
      if (!this.isValueParams()) return;
      if (this.content.length < 100) {
        promptModule.showPrompt({ msg: "使用感受必须大于等于100字", type: "prompt" });
        return;
      }
      this.base_infoString = JSON.stringify(this.base_info);
      loadingModule.show({ msg: '正在提交...' });
      var self = this;
      self.Post(self.api().activity_report_save, {
        id: self.id,
        rid: self.rid,
        score: self.score,
        base_info: self.base_infoString,
        ...
        loadingModule.cancel(function (ret) {
          ...
          if (result == 'dismiss') {
            if (res.data.code == "0") {
              mytryout.postMessage(self.eventBusTag().REFRESH_TRYOUT);
              promptModule.showListener({ msg: "报告提交成功，请耐心等待审核", type: "success" }, function (ret) {
                self.popBar();
              });
            } else {
              promptModule.showPrompt({ msg: res.data.msg, type: "err" });
    ...
    //跳转如何填写试用报告
    click_tryoutHow: function click_tryoutHow() {
      this.jumpTo2("tryout/tryoutHow", "", {});
    },
    getGrades: function getGrades(e) {
      this.score = e.value;
      switch (e.value) {
        case "1":
          this.score_tips = "差";
  ...
  created: function created() {
    ...
    if (weex.config.env.platform.toLowerCase() == "android") {
      this.textReportStyle = {
        minHeight: 100 + "px",
        maxHeight: 150 + "px"
      };
    } else {
      this.textReportStyle = { height: "245px" };
    }
    this.id = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, "id"));
    this.rid = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, "rid"));
    this.title = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, "title"));
    this.getData();
    // promptModule.showPrompt({ msg: this.id + "," + this.rid, type: "err" });
  },
...
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-ebb74560"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-3b43cb32"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
  ...
  }, [_vm._v("亲爱的ABM用户")]), _c('image', {
    staticClass: ["img_content"],
    attrs: {
      "src": "http://static-daitao.t.abmau.com/assets/img/content.png"
    }
  }), _c('div', {
    staticClass: ["text_line"]
  }), _c('div', {
    staticClass: ["goodsGrade_view"]
  }, [_c('image', {
    ...
    staticClass: ["goods_grade_bg"]
  }, [_c('div', {
    staticClass: ["goods_star"]
  }, [_c('text', {
    ...
    on: {
      "getGrades": _vm.getGrades
    }
  })], 1), _c('text', {
    staticClass: ["goods_grade_level"]
  }, [_vm._v(_vm._s(_vm.score_tips))])])]), _c('div', {
    staticClass: ["interval"]
  }), _vm._l((_vm.list_one), function(item, i) {
    return _c('div', {
      key: i,
      staticClass: ["list"]
    }, [_c('text', {
      staticClass: ["list_title"]
    }, [_vm._v(_vm._s(item.title))]), _vm._l((item.content), function(items, j) {
      return _c('div', {
        key: j,
        staticClass: ["list_option"]
      }, [_c('text', {
        staticClass: ["list_name"]
      }, [_vm._v(_vm._s(items.name))]), (items.style_type == 3) ? _c('div', {
        staticClass: ["list_values"]
      }, _vm._l((items.content), function(itemss, k) {
        return _c('div', {
          key: k,
          staticClass: ["list_value"],
          on: {
            "click": function($event) {
              _vm.listOneCheckedSingle(items.id, k, items.is_statistic)
            }
          }
        }, [(_vm.isInArray(items.checkval, k)) ? _c('image', {
          staticClass: ["list_icon"],
          attrs: {
            "src": "assets://icon_report_selected"
          }
        }) : _vm._e(), (!_vm.isInArray(items.checkval, k)) ? _c('image', {
          staticClass: ["list_icon"],
          attrs: {
            "src": "assets://icon_report_unselected"
          }
        }) : _vm._e(), _c('text', {
          staticClass: ["list_text"]
        }, [_vm._v(_vm._s(itemss))])])
      })) : _vm._e(), (items.style_type == 2) ? _c('div', {
        staticClass: ["list_values_two"]
      }, _vm._l((items.content), function(itemss, k) {
        return _c('div', {
          key: k,
          on: {
        ...
        }, [(_vm.isInArray(items.checkval, k)) ? _c('text', {
          staticClass: ["list_text_two", "checked"]
        }, [_vm._v(_vm._s(itemss))]) : _vm._e(), (!_vm.isInArray(items.checkval, k)) ? _c('text', {
          staticClass: ["list_text_two"]
        }, [_vm._v(_vm._s(itemss))]) : _vm._e()])
      })) : _vm._e()])
    })], 2)
  }), _c('div', {
    staticClass: ["interval"]
  }), _c('div', {
    staticClass: ["list_title_height", "row", "between"]
  }, [_c('text', {
    staticClass: ["list_title_title"]
  }, [_vm._v("您的使用感受")]), _c('text', {
    staticClass: ["list_title_two"],
    on: {
      "click": function($event) {
        _vm.click_tryoutHow()
      }
    }
  }, [_vm._v("试用报告如何填写？")])]), _c('line'), _c('textarea', {
    staticClass: ["list_textarea"],
    style: _vm.textReportStyle,
    attrs: {
      "rows": "8",
      "placeholder": "你对本次试用完的商品有哪些建议比如：包装，味道，使用方法，商品量等自由填写（100字以上）",
      "value": (_vm.content)
    },
    on: {
  ...
  }, [_vm._l((_vm.cover), function(item, i) {
    return _c('div', {
      key: i,
      staticClass: ["images_upload"]
    }, [_c('image', {
      staticClass: ["images"],
      attrs: {
        "src": item.src
      ...
      staticClass: ["images_close"],
      attrs: {
        "src": "assets://icon_tryout_close"
      },
      on: {
        "click": function($event) {
          _vm.delImg(item.id)
    ...
    staticClass: ["add_img_image"],
    attrs: {
      "src": "assets://icon_comera"
    }
  })]) : _vm._e()], 2), _c('line', {
