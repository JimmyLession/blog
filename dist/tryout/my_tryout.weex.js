...
var _my_tryout = __webpack_require__(2);

var _my_tryout2 = _interopRequireDefault(_my_tryout);
...
_my_tryout2.default.el = '#root';
new Vue(_my_tryout2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\tryout\\my_tryout.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-176c7c9a"
...
module.exports = {
  ...
  "item_bottom_text_div": {
    "flexDirection": "row",
    "justifyContent": "flex-end"
  },
  "item_bottom_text_one": {
    "fontSize": "26",
    "paddingRight": "20"
  },
  "item_bottom_text_two": {
    "width": "160",
    "height": "46",
    "lineHeight": "46",
    "textAlign": "center",
  ...
  "item_bottom_text_refuse_images": {
    "flexDirection": "column",
    "justifyContent": "center"
  },
  "item_bottom_text_refuse_image": {
    "width": "25",
    "height": "25",
    "marginRight": "5"
  },
  "item_bottom_text_refuse_text": {
    "lineHeight": "46",
    "fontSize": "24",
    "color": "#707070"
  },
  ...
  "refuses_div_title_images": {
    "width": "460",
    "height": "80",
    "marginTop": "50"
  },
  "refuses_div_title_image": {
    "width": "80",
    "height": "80",
    "borderRadius": "40"
  },
  "refuses_div_title_texts": {
    "width": "460",
    "minHeight": "150",
    "marginTop": "50"
  },
  "refuses_div_title_text": {
    "width": "400",
    "fontSize": "30",
    "color": "#333333",
    "textAlign": "center"
...
var _titleBar = __webpack_require__(13);
...
var tryoutdetail = new BroadcastChannel("tryoutdetail");
var refresh = new BroadcastChannel("refresh_try");
var height = 0; //所能用到的最大高度
var bottom = 0;
exports.default = {
  components: { refresher: _refresh2.default, empty: _empty2.default, title: _titleBar2.default },
  data: function data() {
    return {
      height: height,
      bottom: bottom,
  ...
  methods: {
    ...
    //跳转订单详情
    click_orderDetail: function click_orderDetail(item) {
      var _this2 = this;

      titleBar.jumpToOrderDetail({ order_id: item.order_id }, function (res) {
        tryoutdetail.postMessage(_this2.eventBusTag().REFRESH_CORE);
        if (res.result == "del") {
          _this2.loadingDown();
    ...
    //跳转报告
    click_report: function click_report(i, ri, name) {
      ...
      tryoutdetail.postMessage(this.eventBusTag().REFRESH_CORE);
      this.jumpTo2("tryout/tryoutReport", "", { id: i, rid: ri, title: name });
    },

    //查看报告
    click_see_report: function click_see_report(i, ri) {
      this.jumpTo2("tryout/seeTryoutReport", "", { id: i, rid: ri });
    },

    //跳转商品详情
    click_detail: function click_detail(i) {
      this.jumpTo2("tryout/tryDetail", "", { id: i });
    },
    ...
    getData: function getData(type) {
      ...
      this.Get(this.api().activity_apply, {
        limit: this.limit,
        offset: this.offset
      }, function (res) {
        if (res.data.code == "0") {
          ...
          if (type == 1) {
            res.data.data.forEach(function (element) {
              _this3.data.push(element);
            });
          } else if (type == 2) {
            _this3.data = res.data.data;
          }
          _this3.url = res.data.pay_url;
          _this3.offset += _this3.limit;
          if (res.data.count > _this3.offset) {
            _this3.isloading = false;
            _this3.allLoadingFinish = false;
          } else {
            _this3.isloading = true;
            _this3.allLoadingFinish = true;
          }
        } else {
    ...
    //确认收货
    click_collect_goods: function click_collect_goods(oid) {
      var _this4 = this;
      ...
      promptModule.show({
        ...
        if (result == "success") {
          _this4.Get(_this4.api().confirm_order, {
            order_id: oid
          }, function (res) {
            if (res.data.code == "0") {
              tryoutdetail.postMessage(_this4.eventBusTag().REFRESH_CORE);
              _this4.loadingDown();
    ...
    //继续付款
    click_payment: function click_payment(b_order_ids) {
      this.jumpTo2("order/payOrder", "", {
        b_order_ids: b_order_ids,
        type: "tryout_continue"
      });
    },

    //查看拒绝原因
    refusesFun: function refusesFun(r) {
      var _this5 = this;

      this.Get(this.api().activity_Refuse, {
        rid: r
      }, function (res) {
        if (res.data.code == "0") {
          _this5.refusesText = res.data.data.desc;
          _this5.refusesShow = !_this5.refusesShow;
        } else {
          promptModule.showPrompt({ msg: res.data.msg, type: "err" });
        }
      });
  ...
  created: function created() {
    ...
    _this.isIpx(_this);
    _this.getData(1);
    _this.entryType = this.GetUrlPara(weex.config.bundleUrl, "entryType");
    mytryout.onmessage = function (event) {
      if (!event.data) {
        return;
      }
      switch (event.data) {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
    ...
    return _c('cell', {
      ...
      staticClass: ["item_state_text"]
    }, [_vm._v("订单号：" + _vm._s(item.sn))]), _c('div', {
      staticClass: ["item_state_copydiv"]
    }, [_c('text', {
      staticClass: ["item_state_copy"],
      on: {
      ...
      on: {
        "click": function($event) {
          _vm.click_detail(item.id)
        }
      }
    }, [_c('div', {
      staticClass: ["item_left"]
    }, [_c('image', {
      staticClass: ["item_left_iamge"],
      attrs: {
        "src": item.image
      }
    })]), _c('div', {
      ...
      staticClass: ["item_right_name"]
    }, [_vm._v(_vm._s(item.name))]), _c('div', {
      staticClass: ["item_right_bottom"]
    }, [_c('text', {
      staticClass: ["item_right_price"]
    }, [_vm._v("¥" + _vm._s(item.market_price))]), _c('text', {
      staticClass: ["item_right_num"]
    }, [_vm._v("x 1")])])])]), _c('div', {
      staticClass: ["item_bottom"]
    }, [_c('div', {
      staticClass: ["item_bottom_text_div"]
    }, [_c('text', {
      staticClass: ["item_bottom_text_one", "colorTwo"]
    }, [_vm._v("共1件商品")]), _c('text', {
      staticClass: ["item_bottom_text_one", "colorOne"]
    }, [_vm._v("合计  ¥" + _vm._s(item.amount))])]), _c('div', {
      staticClass: ["row"]
    }, [_c('div', {
      staticClass: ["item_bottom_text_refuses"]
    }, [(item.status == 3) ? _c('div', {
      staticClass: ["item_bottom_text_refuse"]
    }, [_vm._m(0, true), _c('text', {
      staticClass: ["item_bottom_text_refuse_text"],
      on: {
        "click": function($event) {
          _vm.refusesFun(item.rid)
        }
      }
    }, [_vm._v("查看拒绝原因")])]) : _vm._e()]), _c('div', {
      staticClass: ["item_bottom_text_div", "fun"]
    }, [_c('text', {
      staticClass: ["item_bottom_text_two", "see"],
      on: {
        "click": function($event) {
          _vm.click_orderDetail(item)
        }
      }
    }, [_vm._v("订单详情")]), ((item.dstatus == 4 || item.dstatus == 1) && !item.rid) ? _c('text', {
      staticClass: ["item_bottom_text_two", "write"],
      on: {
        "click": function($event) {
          _vm.click_report(item.id, '', '填写试用报告')
        }
      }
    }, [_vm._v("撰写报告")]) : _vm._e(), ((item.dstatus == 4 || item.dstatus == 1) && (item.status == 3 || item.status == 1)) ? _c('text', {
      staticClass: ["item_bottom_text_two", "write"],
      on: {
        "click": function($event) {
          _vm.click_report(item.id, item.rid, '修改试用报告')
        }
      }
    }, [_vm._v("修改报告")]) : _vm._e(), (item.status == 2) ? _c('text', {
      staticClass: ["item_bottom_text_two", "write"],
      on: {
        "click": function($event) {
          _vm.click_see_report(item.id, item.rid)
        }
      }
    }, [_vm._v("查看报告")]) : _vm._e(), (item.dstatus == 0) ? _c('text', {
      staticClass: ["item_bottom_text_two", "write"],
      on: {
        "click": function($event) {
          _vm.click_payment(item.b_order_ids, item.id)
        }
      }
    }, [_vm._v("继续付款")]) : _vm._e(), (item.dstatus == 3) ? _c('text', {
      staticClass: ["item_bottom_text_two", "write"],
      on: {
        "click": function($event) {
          _vm.click_collect_goods(item.order_id)
        }
      }
    }, [_vm._v("确认收货")]) : _vm._e()])])]), _c('div', {
  ...
  }, [_c('empty')], 1) : _vm._e(), (_vm.data.length > 3) ? _c('cell', {
    ...
    staticClass: ["loading-over-txt"]
  }, [_vm._v("已经全部加载完毕")]) : _vm._e()]) : _vm._e(), _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [(_vm.refusesShow) ? _c('div', {
    staticClass: ["refuses_model"]
  }, [_c('div', {
    staticClass: ["refuses_div"]
  }, [_c('div', {
    staticClass: ["refuses_div_close"]
  }, [_c('image', {
    staticClass: ["refuses_div_close_image"],
    attrs: {
      "src": "assets://icon_ic_finish"
    },
    on: {
      "click": function($event) {
        _vm.refusesShow = !_vm.refusesShow
      }
    }
  })]), _vm._m(1), _c('div', {
    staticClass: ["refuses_div_title_texts", "row", "center"]
  }, [_c('text', {
    staticClass: ["refuses_div_title_text"]
  }, [_vm._v(_vm._s(_vm.refusesText))])]), _c('div', {
  ...
  }, [_vm._v("确定")])])])]) : _vm._e()])], 2)], 1)
},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["item_bottom_text_refuse_images"]
  }, [_c('image', {
    staticClass: ["item_bottom_text_refuse_image"],
    attrs: {
  ...
  return _c('div', {
    staticClass: ["refuses_div_title_images", "row", "center"]
  }, [_c('image', {
    staticClass: ["refuses_div_title_image"],
    attrs: {
      "src": "assets://icon_cancal_pop"
    }
  })])
}]}
module.exports.render._withStripped = true
