...
var _complainKF = __webpack_require__(2);

var _complainKF2 = _interopRequireDefault(_complainKF);
...
_complainKF2.default.el = '#root';
new Vue(_complainKF2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\suggestion\\complainKF.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-0bf84662"
...
var _chooseItem = __webpack_require__(17);
...
var event = new BroadcastChannel('feedbackcomplainKF');
var height = 0;
var bottom = 0;
exports.default = {
    components: { titleBar: _titleBar2.default, line: _line2.default, empty: _empty2.default, chooseItem: _chooseItem2.default },
    data: function data() {
        return {
            isParam: false,
            height: height,
            ...
            nicknameList: '', //昵称选择类型
            cover: [], //上传图片
            textareaContent: '', //补充文字说明
            complaintType: '', //客服投诉原因  number类型
            respondentId: '', //被投诉客服id
            addImgShow: true,
            chooseMask: false,
            isChooseReson: false,
            chooseResonText: '点击选择',
            chooseNicknameText: '点击选择',
            chooseResonTextColor: false,
            chooseNicknameTextColor: false
        };
    },
    ...
    methods: {
        inputContent: function inputContent(e) {
            ...
            this.isParam = this.isValueParam(this, [this.textareaContent]);
        },
        ...
        submit: function submit() {
            var self = this;
            if (!self.complaintType) {
                promptModule.showPrompt({ msg: "请选择投诉原因", type: "err" });
                return;
            }
            if (!self.respondentId) {
                promptModule.showPrompt({ msg: "请选择客服昵称", type: "err" });
                return;
            }
            if (!self.textareaContent) {
                promptModule.showPrompt({ msg: "请输入补充说明", type: "err" });
                return;
            }
            if (self.textareaContent.length > 200) {
                promptModule.showPrompt({ msg: "请输入200字以内", type: "err" });
                return;
            }
            if (self.cover.length == 0) {
                promptModule.showPrompt({ msg: "请上传图片", type: "err" });
                return;
            }
            var coverString = JSON.stringify(self.cover);
            self.Post(self.api().complaintCustomer, {
                type: 0,
                complaint_type: self.complaintType,
                content: self.textareaContent,
                album: coverString,
                respondent_id: self.respondentId
            }, function (res) {
                if (res.data.code == "0") {
                    promptModule.showListener({ msg: "投诉提交成功，请耐心等待处理", type: "success" }, function (ret) {
                        event.postMessage(self.eventBusTag().COMPLAINKF);
                        self.popBar();
                    });
                } else {
        ...
        complainReason: function complainReason() {
            //投诉原因
            // promptModule.showPrompt({ msg: 1, type: "err" });
            utilModule.hideKeyBorad();
            this.isChooseReson = true;
            // this.items=[
            //     {title:"态度较差",select:false},
            //     {title:"答复缓慢",select:false},
            //     {title:"处理不及时",select:false},
            //     {title:"未解决问题",select:false},
            //     {title:"其它原因",select:false}
            // ];
            this.items = this.reasonList;
            if (this.chooseResonText != "点击选择") {
                for (var i = 0; i < this.items.length; i++) {
                    if (this.items[i].title == this.chooseResonText) {
            ...
            this.chooseMask = true;
        },
        nickname: function nickname() {
            //选择昵称
            utilModule.hideKeyBorad();
            this.isChooseReson = false;
            // this.items=[
            //     {title:"啦啦",select:false},
            //     {title:"多多",select:false},
            //     {title:"哈哈",select:false},
            //     {title:"么么",select:false},
            //     {title:"意义",select:false},
            //     {title:"妹妹",select:false},
            // ];
            this.items = this.nicknameList;
            if (this.chooseNicknameText != "点击选择") {
                for (var i = 0; i < this.items.length; i++) {
                    if (this.items[i].title == this.chooseNicknameText) {
                        this.items[i].select = true;
                    } else {
                        this.items[i].select = false;
                    }
        ...
        closeChooseMask: function closeChooseMask() {
            //关闭弹框
            this.chooseMask = false;
        },
        getItemValue: function getItemValue(val) {
            //获取item的值
            if (this.isChooseReson) {
                this.chooseResonText = val.title;
                this.chooseResonTextColor = val.select;
                this.complaintType = val.id;
            } else {
                this.chooseNicknameText = val.title;
                this.chooseNicknameTextColor = val.select;
                this.respondentId = val.id;
            }
            this.chooseMask = false;
        },
        getReasonList: function getReasonList() {
            //获取投诉原因的列表
            var self = this;
            self.Get(self.api().complaint, {}, function (res) {
                if (res.data.code == 0) {
                    // console.log("res.data.data.complaint:   "+res.data.data.complaint);
                    self.reasonList = res.data.data.complaint;
                    for (var i = 0; i < self.reasonList.length; i++) {
                        if (self.reasonList[i].title == self.chooseResonText) {
                            self.complaintType = self.reasonList[i].id;
                        }
                    }
                } else {
                    promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
        ...
        getNicknameList: function getNicknameList() {
            //获取客服昵称的列表
            var self = this;
            self.Get(self.api().nickName, {}, function (res) {
                if (res.data.code == 0) {
                    // console.log("res.data.data.customer:   "+res.data.data.customer);
                    self.nicknameList = res.data.data.customer;
                } else {
                    promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                }
            });
    ...
    created: function created() {
        ...
        this.isIpx(this);
        var self = this;
        if (self.GetUrlPara(weex.config.bundleUrl, 'reason')) {
            self.chooseResonText = self.GetUrlPara(weex.config.bundleUrl, 'reason');
            self.chooseResonTextColor = self.GetUrlPara(weex.config.bundleUrl, 'select');
        }
        self.getReasonList();
        self.getNicknameList();
    }
};
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\components\\chooseItem.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-6ddf289b"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
    ...
    methods: {
        ...
        chooseItem: function chooseItem(item) {
            //选择item
            item.select = true; //把当前选中的title作为选中状态
            this.$emit('getItemValue', item);
        },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    return _c('cell', {
      ...
      class: [item.select ? 'item-text2' : 'item-text'],
      on: {
        "click": function($event) {
          _vm.chooseItem(item)
        }
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    staticClass: ["complain-reason", "common-style"],
    appendAsTree: true,
    attrs: {
      "append": "tree"
    },
    ...
    class: [_vm.chooseResonTextColor ? 'choose-text2' : 'choose-text']
  }, [_vm._v(_vm._s(_vm.chooseResonText))]), _c('image', {
    staticClass: ["next"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
  })])]), _c('cell', {
    staticClass: ["nickname", "common-style"],
    appendAsTree: true,
    attrs: {
      "append": "tree"
    },
    ...
    class: [_vm.chooseNicknameTextColor ? 'choose-text2' : 'choose-text']
  }, [_vm._v(_vm._s(_vm.chooseNicknameText))]), _c('image', {
    staticClass: ["next"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
  })])]), _c('cell', {
    staticClass: ["resoan-head"],
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('text', {
    staticClass: ["resoan-head-text"]
  }, [_vm._v("补充说明(200字以内)")]), _c('textarea', {
    ...
    on: {
      "input": function($event) {
        _vm.textareaContent = $event.target.attr.value
      }
    }
  }), _c('div', {
    ...
    staticClass: ["sure-btn-text"],
    on: {
      "click": function($event) {
        _vm.submit()
      }
    }
  }, [_vm._v("提交")])]), (_vm.chooseMask) ? _c('choose-item', {
    attrs: {
      "data": _vm.items
    },
    on: {
      "close": _vm.closeChooseMask,
      "getItemValue": _vm.getItemValue
    }
  }) : _vm._e()], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
