...
var _webMovieView = __webpack_require__(2);

var _webMovieView2 = _interopRequireDefault(_webMovieView);
...
_webMovieView2.default.el = '#root';
new Vue(_webMovieView2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\web\\webMovieView.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-31b6fa66"
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            sharetitle: '',
            sharedesc: '',
            rightButtonTxt: '', //导航右按钮文字
            attachment_url: '' //规则详情图片下载
        };
    },
    ...
    methods: {
        ...
        wxcMinibarRightButtonClicked: function wxcMinibarRightButtonClicked() {
            var _this = this;
            var webTitle = _this.title;
            if (webTitle == '规则详情') {
                var attachment_url = _this.attachment_url;
                utilModule.saveImage({ 'image': [attachment_url] });
            } else {
                utilModule.share({
                    'share_type': 'share_goods',
                    'title': _this.sharetitle,
                    'desc': _this.sharedesc,
                    'webUrl': _this.sharelink,
    ...
    created: function created() {
        ...
        this.isIpx(this);
        this.url = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'url'));
        this.title = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'title'));
        this.sharelink = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'sharelink'));
        this.shareimg = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'shareimg'));
        this.sharetitle = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'sharetitle'));
        this.sharedesc = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'sharedesc'));
        this.attachment_url = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, 'saveImg'));

        if (this.title == '规则详情' && this.attachment_url != '' && this.attachment_url != 'undefined' && this.attachment_url != 'null') {
            console.log('--------------规则详情图片Url========>:' + this.attachment_url);
            this.rightButtonTxt = '保存';
        } else if (this.sharelink == 'undefined' || this.sharelink == null || this.shareimg == 'undefined' || this.shareimg == null) {
            console.log('-------------- banner广告 NONONO========>:' + this.sharelink + ',img:' + this.shareimg);
            this.rightButtonImg = '';
        } else if (this.sharelink != '' && this.shareimg != '') {
            console.log('-------------- banner广告 分享有链接========>:' + this.sharelink + ',img:>>>>' + this.shareimg);
            this.rightButtonImg = 'http://img.abmau.com/abmau/32cd222499b157f8697480045155b334.png';
        } else {
            console.log('-------------- banner广告 分享NoLink========>:');
            this.rightButtonImg = '';
        }
    }
};
