...
var _search_goods = __webpack_require__(2);

var _search_goods2 = _interopRequireDefault(_search_goods);
...
_search_goods2.default.el = '#root';
new Vue(_search_goods2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\home\\search_goods.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-7c8c2962"
...
exports.default = {
  ...
  methods: {
    selectTitleJump: function selectTitleJump(ret) {
      var _this = this;
      if (ret == '') {
        _this.jumpTo2("classify/search", "", { keyword: ret });
      } else {
        _this.lately_data.push(ret);
        _this.lately_data = Array.from(new Set(_this.lately_data));
        storage.setItem('search_goods_data', JSON.stringify(_this.lately_data), function (event) {
    ...
    didSelectdCatrgory: function didSelectdCatrgory(id) {
      this.jumpTo2('home/skinCare', '', { id: id });
    },
    fetch: function fetch() {
      var _this = this;
      _this.Get(_this.api().hot_search_new, {}, function (res) {
        if (res.data.code == '0') {
          _this.guessseek_data = res.data.data.hotSearch;
          _this.category_data = res.data.data.category;
        } else promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
      });
    },
    onSubmit: function onSubmit(e) {
      var value = e.value;
      if (value == '' && this.keyword != '搜索商品名称') value = this.keyword;
      this.selectTitleJump(value);
    },
    cleanLocalCacheData: function cleanLocalCacheData() {
      var _this = this;
      storage.setItem('search_goods_data', "", function (event) {
        if (event.result == "success") {
          _this.lately_data = [];
        }
      });
  ...
  created: function created() {
    ...
    _this.keyword = decodeURIComponent(_this.GetUrlPara(weex.config.bundleUrl, "keyword"));
    if (_this.keyword == '') _this.keyword = '搜索商品名称';
    _this.fetch();
    storage.getItem('search_goods_data', function (event) {
      if (event.result == "success") {
        _this.lately_data = JSON.parse(event.data);
      }
    });
  }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
  ...
  }, [_vm._v("关闭")])]), _c('line')], 1), (_vm.lately_data.length != 0) ? _c('div', {
    staticClass: ["item"],
    staticStyle: {
      paddingLeft: "30px"
    }
    ...
    staticClass: ["item_title_right"],
    on: {
      "click": function($event) {
        _vm.cleanLocalCacheData()
      }
    }
  }, [_vm._v("清除")])]), (_vm.lately_data.length != 0) ? _c('div', {
    staticClass: ["row", "wrap"]
  }, _vm._l((_vm.lately_data), function(item, i) {
    return _c('div', {
      key: i,
      staticClass: ["item_content"],
      on: {
        "click": function($event) {
          _vm.selectTitleJump(item)
        }
      }
    }, [_c('text', {
      staticClass: ["item_content_text"]
    }, [_vm._v(_vm._s(item))])])
  })) : _vm._e()]) : _vm._e(), (_vm.lately_data.length != 0) ? _c('div', {
    staticStyle: {
      height: "20px"
    }
  }) : _vm._e(), (_vm.guessseek_data.length != 0) ? _c('div', {
    staticClass: ["item-div"],
    staticStyle: {
      paddingLeft: "30px"
    }
  }, [_vm._m(0), (_vm.guessseek_data.length != 0) ? _c('div', {
    staticClass: ["row", "wrap"]
  }, _vm._l((_vm.guessseek_data), function(item, i) {
    return _c('div', {
      key: i,
      staticClass: ["item_content"],
      on: {
  ...
  })) : _vm._e()]) : _vm._e(), (_vm.guessseek_data.length != 0) ? _c('div', {
    staticStyle: {
      height: "20px"
    }
  }) : _vm._e(), (_vm.category_data.length != 0) ? _c('div', {
    staticClass: ["item-div"]
  }, [_vm._m(1), (_vm.category_data.length != 0) ? _c('div', {
    staticClass: ["row", "wrap"]
  }, _vm._l((_vm.category_data), function(item, i) {
    return _c('div', {
      key: i,
      staticClass: ["item_bottom_content-wap"]
    }, [_c('div', {
      staticClass: ["item_bottom_content"],
      on: {
        "click": function($event) {
          _vm.didSelectdCatrgory(item.id)
        }
      }
    }, [_c('text', {
      staticClass: ["item_bottom_content_text"]
