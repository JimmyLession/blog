...
var _starMedia = __webpack_require__(4);

var _starMedia2 = _interopRequireDefault(_starMedia);
...
_starMedia2.default.el = '#root';
new Vue(_starMedia2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\home\\starMedia.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-e0c579dc"
...
var _starMediaConfig = __webpack_require__(24);

var _starMediaConfig2 = _interopRequireDefault(_starMediaConfig);
...
exports.default = {
    ...
    data: function data() {
        return {
            ...
            tabTitles: [],
            loading: [],
            tabStyles: _starMediaConfig2.default.tabStyles,
            supportSlide: false,
            isTabView: true,
            needSlider: false,
            empty: [],
    ...
    methods: {
        ...
        requestToGetTabTittles: function requestToGetTabTittles() {
            var _this = this;
            _this.Get(_this.api().newStarcategory, {}, function (res) {
                if (res.data.code != '0') {
                    //请求数据失败
                    promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                } else {
                    var maxLen = 0;
                    if (res.data.data.starcategory.data && res.data.data.starcategory.data.length > 0) {
                        for (var arr in res.data.data.starcategory.data) {
                            _this.loading.push(false);
                            _this.empty.push(false);
                            _this.allLoadingFinish.push(false);
                            _this.tabTitles.push({ 'title': res.data.data.starcategory.data[arr].name });
                            _this.tabArray.push({ 'cid': res.data.data.starcategory.data[arr].cid });
                            var tit = res.data.data.starcategory.data[arr].name;
                            var curLen = tit.length;
                            if (curLen > maxLen) {
                                maxLen = curLen;
                            }
                            _this.page.push(1);
                        ...
                        var count = _this.tabList.length;
                        if (count == 1) {
    ...
    created: function created() {
        ...
        this.title = this.GetUrlPara(weex.config.bundleUrl, 'title');
        //获取分类Tittle
  ...
  return _c('div', [_c('title', {
    ...
    return _c('list', {
      ...
      return _c('cell', {
        ...
        on: {
          "click": function($event) {
            _vm.startWeb(item.title, item.url, item.sharelink, item.shareimg, item.name, item.sharecontent)
          }
        }
      }, [_c('image', {
      ...
      }, [_vm._v("查看详情>")])])]), (y != (v.length - 1)) ? _c('div', {
        staticClass: ["line-item"]
      }) : _vm._e()])
    }), (_vm.loading[index] && v.length > 6) ? _c('cell', {
      appendAsTree: true,
      attrs: {
        "append": "tree"
      }
