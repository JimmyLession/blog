...
var _home_newProduct = __webpack_require__(2);

var _home_newProduct2 = _interopRequireDefault(_home_newProduct);
...
_home_newProduct2.default.el = '#root';
new Vue(_home_newProduct2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\home\\home_newProduct.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-269d5f22"
...
exports.default = {
    ...
    methods: {
        ...
        onloading: function onloading() {
            if (!this.loading && this.newproduct.data.length < this.size) return;
            this.fetch();
        },

        //跳转商品详情
        jumpGD: function jumpGD(id, url) {
            if (this.compareVersion(weex.config.env.appVersion, '3.4.0')) {
                this.jumpTo2('shop/goodsDetailsParent', '', { goods_id: id });
            } else if (this.compareVersion(weex.config.env.appVersion, '3.0.7')) this.jumpTo2('shop/goodsDetails', '', { goods_id: id });else titleBar.jumpGD({ id: id, url: url });
        },
        fetch: function fetch() {
            var _this = this;
            _this.Get(_this.api().home_newproduct, {
                page: _this.page,
                size: _this.size
            }, function (res) {
                if (res.data.code == '0') {
                    _this.newproduct.title = res.data.data.newproduct.title;
                    if (_this.page == 1) {
                        _this.newproduct.data = res.data.data.newproduct.data;
                    } else {
                        for (var arr in res.data.data.newproduct.data) {
                            _this.newproduct.data.push(res.data.data.newproduct.data[arr]);
                        }
                    }
                    _this.totalnum = res.data.data.totalnum;
                    _this.total_page = _this.totalnum / _this.size + (_this.totalnum % _this.size > 0 ? 1 : 0);
                    _this.increment();
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
  ...
  }, _vm._l((_vm.newproduct.data), function(v, index) {
    return (_vm.newproduct.data.length != 0) ? _c('div', {
      key: index,
      staticClass: ["recommend"]
    }, [(!(v.length > 1 && v[index - 1].time == v[index].time)) ? _c('div', {
      staticClass: ["item_time"]
    }, [_c('text', {
      staticClass: ["time_text"]
    }, [_vm._v(_vm._s(v.time))])]) : _vm._e(), (v.data.length == 1) ? _c('div', {
      staticClass: ["hor"]
    }, _vm._l((v.data), function(item, i) {
      return _c('div', {
        key: i,
        ref: "i",
        refInFor: true,
    ...
    })) : _vm._e(), (v.data.length > 1) ? _c('div', {
      staticClass: ["box"]
    }, _vm._l((v.data), function(item, y) {
      return _c('div', {
        key: y,
        ref: "y",
        refInFor: true,
  ...
  })), (_vm.loading && _vm.newproduct.data.length > 18) ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
    ...
    staticClass: ["loading-over-txt"]
  }, [_vm._v("已经全部加载完毕")]) : _vm._e()])]) : _vm._e()], 1)], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
