...
var _goodsComing = __webpack_require__(4);

var _goodsComing2 = _interopRequireDefault(_goodsComing);
...
_goodsComing2.default.el = '#root';
new Vue(_goodsComing2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\home\\goodsComing.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-516c60d2"
...
module.exports = {
  ...
  "items-bottom1-left-time-img": {
    "width": "30",
    "height": "30",
    "marginRight": "14"
  },
  "items-bottom1-left-time-text": {
    "color": "#666666",
    "fontSize": "26"
  },
  "items-bottom-right": {
  ...
  "items-bottom1-right-color": {
    "backgroundColor": "#CF1126"
  },
  "items-bottom2-right-color": {
    "backgroundColor": "#999999"
  },
  "items-bottom2-left-title": {
    "color": "#333333",
    "fontSize": "26"
  },
  "items-bottom2-left-time": {
    "width": "140",
    "marginRight": "90",
    "flexDirection": "row",
    "justifyContent": "space-between",
  ...
  "items-bottom2-left-time-text2": {
    "color": "#333333",
    "fontSize": "28"
  },
  "items-bottom3-left-title": {
    "color": "#333333",
    "fontSize": "26"
  },
  "items-bottom3-right-color": {
    "backgroundColor": "#CF1126"
  },
  "items-bottom3-right-color2": {
    "color": "#666666",
    "backgroundColor": "#ffffff",
    "borderStyle": "solid",
    "borderColor": "#999999",
...
var _wxcTabPage = __webpack_require__(10);
...
var _titleBar = __webpack_require__(37);
...
var bottom = 0;
exports.default = {
    components: { WxcTabPage: _wxcTabPage2.default, titleBar: _titleBar2.default, refresher: _refresh2.default, line: _line2.default, empty: _empty2.default, WxcMask: _WxcMask2.default },
    data: function data() {
        return {
            bottom: bottom,
            height: height,
            tabPageHeight: 0,
            title: '到货',
            tabTitles: [{ title: '最新到货' }, { title: '即将到货' }],
            tabStyles: {
                bgColor: '#FFFFFF',
                titleColor: '#333333',
            ...
            new_arrival: null, //即将到货
            countDownInterval: '',
            showNewArrival: false, //是否加载即将到货,进来默认不加在
            subscribeArrivalData: '', //当前订阅数据
            subscribeParentIndex: -1,
            subscribeIndex: -1
        };
    },
    ...
    methods: {
        ...
        wxcTabPageCurrentTabSelected: function wxcTabPageCurrentTabSelected(e) {
            var _this = this;
            var index = e.page;
            if (_this.indexPage === index) {
                return;
            }
            _this.indexPage = index;
            switch (_this.indexPage) {
                case 0:
                    if (!_this.latest_arrival.data || _this.latest_arrival.data.length <= 0) {
                        _this.getLatestArrival();
                    }
                    break;
                case 1:
                    _this.showNewArrival = true;
                    if (!_this.new_arrival || _this.new_arrival.length <= 0) {
                        _this.getNewArrival();
                    }
                    break;
            }
        },
        getNewArrival: function getNewArrival() {
            var _this = this;
            _this.showNewArrival = true;
            _this.Get(_this.api().new_arrival, {}, function (res) {
                if (res.data.code == '0') {
                    _this.new_arrival = res.data.data;
                    _this.new_arrival.forEach(function (data, index, array) {
                        data.item_data.forEach(function (item_data, i, dataArray) {
                            var time = item_data.arrive_at_time - item_data.server_time;
                            var timer = _this.getTimeDifferences(time);
                            item_data.sale_time_str = timer.day + '天' + timer.hour + '小时';
                        });
                    });
                } else {
        ...
        getLatestArrival: function getLatestArrival() {
            var _this = this;
            _this.Get(_this.api().latest_arrival, {}, function (res) {
                if (res.data.code == '0') {
                    clearInterval(_this.countDownInterval);
                    _this.countDownInterval = null;
                    _this.latest_arrival = res.data.data;
                    _this.latest_arrival.latestLength = res.data.data.latest.length;
                    _this.latest_arrival.data = res.data.data.latest;
                    _this.latest_arrival.length = res.data.data.latest.length;
                    if (res.data.data.history && res.data.data.history.length > 0) {
                        _this.latest_arrival.data.push({ isHistory: true, item_data: res.data.data.history });
                        _this.latest_arrival.length++;
                    }
        ...
        startCountDown: function startCountDown() {
            var _this = this;
            aa();
            _this.countDownInterval = setInterval(aa, 1000);
            function aa() {
                if (_this.server_time != 0) {
                    _this.server_time++;
                }
                var isShowInterval = false;
                for (var i = 0; i < _this.latest_arrival.latestLength; i++) {
                    var arrival = _this.latest_arrival.data[i];
                    var item_data = arrival.item_data;
                    item_data.forEach(function (data, index, array) {
                        if (data.online_status === 1) {
                            if (_this.server_time == 0) {
                                _this.server_time = data.server_time;
                            }
                            var sale_at_time = data.sale_at_time;
                            if (sale_at_time <= _this.server_time) {
                                _this.$set(data, 'online_status', 0);
                            } else {
                                isShowInterval = true;
                                var time = sale_at_time - _this.server_time;
                                var timer = _this.getTimeDifferences(time);
                                timer.hour = timer.day * 24 + timer.hour;
                                timer.hourStr = timer.hour < 10 ? '0' + timer.hour : timer.hour;
                                _this.$set(data, 'timer', timer);
                            }
                        }
                    });
                }
        ...
        getTimeDifferences: function getTimeDifferences(usedTime) {
            ...
            value.dayStr = dayStr.toString();
            value.hourStr = hourStr.toString();
            value.minuteStr = minuteStr.toString();
            value.secondStr = secondStr.toString();
            value.day = days;
            value.hour = hours;
        ...
        sendSubscribe: function sendSubscribe(item, parentIndex, index) {
            this.subscribeArrivalData = item;
            this.subscribeParentIndex = parentIndex;
            this.subscribeIndex = index;
            this.isShowMask = true;
            this.changeSubscribe();
        },
        cancelSubscribe: function cancelSubscribe(item, parentIndex, index) {
            this.subscribeArrivalData = item;
            this.subscribeParentIndex = parentIndex;
            this.subscribeIndex = index;
            this.changeSubscribe();
        },
        changeSubscribe: function changeSubscribe() {
            var _this = this;
            _this.isShowMask = false;
            loadingModule.show({ msg: '请稍候...' });
            _this.Get(_this.api().arrival_subscribe, {
                goods_id: _this.subscribeArrivalData.goods_id,
                act_id: _this.subscribeArrivalData.act_id
            }, function (res) {
                loadingModule.cancel(function (ret) {
                    if (ret.result == 'dismiss') {
                        if (res.data.code == '0') {
                            _this.closeMask();
                            if (_this.subscribeArrivalData.is_subscribe === 0) {
                                modal.toast({ message: "提醒成功", duration: 5 });
                            } else {
                                modal.toast({ message: "取消提醒,你可能错过商品的发售期哦！", duration: 5 });
                            }

                            _this.subscribeArrivalData.is_subscribe = _this.subscribeArrivalData.is_subscribe === 0 ? 1 : 0;
                            _this.$set(_this.new_arrival[_this.subscribeParentIndex].item_data, _this.subscribeIndex, _this.subscribeArrivalData);
                        } else {
                            promptModule.showPrompt({ msg: res.data.msg, type: 'err' });
                        }
                    }
        ...
        showActive: function showActive(data) {
            this.jumpToBanner(data.act_name, data.act_url, '', '', '', '');
        }
    },
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    style: {
      height: (_vm.tabPageHeight - 100) + 'px'
    },
    attrs: {
      "loadmoreoffset": "10"
    },
  ...
  }), _vm._l((_vm.latest_arrival.data), function(item, index) {
    return (_vm.latest_arrival.data && _vm.latest_arrival.data.length > 0) ? _c('cell', {
      key: index,
      ...
      staticClass: ["title-time"]
    }, [(!item.isHistory) ? _c('image', {
      staticClass: ["time-img"],
      attrs: {
        "src": "http://img.abmau.com/abmau/04fd1b9b52cf2a17bf7c10d391f0379e.png"
      }
    }) : _vm._e(), (item.isHistory) ? _c('text', {
      staticClass: ["title-time-text"]
    }, [_vm._v("往期到货")]) : _c('text', {
      staticClass: ["title-time-text"]
    }, [_vm._v(_vm._s(item.act_arrive_at) + "到货")])]), (!item.isHistory && item.act_image) ? _c('div', {
      staticClass: ["banner"],
      style: {
    ...
    })]) : _vm._e(), _vm._l((item.item_data), function(item_data, i) {
      return _c('div', {
        key: i,
        staticClass: ["items"],
        style: {
          borderBottomWidth: i < (item.item_data.length - 1) ? '1.4px' : '0',
          borderBottomLeftRadius: i < (item.item_data.length - 1) ? '0' : '8px',
          borderBottomRightRadius: i < (item.item_data.length - 1) ? '0' : '8px',
          borderTopLeftRadius: i === 0 && item.isHistory ? '8px' : '0',
          borderTopRightRadius: i === 0 && item.isHistory ? '8px' : '0'
        },
        on: {
          "click": function($event) {
            _vm.buy(item_data)
        ...
        staticClass: ["items-top"]
      }, [_c('div', {
        staticClass: ["items-top-left"]
      }, [_c('image', {
        staticClass: ["goods-img"],
        attrs: {
          "src": item_data.goods_image_url
        }
      }), (item_data.online_status === 0 && item_data.stock <= 0) ? _c('image', {
        staticClass: ["empty-status"],
        attrs: {
          "src": "http://img.abmau.com/abmau/9a403a8a58de343a9fa35c394891f633.png"
        }
      }) : _vm._e()]), _c('div', {
        staticClass: ["items-top-right"]
      }, [_c('text', {
        staticClass: ["goods-dec"]
      }, [_vm._v(_vm._s(item_data.goods_name))]), _c('div', {
        staticClass: ["goods-tags"]
      }, _vm._l((item_data.recom_label_name), function(tag, k) {
        return (item_data.recom_label_name &&
          item_data.recom_label_name.length > 0) ? _c('div', {
          key: k
        }, [(k < 3) ? _c('text', {
          staticClass: ["goods-tag-text"],
          style: {
            'background-color': tag.color
          }
        }, [_vm._v(_vm._s(tag.title))]) : _vm._e()]) : _vm._e()
      })), _c('div', {
        staticClass: ["goods-price"]
      }, [(item_data.shareprice != 0) ? _c('text', {
        staticClass: ["goods-shareprice"]
      }, [_vm._v(_vm._s(item_data.title) + " ¥" + _vm._s(item_data.shareprice))]) : _vm._e(), (item_data.shareprice == 0 && item_data.retail_tax == 'no_show') ? _c('text', {
        staticClass: ["goods-shareprice"]
      }, [_vm._v(_vm._s(item_data.retail_title) + " ¥" + _vm._s(item_data.retail_price))]) : _vm._e(), (item_data.shareprice == 0 && item_data.retail_tax != 'no_show') ? _c('text', {
        staticClass: ["goods-shareprice"]
      }, [_vm._v(_vm._s(item_data.retail_title) + " ¥" + _vm._s(item_data.retail_price) + " ")]) : _vm._e(), (false) ? _c('text', {
        staticClass: ["goods-shareprice-hint"]
      }, [_vm._v("(" + _vm._s(item_data.retail_tax_title) + "¥" + _vm._s(item_data.retail_tax) + ")")]) : _vm._e(), _c('div', {
        staticClass: ["retail-price-parent"]
      }, [_c('text', {
        staticClass: ["retail-price"]
      }, [_vm._v(_vm._s(item_data.price_title) + " ¥" + _vm._s(item_data.price))]), _c('text', {
        staticClass: ["retail-price"]
      }, [_vm._v(" (")]), _c('text', {
        staticClass: ["retail-disused-price"]
      }, [_vm._v(" ¥" + _vm._s(item_data.market_price))]), _c('text', {
        staticClass: ["retail-price"]
      }, [_vm._v(")")])])]), _c('text', {
        staticClass: ["goods-num-text"]
      }, [_vm._v("到货数量：" + _vm._s(item_data.quantity) + "件")])])]), _c('div', {
        staticClass: ["items-bottom"]
      }, [_c('div', {
        staticClass: ["items-bottom-left"]
      }, [(item_data.online_status == 1　 && (item_data.sale_at_time - item_data.server_time) > 1 && item_data.timer) ? _c('div', {
        style: {
          flexDirection: 'row',
          alignItems: 'center'
        }
      }, [_c('text', {
        staticClass: ["items-bottom3-left-title"],
        style: {
          color: '#333333'
        }
      }, [_vm._v("开售倒计时")]), _c('div', {
        staticClass: ["items-bottom2-left-time"],
        style: {
          marginLeft: '20px'
        }
      }, [_c('text', {
        staticClass: ["items-bottom2-left-time-text"]
      }, [_vm._v(_vm._s(item_data.timer.hourStr))]), _c('text', {
        staticClass: ["items-bottom2-left-time-text2"]
      }, [_vm._v(":")]), _c('text', {
        staticClass: ["items-bottom2-left-time-text"]
      }, [_vm._v(_vm._s(item_data.timer.minuteStr))]), _c('text', {
        staticClass: ["items-bottom2-left-time-text2"]
      }, [_vm._v(":")]), _c('text', {
        staticClass: ["items-bottom2-left-time-text"]
      }, [_vm._v(_vm._s(item_data.timer.secondStr))])])]) : _c('div', {
        style: {
          flexDirection: 'row'
        }
      }, [_c('image', {
        staticClass: ["items-bottom1-left-time-img"],
        attrs: {
          "src": "http://img.abmau.com/abmau/3d8b8a948991764eeb75b2744e52e406.png"
        }
      }), _c('text', {
        staticClass: ["items-bottom3-left-title"],
        style: {
      ...
      }, [_vm._v("预计下次到货时间" + _vm._s(item_data.next_arrive_at))])])]), (item_data.online_status == 1　 && (item_data.sale_at_time - item_data.server_time) > 1) ? _c('text', {
        staticClass: ["items-bottom-right"],
        style: {
          color: '#FFFFFF',
          backgroundColor: '#999999',
          borderRadius: '6px'
        }
      }, [_vm._v("即将开售")]) : _vm._e(), (item_data.online_status == 1 && (item_data.sale_at_time - item_data.server_time) < 1) ? _c('text', {
        staticClass: ["items-bottom-right"],
        style: {
          color: '#FFFFFF',
          backgroundColor: '#999999',
      ...
      }, [_vm._v("立即购买")]) : _vm._e(), (item_data.online_status != 1 && item_data.stock > 0) ? _c('text', {
        staticClass: ["items-bottom-right", "items-bottom3-right-color"],
        style: {
          borderRadius: '6px'
        },
        on: {
  ...
  }), (_vm.latest_arrival.data && _vm.latest_arrival.data.length <= 0) ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('empty')], 1) : _vm._e()], 2), _c('list', {
    key: _vm.key,
    staticClass: ["item-container"],
    style: {
      height: (_vm.tabPageHeight - 100) + 'px'
  ...
  }), _vm._l((_vm.new_arrival), function(item, index) {
    return (_vm.new_arrival && 　_vm.new_arrival.length > 0) ? _c('cell', {
      key: index,
      staticClass: ["list-item"],
      appendAsTree: true,
      attrs: {
      ...
      staticClass: ["title-time-text"]
    }, [_vm._v(_vm._s(item.act_arrive_at) + "即将到货")])]), (item.act_image) ? _c('div', {
      staticClass: ["banner"],
      on: {
        "click": function($event) {
          _vm.showActive(item)
      ...
      return _c('div', {
        ...
        staticClass: ["goods-shareprice"]
      }, [_vm._v(_vm._s(item_data.title) + " ¥" + _vm._s(item_data.shareprice))]) : _vm._e(), (item_data.shareprice == 0 &&
        item_data.retail_tax == 'no_show') ? _c('text', {
        staticClass: ["goods-shareprice"]
      }, [_vm._v(_vm._s(item_data.retail_title) + " ¥" + _vm._s(item_data.retail_price))]) : _vm._e(), (item_data.shareprice == 0 && item_data.retail_tax != 'no_show') ? _c('div', [_c('text', {
        staticClass: ["goods-shareprice"]
      }, [_vm._v(_vm._s(item_data.retail_title) + " ¥" + _vm._s(item_data.retail_price) + " ")]), (false) ? _c('text', {
        staticClass: ["goods-shareprice-hint"]
      }, [_vm._v("(" + _vm._s(item_data.retail_tax_title) + "¥" + _vm._s(item_data.retail_tax) + ")")]) : _vm._e()]) : _vm._e(), _c('div', {
        staticClass: ["retail-price-parent"]
      }, [_c('text', {
        staticClass: ["retail-price"]
      }, [_vm._v(_vm._s(item_data.price_title) + " ¥" + _vm._s(item_data.price))]), _c('text', {
        ...
        staticClass: ["items-bottom3-left-title"]
      }, [_vm._v("到货倒计时：" + _vm._s(item_data.sale_time_str))])]), (item_data.is_subscribe == 0) ? _c('text', {
        staticClass: ["items-bottom-right"],
        style: {
          color: '#FFFFFF',
          backgroundColor: '#64B650',
          borderRadius: '6px'
        },
        on: {
          "click": function($event) {
            _vm.sendSubscribe(item_data, index, i)
          }
        }
      }, [_vm._v("到货提醒")]) : _c('text', {
        staticClass: ["items-bottom-right"],
        ...
        on: {
          "click": function($event) {
            _vm.cancelSubscribe(item_data, index, i)
          }
        }
      }, [_vm._v("取消提醒")])])])
    })], 2) : _vm._e()
  ...
  }), (_vm.new_arrival && 　_vm.new_arrival.length <= 0) ? _c('cell', {
    appendAsTree: true,
    attrs: {
      "append": "tree"
    }
  }, [_c('empty')], 1) : _vm._e()], 2)])], 1)
},staticRenderFns: []}
module.exports.render._withStripped = true
