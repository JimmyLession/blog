...
var _welfareDetail = __webpack_require__(2);

var _welfareDetail2 = _interopRequireDefault(_welfareDetail);
...
_welfareDetail2.default.el = '#root';
new Vue(_welfareDetail2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\home\\welfareDetail.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-15ae94dd"
...
var promptModule = weex.requireModule('promptModule');
exports.default = {
    components: { CountDown: _countDown2.default, line: _line2.default },
    data: function data() {
        return {
            data: {},
            height: height,
    ...
    methods: {
        ...
        //     if(self.shareData && self.goods.no_share == 0){
        //         utilModule.getABMShareView({
        //             'share_type': 'share_goods',
        //             'title': self.shareData.title,
        //             'desc': self.shareData.desc,
        //             'webUrl': self.shareData.share_url,
        //             'imageUrl': self.shareData.goods_img,
        //             'copy_item': self.shareData.share_url,
        //         });
        //     }else{
        //     }
        // },
        clickBrandInfo: function clickBrandInfo(url) {
            this.jumpToBanner('品牌介绍', url);
        },
        clickDetail: function clickDetail(item, goods_id) {
            if (this.data.server_time >= this.data.end_time) return;
            if (goods_id != 0) {
                if (this.compareVersion(weex.config.env.appVersion, '3.4.0')) {
        ...
        fetch: function fetch(countDownRef, id) {
            var self = this;
            self.Get(self.api().welfare_brand, { id: id }, function (res) {
                if (res.data.code == 0) {
                    self.data = res.data.data;
                    self.banner_height = 750 / (self.data.width / self.data.height);
                    if (countDownRef != null) {
                        var time_end_timestamp = self.data.end_time - self.data.server_time < 4 ? 0 : self.data.end_time - self.data.server_time;
                        var time_start_timestamp = self.data.start_time - self.data.server_time;
                        var time = time_start_timestamp >= 4 ? time_start_timestamp : time_end_timestamp;
                        self.$refs.countDown.left_title = time_start_timestamp >= 4 ? '距开始' : '距结束';
                        self.data.server_time = time_start_timestamp >= 4 ? self.data.start_time : time_end_timestamp < 4 ? self.data.end_time : self.data.server_time + time_start_timestamp;
                        self.$refs.countDown.startCountDownDate(time);
                    }
                } else {
        ...
        countDownFinish: function countDownFinish(e) {
            this.fetch(e.ref, this.brand_id);
        }
    },
    created: function created() {
        ...
        _this.isIpx(_this);
        _this.brand_id = _this.GetUrlPara(weex.config.bundleUrl, 'id');
        _this.fetch(null, _this.brand_id);
    }
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('div', {
    ...
    attrs: {
      "src": _vm.data.image_thumb_url
    }
  }), (_vm.data.end_time > _vm.data.server_time) ? _c('count-down', {
    ref: "countDown",
    staticClass: ["countDown_bg"],
    attrs: {
      "left_title": _vm.data.start_time > _vm.data.server_time ? '距开始' : '距结束',
      "time": _vm.data.start_time > _vm.data.server_time ? _vm.data.start_time - _vm.data.server_time : _vm.data.end_time - _vm.data.server_time
    },
    on: {
      "countDownFinish": _vm.countDownFinish
    }
  }) : _vm._e()], 1), (_vm.data.server_time >= _vm.data.end_time) ? _c('div', {
    staticStyle: {
      position: "absolute",
      width: "321px",
      height: "233px",
  ...
  }, [_vm._v("本场已结束 ")])]) : _vm._e(), (_vm.data.notice != null && _vm.data.notice.length > 0) ? _c('div', {
    staticStyle: {
      backgroundColor: "#222222",
      width: "750px",
      padding: "21px 30px 24px 49px",
      justifyContent: "space-between"
    }
  }, [(_vm.data.notice.length > 0) ? _c('text', {
    staticStyle: {
      lines: "2",
      fontSize: "26px",
      color: "#fff"
    }
  }, [_vm._v(_vm._s(_vm.data.notice))]) : _vm._e(), (_vm.data.url && _vm.data.url.length > 0) ? _c('text', {
    staticStyle: {
      fontSize: "22px",
      color: "#fff",
      opacity: "0.8",
    ...
    on: {
      "click": function($event) {
        _vm.clickBrandInfo(_vm.data.url)
      }
    }
  }, [_vm._v("查看品牌介绍 >")]) : _vm._e()]) : _vm._e()]), _c('cell', {
    staticStyle: {
      backgroundColor: "#333333",
      paddingLeft: "25px",
      paddingRight: "25px",
    ...
    return _c('div', {
    ...
    }, [_vm._v("限量" + _vm._s(item.quantity) + "件")])])]), _c('div', {
      staticStyle: {
      ...
      staticClass: ["content_bottom_price"]
    }, [_vm._v(")")]) : _vm._e()]), (_vm.data.start_time > _vm.data.server_time) ? _c('text', {
    ...
    }, [_vm._v("即将开售")]) : _vm._e(), (item.stock == 0 && _vm.data.start_time <= _vm.data.server_time && _vm.data.end_time > _vm.data.server_time) ? _c('text', {
      staticClass: ["content_bottom_buy_button"],
      style: {
    ...
    }, [_vm._v("已售罄")]) : _vm._e(), (item.type != 4 && item.stock > 0 && _vm.data.server_time >= _vm.data.start_time && _vm.data.end_time > _vm.data.server_time) ? _c('text', {
      staticClass: ["content_bottom_buy_button"],
      style: {
    ...
    }, [_vm._v("立即抢购")]) : _vm._e(), (_vm.data.end_time <= _vm.data.server_time) ? _c('text', {
      staticClass: ["content_bottom_buy_button"],
      style: {
        backgroundColor: '#999999'
      },
