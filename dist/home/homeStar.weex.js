...
var _homeStar = __webpack_require__(4);

var _homeStar2 = _interopRequireDefault(_homeStar);
...
_homeStar2.default.el = '#root';
new Vue(_homeStar2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\home\\homeStar.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-571417be"
...
var _homeStarConfig = __webpack_require__(24);

var _homeStarConfig2 = _interopRequireDefault(_homeStarConfig);

var _titleBar = __webpack_require__(25);
...
var height = 0;
exports.default = {
  components: { title: _titleBar2.default, WxcTabPage: _wxcTabPage2.default, line: _line2.default, refresher: _refresh2.default, empty: _empty2.default },
  data: function data() {
    return {
      height: height,
      page: [1, 1],
      ...
      tabTitles: [],
      loading: [false, false],
      tabStyles: _homeStarConfig2.default.tabStyles,
      supportSlide: false,
      isTabView: true,
      needSlider: false,
      empty: [false, false],
      allLoadingFinish: [false, false],
      index: 0, //选中的分类index
      cidSelected: -2, //选中的分类id
      tabArray: [], //分类集合
      productArray: [], //选中的id对应的产品列表集合
      strTitle: ''
    };
  },
  ...
  methods: {
    ...
    onloading: function onloading() {
      if (!this.loading && this.tabList[this.index].length < this.size) return;
    ...
    //请求上面的tab分类tittle - tabTitles
    requestToGetTabTittles: function requestToGetTabTittles() {
      var _this = this;
      _this.Get(_this.api().starcategory, {}, function (res) {
        if (res.data.code != '0') {
          //请求数据失败
          modal.toast({ message: res.data.msg, duration: 3 });
        } else {
          var resDict = {};
          resDict = res.data.data;
          for (var i = 0; i < resDict.starcategory.data.length; i++) {
            _this.tabTitles.push({ 'title': resDict.starcategory.data[i].name });
          }
          _this.tabArray = resDict.starcategory.data;
          //定义二维数组
          _this.tabList = [].concat(_toConsumableArray(Array(_this.tabTitles.length).keys())).map(function (i) {
            return [];
          });
          _this.startLoading(0);
        }
      });
    },
    ...
    startWeb: function startWeb(title, url, shareTitle, shareDes, shareUrl, shareImg) {
      titleBar.startWeb({
        title: title,
        url: url,
        shareUrl: shareUrl,
    ...
    startLoading: function startLoading(index) {
      var _this = this;
      var selectCid = _this.tabArray[index].cid;
      //获取明星产品列表
      _this.Get(_this.api().starproduct, {
        size: _this.size,
        ...
        if (res.data.code != '0') {
          ...
          _this.strTitle = res.data.data.starproduct.title;
          if (res.data.data.starproduct.data.length == _this.size) {
          ...
          for (var arr in res.data.data.starproduct.data) {
            _this.data.push(res.data.data.starproduct.data[arr]);
          }
  ...
  return _c('div', [_c('title', {
    ...
    return _c('list', {
      ...
      return _c('cell', {
        ...
        on: {
          "click": function($event) {
            _vm.startWeb(_vm.strTitle, item.url, item.sharetitle, item.sharecontent, item.sharelink, item.shareimg)
          }
        }
      }, [_c('div', {
        staticClass: ["content"]
        ...
        staticClass: ["tabel"]
      }, _vm._l((item.goods_tag), function(goodsTag, index) {
        return _c('div', {
          key: index,
          staticClass: ["tabel_infor"],
          attrs: {
            "accessible": true
          }
        }, [_c('div', {
          staticClass: ["tag"],
          style: {
            'background-color': goodsTag.color
          }
        }, [_c('text', {
          staticClass: ["text"]
        }, [_vm._v(_vm._s(goodsTag.title))])])])
      })), _c('div', {
      ...
      }, [_vm._v("优惠价 ¥" + _vm._s(item.disprice))]), _c('text', {
        staticClass: ["goods_retail_price"]
      }, [_vm._v("售价 ¥" + _vm._s(item.price))])])])])])])]), _c('line')], 1)
    }), (_vm.loading[index] && v.length > 18) ? _c('cell', {
      appendAsTree: true,
      attrs: {
        "append": "tree"
      }
      ...
      staticClass: ["loading-over-txt"]
    }, [_vm._v("已经全部加载完毕")]) : _vm._e()])]) : _vm._e(), (_vm.empty[index]) ? _c('cell', {
      style: {
        height: (_vm.height - _vm.tabStyles.height) + 'px'
      },
      appendAsTree: true,
