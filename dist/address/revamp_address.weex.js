...
var _revamp_address = __webpack_require__(2);

var _revamp_address2 = _interopRequireDefault(_revamp_address);
...
_revamp_address2.default.el = '#root';
new Vue(_revamp_address2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\address\\revamp_address.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-53b5dc7b"
...
var _wxSwitch = __webpack_require__(13);
...
var address_ms = new BroadcastChannel('choose_address');
var loadingModule = weex.requireModule('loadingModule');
var height = 0; //所能用到的最大高度
var bottom = 0;
exports.default = {
  components: { WxSwitch: _wxSwitch2.default, line: _line2.default, title: _titleBar2.default },
  data: function data() {
    return {
      address_info: null,
      title: "",
  ...
  methods: {
    ...
    finishAction: function finishAction() {
      var _this = this;
      if (_this.name == "") {
        promptModule.showPrompt({ msg: "请输入姓名", type: "prompt" });
        return;
      } else if (_this.mobile == "") {
        promptModule.showPrompt({ msg: "请输入手机号码", type: "prompt" });
        return;
      } else if (_this.province_id == 0 || this.city_id == 0 || this.district_id == 0) {
        promptModule.showPrompt({ msg: "请选择省份", type: "prompt" });
        return;
      } else if (_this.addressInfo == "") {
        promptModule.showPrompt({ msg: "请输入地址", type: "prompt" });
        return;
      } else {
        var params = {
          address: _this.addressInfo,
          city: _this.city_id,
          district: _this.district_id,
          idcard_name: _this.id_card_name,
          idcard_no: _this.id_card_no,
          idcard_upper: _this.faceId,
          idcard_underside: _this.backId,
        ...
        if (_this.id != 0) params["id"] = _this.id;
        loadingModule.show({ msg: '正在提交...' });
        _this.Post(_this.api().user_brandprovider_address, params, function (res) {
          loadingModule.cancel(function (ret) {
            var result = ret.result;
            if (result == 'dismiss') {
              if (res.data.code == 0) {
                console.log("type:" + _this.entryType);
                if (_this.entryType == 'default') {
                  Steve.postMessage(_this.eventBusTag().REFRESH_MYDEALER);
                  _this.popBar();
                } else if (_this.entryType == 'edit') {
                  Steve.postMessage(_this.eventBusTag().REFRESH_MYDEALER);
                  if (_this.address_id == _this.id) {
                    storage.setItem('address_id', res.data.data.id + "", function (event) {
                      if (event.result == 'success') {
                        address_ms.postMessage(_this.eventBusTag().REFRESH_ADDRESS);
                        _this.popBar();
                      }
                    });
                  } else {
                ...
                } else if (_this.entryType == 'add' || _this.entryType == 'list') {
                  storage.setItem('address_id', res.data.data.id + "", function (event) {
                    if (event.result == 'success') {
                      address_ms.postMessage(_this.eventBusTag().REFRESH_ADDRESS);
                      if (_this.entryType == 'add') {
                        Steve.postMessage(_this.eventBusTag().REFRESH_MYDEALER);
                        _this.popBar();
                      } else if (_this.entryType == 'list') {
                        _this.kill(2);
                      }
                    }
                  });
    ...
    chooseAddress: function chooseAddress() {
      ...
      var city;
      var district;
      utilModule.getAddressInfo({ address: _this.address }, function (data) {
        province = data.province;
        city = data.city;
        district = data.district;
        _this.province_id = data.provinceId;
        _this.city_id = data.cityId;
        _this.district_id = data.districtId;
        _this.addressText = province + " " + city + " " + district;
      });
    },

    //添加身份证正面
    takeFacePhoto: function takeFacePhoto() {
      var _this = this;
      var titlesDict = { caram: "拍照", photoFile: "从手机相册选择" };
      comModal.uploadPhoto({ titlesDict: titlesDict, isIdentification: true }, function (data) {
        _this.showFace = false;
        _this.faceId = data.id;
        if (data.url) _this.faceImg = data.url;else _this.faceImg = _this.defaultImg;
      });
    },


    //添加身份证反面
    takeBackPhoto: function takeBackPhoto() {
      var _this = this;
      var titlesDict = { caram: "拍照", photoFile: "从手机相册选择" };
      comModal.uploadPhoto({ titlesDict: titlesDict, isIdentification: true }, function (data) {
        _this.showBack = false;
        _this.backId = data.id;
        if (data.url) _this.backImg = data.url;else _this.backImg = _this.defaultImg;
      });
    },
    ...
    handleChange: function handleChange(checked) {
      ...
      if (this.checked) this.is_first = 1;else this.is_first = 0;
    }
  },
  created: function created() {
    this.getTopHeight(this);
    this.isIpx(this);
    this.entryType = this.GetUrlPara(weex.config.bundleUrl, 'type');
    if (this.entryType == 'undefined') this.entryType = 'default';
    this.id = this.GetUrlPara(weex.config.bundleUrl, 'id');
    this.address_id = this.GetUrlPara(weex.config.bundleUrl, 'address_id');
    if (this.id == 'undefined') this.id = 0;
    this.address_info = this.GetUrlPara(weex.config.bundleUrl, 'address');
    this.title = decodeURIComponent(this.GetUrlPara(weex.config.bundleUrl, "title"));
    if (this.id != 0 && this.address_info != 'undefined') {
      var data = JSON.parse(this.address_info);
      this.name = data.name;
      this.mobile = data.mobile;
      this.addressInfo = data.address;
      this.addressText = data.province.name + "," + data.city.name + "," + data.district.name;
      this.province_id = data.province.id;
      this.city_id = data.city.id;
      this.district_id = data.district.id;
      if (data.is_first == 1) this.checked = true;else this.checked = false;
      if (data.idcard) {
        if (data.idcard.idcard_name) this.id_card_name = data.idcard.idcard_name;
        if (data.idcard.idcard_no) this.id_card_no = data.idcard.idcard_no;
        if (data.idcard.idcard_upper) this.faceImg = data.idcard.idcard_upper;
        if (data.idcard.idcard_upper_id) this.faceId = data.idcard.idcard_upper_id;
        if (this.faceImg != "") this.showFace = false;
        if (data.idcard.idcard_underside) this.backImg = data.idcard.idcard_underside;
        if (data.idcard.idcard_underside_id) this.backId = data.idcard.idcard_underside_id;
      }
      if (this.backImg != "") this.showBack = false;
      this.is_first = data.is_first;
      if (this.id_card_name.length > 0 || this.id_card_no.length > 0) this.showIdentity = true;
      if (this.faceId != 0 || this.backId != 0) this.photoShow = true;
    }
  },
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\components\\wx-switch.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-d0159c42"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    ...
    style: {
      'background-color': _vm.checked ? _vm.checkedColor : '#999999'
    },
    on: {
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('title', {
    ...
    staticClass: ["content_background"]
  }, [_c('text', {
    staticClass: ["left"],
    staticStyle: {
      color: "#cf1126"
  ...
  }, [_vm._v(_vm._s(_vm.addressText))])]), _c('line'), _c('div', {
    staticClass: ["content_background"]
  }, [_c('text', {
    staticClass: ["left"],
    staticStyle: {
    ...
    on: {
      "input": function($event) {
        _vm.addressInfo = $event.target.attr.value
      }
    }
  })]), _c('div', {
    staticClass: ["gray_background"]
  }), _c('div', {
    staticClass: ["switch_background"]
  }, [_vm._m(0), _c('wx-switch', {
    staticClass: ["switch"],
    attrs: {
      "slot": "right",
      "value": _vm.checked,
  ...
  }, [_vm._m(1), (!_vm.showIdentity) ? _c('image', {
    staticClass: ["morex"],
    attrs: {
  ...
  }) : _vm._e()]), (_vm.showIdentity) ? _c('div', [_c('div', {
    staticClass: ["content_background"]
  }, [_c('text', {
    staticClass: ["left_content"],
    staticStyle: {
  ...
  }, [_vm._v("身份证名字")]), _c('input', {
    staticClass: ["input"],
    attrs: {
      "placeholder": "请输入身份证所属姓名",
      "type": "text",
      "value": (_vm.id_card_name)
    },
    on: {
      "input": function($event) {
        _vm.id_card_name = $event.target.attr.value
      }
    }
  })]), _c('line'), _c('div', {
    staticClass: ["content_background"]
    ...
    attrs: {
      "placeholder": "请输入身份证号",
      "type": "text",
      "value": (_vm.id_card_no)
    },
    on: {
      "input": function($event) {
        _vm.id_card_no = $event.target.attr.value
      }
    }
  })]), _c('line'), _c('div', {
    staticClass: ["switch_background"],
    on: {
      "click": function($event) {
        _vm.photoShow = !_vm.photoShow
      }
    }
  }, [_c('text', {
    staticClass: ["left_content_bottom"]
  }, [_vm._v("身份证正反面照片")]), (!_vm.photoShow) ? _c('image', {
    staticClass: ["morex"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
  }) : _vm._e(), (_vm.photoShow) ? _c('image', {
    staticClass: ["mores"],
    attrs: {
      "src": "http://img.abmau.com/abmau/197260145f13acf70e06fe37ad4ba0c8.png"
    }
  }) : _vm._e()]), (_vm.photoShow) ? _c('div', {
    staticClass: ["photoDiv"]
  }, [_c('div', {
    staticClass: ["photo_parent"]
  }, [_c('div', {
  ...
  }), (_vm.showFace) ? _c('image', {
    staticClass: ["img"],
    attrs: {
      "src": "assets://icon_add_idcard"
    }
  }) : _vm._e()]), _c('div', {
    staticClass: ["photo_item"],
    on: {
  ...
  }) : _vm._e()])]), _vm._m(2), _c('text', {
    staticClass: ["bottom_text_hint"]
  }, [_vm._v("注意：请上传原始比例的身份证正反面照，请勿裁剪涂改，确保身份证信息清晰显示，否则无法通过审核")])]) : _vm._e()], 1) : _vm._e(), _c('line'), _c('text', {
    staticClass: ["bottom_title"]
  }, [_vm._v("为什么要上传身份证信息？")]), _c('text', {
    staticClass: ["bottom_text"]
  }, [_vm._v("根据海关相关规定，购买跨境商品需要办理清关手续；上传身份证信息，可确保您购买的商品顺利通过海关检查。单创承诺：您所传身份证信息仅用于办理跨境商品清关手续，系统加密保存，他人无法获取相关信息。")])], 1)])], 1)
  ...
  return _c('div', {
    ...
    staticClass: ["set_default_info"]
  }, [_vm._v("（注:每次下单时都会使用该地址）")])])
},function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["bottom_right"]
  }, [_c('text', {
    staticClass: ["left_content_bottom"]
  }, [_vm._v("身份证信息")]), _c('text', {
    staticClass: ["info_content"]
  }, [_vm._v("选填（仅用于保税仓和澳洲仓订单）")])])
},function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', {
    staticClass: ["hint_item"]
  }, [_c('text', {
    staticClass: ["img_hint"]
  }, [_vm._v("添加身份证正面")]), _c('text', {
    staticClass: ["img_hint"]
  }, [_vm._v("添加身份证反面")])])
}]}
module.exports.render._withStripped = true
