...
var _dc_login = __webpack_require__(2);

var _dc_login2 = _interopRequireDefault(_dc_login);
...
_dc_login2.default.el = '#root';
new Vue(_dc_login2.default);
...
__vue_options__.__file = "F:\\weex\\weex_develop\\abm_dev_weex\\abm-weexSource\\src\\pages\\login\\dc_login.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-63749859"
...
exports.default = {
    ...
    methods: {
        onInputMobile: function onInputMobile(e) {
            ...
            this.isParam = this.isValueParam(this, [this.applyMobile, this.applyPsw]);
            console.log("this.isValueParam--" + this.isValueParam);
        },
        ...
        forgetPsw: function forgetPsw() {
            this.startAnim('login/forgetPasswordFirstStep', { type: this.type }, 2);
        },
        finish: function finish() {
            if (this.type == "add") this.popBar();else this.finishAnim();
        },
        clickAgreement: function clickAgreement() {
            this.jumpTo2('web/webView', '单创用户隐私政策', { title: "单创用户隐私政策", url: encodeURIComponent("https://wap.abmau.com/privacy_of_individual.html") });
        },
        ...
        clickLogin: function clickLogin() {
            ...
            if (_this.applyPsw == "") {
                promptModule.showPrompt({ msg: '请输入登录密码', type: 'prompt' });
                return;
            }
            _this.md5(_this.applyPsw, function (ret) {
                _this.password = ret.md5;
                _this.register_id = ret.register_id;
                loadingModule.show({ msg: '正在登录...' });
                _this.Post(_this.api().login, {
                    mobile: areaCode + _this.applyMobile,
                    password: _this.password,
                    register_id: _this.register_id
                }, function (res) {
                    loadingModule.cancel(function (ret) {
                        var result = ret.result;
                        if (result == 'dismiss') {
                            if (res.data.code == '0') {
                                if (_this.compareVersion(weex.config.env.appVersion, '3.0.5')) _this.setToken(res.data.data.token);
                                titleBar.loginSuccess({ type: _this.type, id: res.data.data.id, name: res.data.data.name, invite_code: res.data.data.invite_code, nick_name: res.data.data.nickname, heading: res.data.data.heading, verify: res.data.data.verify, token: res.data.data.token, mobile: res.data.data.mobile, level_name: res.data.data.level_name, status: res.data.data.status });
                                if (_this.type == "add" && _this.compareVersion(weex.config.env.appVersion, '3.0.5')) {
                                    add.postMessage(_this.eventBusTag().REFRESH_ACCOUNT);
                                    utilModule.toggleAccount();
    ...
    created: function created() {
        ...
        if (this.type == 'login' && weex.config.env.platform.toLowerCase() === 'android') utilModule.toggleServeDialog();
        //提示
